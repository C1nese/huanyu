...
function getDbList() {
    var f = $("js-setting"),
        params="db_host=" + f["js-db-host"].value + "&"
            ...
            + "IS_AJAX_REQUEST=yes";
    try {
        var result = Ajax.call("/Install/index.php?s=/Install/get_db_list", params, null, "POST", "JSON", false);
    } catch (ex) {
        //alert(ex);
    }
...
function createConfigFile(app_list) {
    ...
    Ajax.call("/Install/index.php?s=/Install/create_config_file", params, function (result) {
...
function createDatabase(app_list) {
    ...
    Ajax.call("/Install/index.php?s=/Install/create_database", params, function (result) {
        if (result.replace(/\s+$/g, '') === "OK") {
            displayOKMsg();
...
function installBaseData(app_list) {
    ...
    Ajax.call("/Install/index.php?s=/Install/install_base_data", params, function (result) {
        if (result.replace(/\s+$/g, '') === "OK") {
            displayOKMsg();
...
function createAdminPassport(app_list) {
    ...
    Ajax.call("/Install/index.php?s=/Install/create_admin_passport", params, function (result) {
        if (result.replace(/\s+$/g, '') === "OK") {
            displayOKMsg();
            doOthers(app_list);
        } else {
...
function doOthers(app_list) {
    ...
    Ajax.call("/Install/index.php?s=/Install/do_others", params, function (result) {
        if (result.replace(/\s+$/g, '') === "OK") {
            displayOKMsg();
            goToDone();
        } else {
...
function goToDone() {
    ...
    window.setTimeout(function () {
        location.href = "/Install/index.php?s=/Install/index/step/done/lang/" + getAddressLang();
    }, 1000);
}

/* 在安装过程中调用该方法 */
function startNotice() {
    $("js-monitor-loading").src = "/Install/Common/images/loading.gif";
    $("js-monitor-wait-please").innerHTML = "<strong style='color:blue'>" + $_LANG["wait_please"] + "</strong>";
};

/* 安装完毕调用该方法 */
function stopNotice() {
    $("js-monitor-loading").src = "/Install/Common/images/loading2.gif";
