...
class ZhifubaoNotify {
	...
	function ZhifubaoNotify($partner,$security_code,$sign_type = "MD5",$_input_charset = "utf-8",$transport= "http") {
...
/**********************************对return_url的认证***************************************/
	function return_verify() {
		...
		while (list ($key, $val) = each ($sort_get)) {
			if($key != "sign" && $key != "sign_type" && $key != "_URL_" && $key != "type")
			$arg.=$key."=".$val."&";
		}
		$prestr = substr($arg,0,count($arg)-2);  //去掉最后一个&号
		$this->mysign = $this->sign($prestr.$this->security_code);
