...
class PayPal implements PayInterface
{
	...
	*/
	function __construct() 
	{
		...
		//读取数据库中的设置
		//币种
		$currency					= isset($data['paypal_currency'])?$data['paypal_currency']:'';
		$this->currency				= $currency?$currency:'';

		//api帐号
		$username					= isset($data['paypal_username'])?$data['paypal_username']:'';
		$this->username				= $username?$username:'';

		//api密码
		$password					= isset($data['paypal_password'])?$data['paypal_password']:'';
		$this->password				= $password?$password:'';

		//api签名
		$signature					= isset($data['paypal_signature'])?$data['paypal_signature']:'';
		$this->signature			= $signature?$signature:'';

		//美元对人民币汇率
		$exchange_rate_usd			= isset($data['paypal_exchange_rate_usd'])?$data['paypal_exchange_rate_usd']:'';
		$this->exchange_rate_usd	= $exchange_rate_usd?$exchange_rate_usd:'';

		//欧元对人民币汇率
		$exchange_rate_euro			= isset($data['paypal_exchange_rate_euro'])?$data['paypal_exchange_rate_euro']:'';
		$this->exchange_rate_euro	= $exchange_rate_euro?$exchange_rate_euro:'';
		
		//企业名称
		$company_name				= isset($data['paypal_company'])?$data['paypal_company']:'';
	...
	public static function getConfigInfo()
	{
		return array(
			...
			array(
				'config_name'=>'paypal_exchange_rate_usd',
				'config_value'=> '6.1865',
				'name'=>'美元汇率',
				'type'=>'text',
				'style'=>'width:100px',
			...
			array(
				'config_name'=>'paypal_exchange_rate_euro',
				'config_value'=> '8.0889',
				'name'=>'欧元汇率',
				'type'=>'text',
				'style'=>'width:100px',
			...
			array(
				'config_name'=>'paypal_currency',  //货币类型
				'config_value'=> '0',
				'name'=>'货币类型',
				'type'=>'radio',
				'options'=>array(
