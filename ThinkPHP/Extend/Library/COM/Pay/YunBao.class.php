...
class YunBao implements PayInterface
{
	...
	*/
	function __construct() 
	{
		...
		//读取数据库中的设置
		$Model						= M();
		$merchant_id				= isset($data['yun_merchant_id'])?$data['yun_merchant_id']:'';
		$this->merchant_id			= $merchant_id?$merchant_id:'';

		$merchant_key				= isset($data['yun_merchant_key'])?$data['yun_merchant_key']:'';
		$this->merchant_key			= $merchant_key?$merchant_key:'';

		$proxy_url					= isset($data['yun_proxy'])?$data['yun_proxy']:'';
		$this->proxy_url			= $proxy_url?$proxy_url:'';

		$rate                       = isset($data['yun_merchant_rate'])?$data['yun_merchant_rate']:1;
