...
class ZhiFuBao implements PayInterface
{
	...
	*/
	function __construct() 
	{
		...
		//读取数据库中的设置
		$Model						= M();
		$account					= isset($data['zhifubao_account'])?$data['zhifubao_account']:'';
		$key						= isset($data['zhifubao_key'])?$data['zhifubao_key']:'';
		$partner					= isset($data['zhifubao_partner'])?$data['zhifubao_partner']:'';
		$location_url				= isset($data['zhifubao_location'])?$data['zhifubao_location']:'';
	...
	public static function getConfigInfo()
	{
		...
		$location_url = $http_type.$_SERVER['HTTP_HOST'].'/Admin/notify.php';
	    
	
		return array(
			array(
				'config_name'=>'zhifubao',
				'config_value'=> '支付宝',
				'name'=>'支付方式名称',
				'type'=>'text',
				'style'=>'width:100px',
	...
	public function submit()
	{
		...
		<?php
		    ...
		    "anti_phishing_key"	=> '',
			"exter_invoke_ip"	=> '',
			"_input_charset"	=> $this->input_charset,		//字符集
		);
		//建立请求
	...
	*/
	public function receive($orderId='')
	{
		...
		$alipay_config['partner']		= $data['zhifubao_partner'];

		//安全检验码，以数字和字母组成的32位字符
		$alipay_config['key']			= $data['zhifubao_key'];
