...
class ShengPay implements PayInterface
{
	...
	*/
	function __construct() 
	{
		...
		//币种
		$currency = isset($data['ShengPay_name'])?$data['ShengPay_name']:'';
		$this->currency = $currency?$currency:'';

		//api帐号
		$username =isset($data['ShengPay_merchant'])?$data['ShengPay_merchant']:'';
		$this->username = $username?$username:'';

		//api密码
		$password =isset($data['ShengPay_key'])?$data['ShengPay_key']:'';
		$this->password = $password?$password:'';
		//代理url
		$proxy =isset($data['ShengPay_proxy'])?$data['ShengPay_proxy']:'';
		$this->ServerLocationUrl = $proxy?$proxy:'';
		//是否支持直连
		$credit = isset($data['ShengPay_credit'])?$data['ShengPay_credit']:'0';
