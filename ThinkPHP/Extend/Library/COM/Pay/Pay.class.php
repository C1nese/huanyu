...
class Pay
{
	...
	*/
	public function __construct($payname = NULL,$init = true,$money = 0,$type = '',$memo = '')
    {
		...
		//产生订单号
		$this->_orderId = $this->createOrderId($payname);
    	...
    	if( $payname && Pay::checkPayName($payname) )
		{
			...
			if($payname=='WangYin'){	
			  $url = "https://";
			  $url.=$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']."/Payment/receive/id/{$this->_orderId}";
			  $this->payment->setServerurl($url);
			}else{
			 $this->payment->setServerurl("$siteDomain/Payment/receive/id/{$this->_orderId}");
			}
			...
			//生成订单
			if( $init )$this->createOrder($this->_orderId,$money,$payname,$memo);
        }
    }
	...
	public function touchEvent($event,$orderId)
	{
		...
		if( $info )
		{
			...
			{
				//初始化事件不进行状态更新
				if( $event!='init' ) $Model->table('pay_order')->where("orderId='{$orderId}'")->setField('status',1);
	...
	public function createOrder($orderId,$money,$payment,$memo)
	{
		...
		if( !$Model->table('pay_order')->where($where)->find() )
		{
			...
			if($bank->bank_scale!=1){
				//$data['realmoney'] = number_format($money/$bank->bank_scale,2);
	...
	*/
	...
	{
		//获取类名
		import("COM.Pay.$payment");
		$payment_ob			= new $payment();
		$result				= array();

		$result["name"]		= call_user_func("$payment::getName");
		$result["memo"]		= call_user_func("$payment::getMemo");
		$result["config"]	= call_user_func("$payment::getConfigInfo");
		if( $payment_ob->isSupportCredit() )
		{
	...
	{
		if(CONFIG($payment.'_installed') == $payment)
		{
			return 1;
		}
		return 0;
	...
	{
		$file_path	= ROOT_PATH.'ThinkPHP/Extend/Library/COM/Pay';
	...
	*/
    ...
    //产生新支付订单编号
    private function createOrderId($name="")
    {
    	//判断是否是网银支付 如果是网银注册的话 订单编号需要修改
    	if($name=='WangYin'){
    		//首先获取网银的支付商户号
    		$orderid = $this->payment->v_mid;
         	return date('Ymd',time())."-".$orderid."-".date('His',time());
    	}else{
