...
class YeePay implements PayInterface
{
	...
	*/
	function __construct() 
	{
		...
		//读取数据库中的设置
		$Model						= M();
		$merchant_id				= isset($data['yee_merchant_id'])?$data['yee_merchant_id']:'';
		$this->merchant_id			= $merchant_id?$merchant_id:'';

		$merchant_key				= isset($data['yee_merchant_key'])?$data['yee_merchant_key']:'';
		$this->merchant_key			= $merchant_key?$merchant_key:'';

		$proxy_url					= isset($data['yee_proxy'])?$data['yee_proxy']:'';
		$this->proxy_url			= $proxy_url?$proxy_url:'';

		$rate                       = isset($data['yee_merchant_rate'])?$data['yee_merchant_rate']:1;
