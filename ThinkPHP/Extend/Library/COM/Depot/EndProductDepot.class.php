...
*
*/
class EndProductDepot implements DepotInterface
{
	private $error_info		= '';  //错误信息
	private $in_table		= 'erp_endproduct_depot_in';	//入库表名
	private $out_table		= 'erp_endproduct_depot_out';	//出库表名
	private $depot_table	= 'erp_endproduct_depot';		//库存表名

	/**
	*
	*	成品入库
	*	$id					 = '要入库的成品表主键' 支持数组方式 array(1,2,3,4,5,6);
	*	$data['number']		 = '入库数量'
	*	$data['remarks']	 = '备注'
	*
	*/
	...
	*
	*	成品出库
	*	$id					 = '要出库的成品表主键' 支持数组方式 array(1,2,3,4,5,6);
	*	$data['number']		 = '出库数量'
	*	$data['remarks']	 = '备注'
	*
	*/
	...
	*/
	public function update_stock($id,$number)
	{
		...
		//获取该成品对应的订单id和对应的订单产品id
		$info						= M()->table('erp_endproduct_register')->find($id);
		$data['order_id']			= $info?$info['order_id']:0;
		$data['order_product_id']	= $info?$info['order_product_id']:0;
	...
	*/
	public function make_in_seria_number($seria_number_suffix_length=4)
	{
		...
		//流水号前缀
		$seria_number_prefix	= 'EPI'.date('ymd');
	...
	*/
	public function make_out_seria_number($seria_number_suffix_length=4)
	{
		...
		//流水号前缀
		$seria_number_prefix	= 'EPO'.date('ymd');
