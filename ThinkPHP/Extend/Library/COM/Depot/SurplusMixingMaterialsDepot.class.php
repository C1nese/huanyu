...
*
*/
class SurplusMixingMaterialsDepot implements DepotInterface
{
	private $error_info		= '';  //错误信息
	private $in_table		= 'erp_surplus_mixing_materials_depot_in';	//入库表名
	private $out_table		= 'erp_surplus_mixing_materials_depot_out';	//出库表名
	private $depot_table	= 'erp_surplus_mixing_materials_depot';		//库存表名
	...
	{	
		...
		if( is_array($id) )
		{
			...
			foreach($id as $k=>$_id)
			{
				...
				if(is_array($remarks)){
					$data['remarks']	= $remarks[$k];
				}
				//检查是否有库存
				$data['mixing_id']		= $_id;		//剩余表id
				...
				layer					层次:内、外、中
				*/
				$mixing_reg				= M()->table('erp_surplus_mixingregister')->find($_id);
				if( $mixing_reg )
				{
			...
			layer					层次:内、外、中
			*/
			$mixing_reg				= M()->table('erp_surplus_mixingregister')->find($id);
			if( $mixing_reg )
			{
				$data['formula_id']			= $mixing_reg['formula_id'];
				$data['layer']				= $mixing_reg['layer'];
	...
	*/
	public function make_in_seria_number($seria_number_suffix_length=4)
	{
		...
		//流水号前缀
		$seria_number_prefix	= 'SI'.date('ymd');
	...
	*/
	public function make_out_seria_number($seria_number_suffix_length=4)
	{
		...
		//流水号前缀
		$seria_number_prefix	= 'SO'.date('ymd');
