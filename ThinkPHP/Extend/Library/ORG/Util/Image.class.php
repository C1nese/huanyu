...
class Image {
     ...
     * @return string
      +----------------------------------------------------------
     */
    static function buildImageVerify($length=4, $mode=1, $type='png', $width=80, $height=22, $verifyName='verify') {
        import('ORG.Util.String');
        $randval = String::randString($length, $mode);
        $_SESSION[$verifyName] = md5(strtolower($randval));
        $width = ($length * 4 + 2) > $width ? $length * 4 + 2 : $width;
        if ($type != 'gif' && function_exists('imagecreatetruecolor')) {
            $im = imagecreatetruecolor($width, $height);
        } else {
            $im = imagecreate($width, $height);
	...
	$pixelColor = imagecolorallocate($im, mt_rand(50, 155), mt_rand(50, 155), mt_rand(50, 155));
        // 干扰
        for ($i = 0; $i < 5; $i++) {
        ...
        for ($i = 0; $i < $length; $i++) {
            imagestring($im, 5, $i * 20, mt_rand(1, 5), $randval{$i}, $stringColor);
        }
        Image::output($im, $type);
    }

	 
	 static function buildImageVerify1($length=4,$mode=1,$type='png',$width=48,$height=22,$verifyName='verify')
    {
		import('ORG.Util.String');
...
//        @imagestring($im, 5, 5, 3, $randval, $stringColor);
	...
	static function generate_captcha($width=145,$height=20) {
        $arr_word_upper = array(
			'a','b','c','d','e','f','g','h',
			'i','j','k','m','n','o','p','r','v',
			'w','x','A','B','C','D','E',
			'F','G','H','I','J','K','L',
        ...
        $arr_all = array_merge($arr_word_upper, $arr_number);
        $rand_keys = array_rand($arr_all, 4);
        shuffle($rand_keys);
        $rand_str = '';
        foreach($rand_keys as $value) {
            $rand_str .= $arr_all[$value];
        }
        $_SESSION['verify'] =md5(strtolower($rand_str));
		$width = ($length * 4 + 2) > $width ? $length * 4 + 2 : $width;
        $img = imagecreatetruecolor($width, $height);
        $background_color = imagecolorallocate($img, mt_rand(240, 255), mt_rand(240, 255), mt_rand(240, 255));
        imagefill($img, 0, 0, $background_color);
        $string_color = imagecolorallocate($img,  mt_rand(0, 200), mt_rand(0, 120), mt_rand(0, 120));
        imagestring($img, 5, $width*0.3, 3, $rand_str, $string_color);
        //打印100个干扰像素
        for($i=0; $i<100; $i++) {
			$pixel_color = imagecolorallocate($img,mt_rand(50, 155), mt_rand(50, 155), mt_rand(50, 155));
