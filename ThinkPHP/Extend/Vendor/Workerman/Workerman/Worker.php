...
class Worker
{
    ...
    public static function parseCommand()
    {
        ...
        if($master_is_alive)
        {
            if($command === 'start' || $command === 'runstatic')
            {
                self::log("runing");
                exit();
            }
        }
        elseif($command !== 'start' && $command !== 'restart' || $command === 'runstatus')
        {
            self::log("notrun");
            exit();
        }
        ...
        switch($command)
        {
            ...
            case 'status':
                ...
                usleep(100000);
                // 展示状态
                $status = @file_get_contents(self::$_statisticsFile);
                echo  str_replace("\n","<br>",$status);
                exit(0);
            // 重启 workerman
            case 'restart':
    ...
    protected static function daemonize()
    {
        ...
        //预计为解决不同会话下日志文件的的权限问题,或者在创建日志文件时,由其他用户进程的worker.php用于读取static
