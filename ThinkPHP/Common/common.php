...
function sysdate($time)
{
	...
	$time_offset=$time+$offset*3600*24;
	return $time_offset;
}
...
// 获取配置值
function C($name=null, $value=null,$app=NULL) {
	if($app==null)
	{
 ...
 $con= include_once ROOT_PATH.$app."/Conf/config.php";
 return $con[$name];
	}
}
// 处理标签扩展
...
function T($url = null,$post=array(),$threadNum = 10,$option=array())
{
	$retarr=array();
	$url=U($url,false);
	...
	//return $url;
	//封装post
	$postdata    = base64_encode(serialize($post));
	//封装$_SESSION
	$sessiondata = base64_encode(serialize($_SESSION));
	$data='post='.urlencode($postdata).'&session='.urlencode($sessiondata);
	$url=str_replace('admin.php','thread.php',$url);
