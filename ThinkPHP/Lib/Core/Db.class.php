...
class Db {
    ...
    protected function debug() {
        ...
        //后增加的部分,用于启动对数据库处理内容的调用来源的记录
		if(C('DB_LOG'))
		{
	        $trace=debug_backtrace();
	        if(strpos($this->queryStr,'SHOW COLUMNS FROM')===false)
	        {
	        		if(strpos($this->queryStr,'INSERT ')!==false)
		        	{
		        		//dump($this->queryStr);
		        	}
		        foreach($trace as $key=>$trac)
		        {
		        	if(strpos($this->queryStr,'INSERT ')!==false)
		        	{
		        	//	dump(iconv('GB2312', 'UTF-8', $trace[$key]['file']).'('.$trace[$key]['line'].')');
		        		//dump($trac['class'].'|'.$trac['function']);
		        	}
		        	if(((!isset($trac['class']) || $trac['class']=='') && $trac['function']=='M') ||
		        	   (isset($trac['class']) && $trac['class']=='Model' && (in_array($trac['function'],array('insert','add','save','startTrans','commit','rollback','find','getField','execute','query','select'))))
		        	)
		        	{
		        		//if($trace[$key]['file']!='')
		        		//{
			        		//$this->debugfile(iconv('GB2312', 'UTF-8', $trace[$key]['file']).'('.$trace[$key]['line'].')');
		        		//}
		        	}
		        }
	        }
        ...
        /*if(defined('runcal'))
        {
        	callog('SQL:'.$this->queryStr);
        }*/
        $this->model  =   '_think_';
        // 记录操作结束时间
        if ( $this->debug ) {
            G('queryEndTime');
            //记录SQL日志.估计因为CLI一直运行的缘故目前处于屏蔽状态
            ...
            if(is_string($val[0])) {
            	...
            	if(preg_match('/^(eq|neq|gt|egt|lt|elt)$/',$exp)) { // 比较运算
                    ...
                    if(is_array($val[1])) {
                        ...
                        if(in_array($likeLogic,array('AND','OR','XOR'))){
                            ...
                            foreach ($val[1] as $item){
                                $like[] = $key.' '.$this->comparison[$exp].' '.$this->parseValue($item);
                            }
                    ...
                    if(isset($val[2]) && 'exp'==$val[2]) {
                        ...
                        $whereStr .= $key.' '.$this->comparison[$exp].' ('.$zone.')';
                    }
    ...
    public function buildSelectSql($options=array()) {
        ...
        //崔工增加的自定义代码，在事物期间查询默认设置写锁
        /* if(!isset($options['lock']) && $this->transTimes>0)
        {
        	$options['lock']=true;
        } */
        //$sql   .= $this->parseLock(isset($options['lock'])?$options['lock']:false);
