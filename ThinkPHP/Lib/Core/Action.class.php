...
// +----------------------------------------------------------------------
// $Id: Action.class.php 2945 2012-05-12 10:28:47Z liu21st@gmail.com $
 ...
 * @subpackage  Core
 * @author   liu21st <liu21st@gmail.com>
 * @version  $Id: Action.class.php 2945 2012-05-12 10:28:47Z liu21st@gmail.com $
 +------------------------------------------------------------------------------
 */
abstract class Action {
   ...
   /**
     ...
     * @return void
     +----------------------------------------------------------
     */
    protected function message($message,$jumpUrl='',$ajax=false) {
        $this->dispatchJump($message,-1,$jumpUrl,$ajax);
    }
    /**
     +----------------------------------------------------------
    ...
    protected function ajaxReturn($data,$info='',$status=1,$type='') {
		...
		$result['navTabTitle']  =  isset($_REQUEST['navTabTitle'])?$_REQUEST['navTabTitle']:'';
        $result['rel']  =  isset($_REQUEST['rel'])?$_REQUEST['rel']:'';
        $result['callbackType']  =  isset($_REQUEST['callbackType'])?$_REQUEST['callbackType']:'';
        $result['forwardUrl']  =  $this->view->get('jumpUrl')?$this->view->get('jumpUrl'):(isset($_REQUEST['forwardUrl'])?$_REQUEST['forwardUrl']:'');
        $result['confirmMsg']  =  isset($_REQUEST['confirmMsg'])?$_REQUEST['confirmMsg']:'';
        $result['message'] =  $info;
        $result['info'] =  $info;
        $result['data'] = $data;
    ...
    protected function redirect($url,$params=array(),$delay=0,$msg='') 
	{
   	...
   	protected function redirect_url($message, $url) {
echo '<html>';
echo '<head><meta http-equiv="Content-Type" content="text/html; charset=utf-8" /></head>';
 if($message)
			{
			  $alert_str = "alert('$message');";
echo <<<STR
        ...
        if($status==1) { //发送成功信息
            ...
            if(!$this->view->get('jumpUrl')) {
            	if(!empty($_SERVER["HTTP_REFERER"])){
            		...
            		$this->assign('jumpUrl',"javascript:location.href=document.referrer;");
            	}
            }
