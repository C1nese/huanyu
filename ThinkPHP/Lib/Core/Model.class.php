...
class Model {
    ...
    public function save($data='',$options=array()) {
        ...
        if(!isset($options['where']) ) {
            ...
            if(isset($data[$this->getPk()])) {
                ...
                // 如果没有任何更新条件则不执行
                $this->error = L('_OPERATION_WRONG_');
                throw_exception('在执行批量更新操作时没有增加条件选项');
                return false;
            }
        }
    ...
    public function autoCheckToken($data) {
        if(C('TOKEN_ON')){
            ...
            if(isset($_SESSION[$name][$key]) && $_SESSION[$name][$key] == $value) { // 防止重复提交
    ...
    */
    ...
    public function regex($value,$rule) {
        ...
        if(isset($validate[strtolower($rule)])){
        	if(strtolower($rule)=='require'){
            	$value=$this->delnull($value);
            }
	...
	*/
	...
	//批量更新的数据
	private $bSaveData = array();
	//批量更新用的数据长度计数
	private $bSaveSize = array();
	//当前内存限制量
	private $bMemory   =0;
	//当前内存限制量
	...
	public function bAdd($data)
	{
		if(!$this->bInsLSql)
		{
			...
			array_walk($fields, array(&$this->db, 'parseKey'));
			$this->bInsLSql = 'INSERT INTO '.$this->trueTableName . ' (' . implode(',' , $fields) . ') VALUE ';
		}
		...
		foreach($data as $val)
		{
			$value[]   =  $this->db->parseValue($val);
		}
		if($this->bInsRSql)
		{
			$this->bInsRSql.=',';
		}
		$this->bInsRSql .= '('.implode(',', $value).')';
		//当数据长度超过数据库阀值,或者当前PHP内存占用量达到上限
		if(strlen($this->bInsRSql)>18000 )
		{
			$this->execute($this->bInsLSql.$this->bInsRSql);
			$this->bInsRSql='';
		}
	}
	public function bSave($data)
	{
		...
		{
			throw_exception("在做批量更新时更新资料中未发现主键");
		}
		//得到主键ID
		$id = $data[$this->pk];
		if(!is_numeric($id))
		{
			throw_exception("主键ID不是数字");
		}
		//清空主键项
		unset($data[$this->pk]);
		...
		foreach($data as $key=>$val)
		{
			//memory_get_usage
			//如果没有关于这个字段的更新资料
			if(!isset($this->bSaveData[$key]))
			{
				//初始化数组
				$this->bSaveData[$key]=array();
				//初始化容量计数
				$this->bSaveSize[$key]=0;
			}
			
			if(is_null($val))
			{
				throw_exception('批量更新操作时的['.$key.']字段值为null');
			}
			$this->bSaveData[$key][$id] = $val;
			
			$this->bSaveSize[$key]+=strlen($val)+strlen($id)*2+14;
			if($this->bSaveSize[$key]>18000)
			{
				$this->bSaveUpdate($key,isset($options['where']) ? $options['where'] : '');
			}

		}
	}
	private function bSaveUpdate($key,$where)
	{
		//对等于 累计加，累计减做处理
		$row = $key;
		$row =$this->db->parseKey($row);
		$row .='=';
		//如果是加号开始，表示在原字段上累加
		if(strpos($key,'+')!==false)
		{
			$t_row=substr($key,0,strlen($key)-1);
			$row=$this->db->parseKey($t_row);
			$row=$row.'='.$row.'+';
		}
		//如果是减号开始，表示在原字段上减少
		if(strpos($key,'-')!==false)
		...
		foreach($this->bSaveData[$key] as $datakey=>$dataval)
		{
			$datasql.=" WHEN " .$datakey." THEN '".$dataval."'";
			$dataids.=$datakey.',';
		}
		unset($this->bSaveData[$key]);
		unset($this->bSaveSize[$key]);
		if(is_array($where))
		{
			throw_exception("在做批量更新时条件不支持数组型");
		}
		if($where != '')
		{
			$where=' and ('.$where.')';
		}
		$this->execute('UPDATE '.$this->trueTableName . ' set '.$row.' CASE '.$this->pk.' '.$datasql.' END where '.$this->pk.' in ('.trim($dataids,',').')'.$where);
	}
	public function bUpdate()
	{
		...
		//处理批量更新缓存
		$options =  $this->_parseOptions(array());
		foreach($this->bSaveData as $key=>$val)
		{
			$this->bSaveUpdate($key,isset($options['where']) ? $options['where'] : '');
		}
	}
	public function bSelect($callback=null,$parameters=null,$limit=1000)
	{
		$i = 0;
		$iffile = false;
		$sql  =   $this->select(false);
		$membegin = memory_get_usage(true);//内存初始量
		$filename = md5($sql.$this->db->_linkID).'.txt';
		...
		do{
			...
			if($i>=$limit || !($tle)){
				if(!$iffile){
					...
					$memdiff = ($memend-$membegin)/1024/1024;
					if($memdiff>=200)$iffile = true;//判定使用内存过大改为写文件
				}
				if($iffile){
					if(!empty($tletmp)){
						fwrite($fp,serialize($tletmp).PHP_EOL);
					}
					$i = 0;
					$tletmp = array();
				}
		...
		if($callback != null){
			if(!empty($tletmp)){
				call_user_func_array($callback,array($tletmp,$parameters)); 
				unset($tletmp);
			}else{
				while (!feof($fp))
	    		{
					...
					if($content!=''){
						$tlearr = unserialize($content);
						call_user_func_array($callback,array($tlearr,$parameters)); 
						unset($tlearr);
						unset($content);
					}
