...
class DbMysql extends Db{
    ...
    public function connect($config='',$linkNum=0,$force=false) {
        if ( !isset($this->linkID[$linkNum]) ) {
            ...
            if($dbVersion >'5.0.1'){
                ...
                mysql_query("SET sql_mode='STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION'",$this->linkID[$linkNum]);
                
            }mysql_query("set @@character_set_server='utf8';",$this->linkID[$linkNum]);
    ...
    public function execute($str) {
        ...
        if ( !$this->_linkID ) return false;
        if((stripos(trim($str),'update')===0 or stripos(trim($str),'delete') === 0) && $this->transTimes==0){
	...
	public function debugfile($file)
	{
        //$this->initConnect(true);
       // if ( !$this->_linkID ) return false;
	//	$result =   mysql_query("set @file='".$file."'", $this->_linkID) ;
	}
    /**
     +----------------------------------------------------------
     * 启动事务
    ...
    public function getFields($tableName) {
        ...
        self::$FieldsCache[$tableName]=$info;
        return $info;
    }
    ...
    public function error() {
        ...
        if($this->debug && '' != $this->queryStr){
            //$this->error .= "\n [ SQL语句 ] : ".$this->queryStr;
            throw_exception($this->error);
            //throw_exception('数据库底层失败'.htmlentities($this->error,ENT_COMPAT ,'UTF-8'));
        }
        return $this->error;
    }
