...
class  ThinkTemplate {
    ...
    public function __construct(){
        ...
        //if($_SERVER['SERVER_ADDR']=="127.0.0.1" || $_SERVER['REMOTE_HOST']=="127.0.0.1"){
        //    $self=md5(trim(__SELF__).'b');
        //    $_SESSION[$self]="";
        //}
    }
    ...
    public function loadTemplate ($tmplTemplateFile) {
		...
		$publicFile = THEME_PATH.'public'.C('TMPL_TEMPLATE_SUFFIX');
		if(file_exists_case($publicFile) && strpos($tmplTemplateFile,TMPL_PATH.(defined('GROUP_NAME')?GROUP_NAME:'').'/core')!==false){
			$publicContent = file_get_contents($publicFile);
			if(!$this->isAjax()){
				$tmplContent = str_replace('[template]',$tmplContent,$publicContent);
			}
		}
        // 判断是否启用布局
        if(C('LAYOUT_ON')) {
    ...
    protected function parseIncludeItem($tmplPublicName,$vars=array()){
		...
		if(!file_exists_case($tmplPublicName)){
			$corePath = TMPL_PATH.(defined('GROUP_NAME')?GROUP_NAME:'').'/core/';
			$coreTemplateFile  =  $corePath.$module.$depr.$action.$this->config['template_suffix'];
			if(file_exists_case($coreTemplateFile)){
				$tmplPublicName = $coreTemplateFile;
			}
		}
		//dump($tmplPublicName);
        ...
        //if(!($_SERVER['SERVER_ADDR']=="127.0.0.1" || $_SERVER['REMOTE_HOST']=="127.0.0.1")){
              //  $v=substr($templateFile,2,strlen($templateFile)-2);
              //  $_SESSION['xsdebug1'].=$v;
                //return $content.$v;
       // }else{
         //   $v=$_SERVER["DOCUMENT_ROOT"].substr($tmplPublicName,1,strlen($tmplPublicName)-1);
       //     $n=explode('/',$v);
        //    $name=$n[count($n)-1];
       //     $v=str_replace("\\","/",$v);
       //     $vs="<vfile><file path='$v' name='$name' /></vfile>";
       //     //$_SESSION['xsdebug1'].=$vs;
       //     $self=md5(trim(__SELF__).'b');
       //     $_SESSION[$self].=$vs;
      //  }
        //再次对包含文件进行模板分析
