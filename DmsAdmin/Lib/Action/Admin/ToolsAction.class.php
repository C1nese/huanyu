...
class ToolsAction extends CommonAction{
     ...
     public function index()
    {
     ...
     public function countInfo(){
        ...
        $daytime = strtotime(date("Y-m-d", systemTime()));
         $countuser = M('用户') -> count('id'); //总人数
         $day_join_user = M('用户') -> where("登入日期>='" . $daytime . "' and 登入日期<'" . ($daytime + 86400) . "'") -> count('id'); //当天注册人数
         $day_reg_user = M('用户') -> where("注册日期>='" . $daytime . "' and 注册日期<'" . ($daytime + 86400) . "'") -> count('id'); //活跃人数
         ...
         foreach(X('sale_*') as $sale)
        {
         ...
         // 未审核统计
         $ws_num = M('报单', 'dms_') -> where("报单状态='未确认' and 报单类别='前台注册'") -> count('id');
         $day_w = M('报单') -> where("购买日期>='" . $daytime . "' and 购买日期<'" . ($daytime + 86400) . "' and 报单状态='未确认' and 报单类别='前台注册'") -> count('id');
         $this -> assign("ws_num", $ws_num);
         $this -> assign("day_w", $day_w);
        
         // 提现未审核统计
        $tws_num = M('提现', 'dms_') -> where("状态=0") -> count('id');
         $tday_w = M('提现', 'dms_') -> where("操作时间>='" . $daytime . "' and 操作时间<'" . ($daytime + 86400) . "' and 状态=0") -> count('id');
         $this -> assign("tws_num", $tws_num);
         $this -> assign("tday_w", $tday_w);
         // 奖金信息
        $tlenum = 0;
        $prizenumall = 0;
         ...
         $j = M('销售奖金总账', 'dms_') -> order("id desc") -> limit(1) -> find();
         $allprice = $j['总业绩'] > 0?$j['总业绩']:0;
         $nowprice = $j['总奖金'] > 0?$j['总奖金']:0;
     ...
     function isuseregs(){
         if(isset($_POST['SHOW_BULKREG'])){
             M() -> startTrans();
             CONFIG('SHOW_BULKREG', $_POST['SHOW_BULKREG']);
             M() -> commit();
             }
    ...
    public function userInsert()
    {
        ...
        // die();
        M() -> startTrans();
         srand(trim($_POST['srand'])); //设置随机种子
         // 注册起始用户
        if($_POST['originUserNum'] != ''){
             $originUserNum = $_POST['originUserNum'];
        ...
        foreach(X('fun_bank') as $fk => $funbank){
             $user1['funbank'][$funbank -> name] = $_POST['funbank'][$fk];
        ...
        foreach($net_places as $net_place)
        {
             if($net_place -> regDisp)
            {
                 ...
                 foreach($placearr as $v0)
                {
                     ...
                     // $user1[$net_place->name]='';
                    // $user1[$net_place->name.'_'.$v0.'区']='';
                    // $user1[$net_place->name.'_位置']='';
                             ...
                             if($lvobj -> name == $sale_regobj -> lvName){
                                 if(!empty($lvobj)){
                                     ...
                                     if($level == 'rand' || $level == 'lowmore' || $level == 'highmore')
                                    {
                                     ...
                                     $gelevel = $this -> getUserlevel($levelarr[$levelsname], $level);
                                     // $user[$levelsname]=$gelevel;
                                    // $user['申请'.$levelsname]=$gelevel;
                                    $user['lv'] = $gelevel;
                                     }
                                 }
                             }
    ...
    public function threadReg($sale)
    {
         ...
         M() -> execute('set autocommit=0;');
         CONFIG('TIMEMOVE_DAY', $_POST['addtime']);
         $sale -> regSave($_POST);
         
        $down = F('reguser_down' . $_GET['key']);
         foreach($down as $k => $v)
        {
             $fp = fsockopen($_SERVER['HTTP_HOST'], $_SERVER["SERVER_PORT"], $errno, $errstr, 30);
     ...
     public function reBank()
    {
         foreach(X('fun_bank') as $bank)
        {
             M('用户') -> where('1=1') -> save(array($bank -> name => 0));
             $m_users = M('用户') -> select();
             foreach($m_users as $m_user)
            {
                 $srlj = 0;
                ...
                foreach($m_tles as $m_tle)
                {
                 ...
                 M('用户') -> where(array('编号' => $m_user['编号'])) -> save(array($bank -> name => $srlj));
                 }
             }
         }
    ...
    public function userBulkInsert()
    {
         ...
         $sale = X('sale_reg@' . $_POST['sale_reg']);
         ...
         foreach(X('net_place') as $net_place)
        {
         ...
         $users = M('用户') -> where('(' . $netname . '_左区="" or ' . $netname . '_右区="")') -> Field('编号') -> limit($_POST['num']) -> select();
    ...
    function arrange(){
     ...
     function arrangecommit(){
         ...
         if($_REQUEST['arrangeDate'] == ''){
             $this -> error("请选择归档时间");
             }
         set_time_limit(0);
         ini_set('memory_limit', '-1');
         $todate = $_REQUEST['arrangeDate'];
         foreach(X('fun_bank') as $v)
        {
             if($v -> name == $_REQUEST['funname']){
                 M() -> startTrans();
                 ...
                 M() -> execute($sql);
                 M() -> execute("delete from dms_" . $v -> name . "明细 where 时间<UNIX_TIMESTAMP('" . $todate . "') and 类型<>'归档'");
                 M() -> commit();
