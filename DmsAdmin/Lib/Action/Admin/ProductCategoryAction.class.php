...
class ProductCategoryAction extends CommonAction 
{
    ...
    */
	...
	public function addSave($product){
		...
		if($_POST['name'] == ''){
			...
			$have=$Category->where(array("名称"=>trim($_POST['name'])))->find();
			if($have){
				$this->error('此分类名称已存在!');
			}
		}
		$data['名称'] = trim($_POST['name']);
		$data['排序'] = $_POST['sort'];
		$data['状态'] = $_POST['status'];
		$data['创建时间'] = time();
	...
	public function editSave($product){
		...
		//验证是否存在
		$have=$Category->where(array("名称"=>trim($_POST['name']),"id"=>array('neq',$_POST['id'])))->find();
		if($have){
			$this->error('此分类名称已存在!');
		}
		$data['id'] = $_POST['id'];
        ...
        if($Category->save($data)){
        	//同步修改产品分类
        	M($product->name)->where(array("分类"=>$old['名称']))->save(array("分类"=>trim($_POST['name'])));
        	M()->commit();
            $this->success("修改成功!");
        }else{
        	M()->rollback();
    ...
    */
    ...
    public function productfunction_addSave(){
      	if(!isset($_POST['gongneng_name']) || $_POST['gongneng_name']==''){
      		$this->error("请填写功能名称");
      	}
		if(!isset($_POST['jiedian']) || count($_POST['jiedian'])<=0){
			$this->error('请选择所属功能');
		}
		//保存数据
		$data = array(
			'名称'=>$_POST['gongneng_name'],
			'创建时间'=>systemTime(),
			'节点名称'=>implode(',',$_POST['jiedian']),
			'状态'=>$_POST['status'],
		);
   ...
   //修改产品功能
	...
	public function productfunction_delete(){
		...
		foreach(X("product") as $product){
			$have=M($product->name)->where("find_in_set('".$_GET['id']."',所属功能)")->find();
			if($have)$this->error("此功能下已有".$product->byname."存在");
		}
		M()->startTrans();
		if(M('产品_功能')->where(array('id'=>intval($_GET['id'])))->delete()){
			M()->commit();
			$this->success('操作成功');
		}else{
			M()->rollback();
