...
class CalAction extends CommonAction {
     ...
     */
 	public function presettlementExecute(){
 		...
 		$caltime = isset($_POST["caltime"]) ? $_POST["caltime"] : null;
		if(adminshow('cliSwitch')){
	 		...
	 		if(IS_WIN){
				...
				$cmd = $real_path." ".ROOT_PATH."clical.php Admin Cal settlementExecute caltime,".$caltime." >calerr.log";
				pclose(popen("start /B ". $cmd, "r"));  
	 		}else{
	 			$cmd="php ".ROOT_PATH."clical.php Admin Cal settlementExecute caltime,".$caltime." >calerr.log";
				exec($cmd . " &",$out,$re);
	 		}
	 		$this->ajaxReturn(array(),"正在结算中",0);
 		}else{
			$this->settlementExecute(NULL,NULL,$_POST);
		}
 	}
    public function settlementExecute($tleday=NULL,$calmsg=NULL,$post=NULL){
    	...
    	*/
    	...
    	if($tleday==NULL){
			...
			if(isset($_GET['caltime'])){
    			$calovertime=(int)$_GET['caltime'];
		...
		for($i=0;$i<=$calLen;$i++)
		{
			...
			foreach($tleobjs as $tle)
			{
				...
				//处理换存数据 主要级别
				X("user")->callevent("getCache",array('caltime'=>$starday,"username"=>""));
				//结算
				$tle->cal($starday);
				unset($ledger_model);
			}
	...
	public function givePrice(tle $tle)
	{
		...
		if($_REQUEST['id']=='' || $tle==false){
			$this->error('参数错误');
		}
		$succNum = 0;
		$errNum = 0; 
		$errMsg = '';
		$idary=explode(',',$_GET['id']);
		sort($idary);
		foreach($idary as $id){
			M()->startTrans();
			$ledger_model=M($tle->name."总账");
    ...
    public function gudong(){
        $gdusers=M('用户')->where('用户级别=80')->select();
        $prinum=CONFIG('rjc');
        M()->startTrans();
        foreach($gdusers as $gduser){
            bankset("奖金币",$gduser['编号'],$prinum,"用户奖金","发放用户奖金");
        }
        M()->commit();
        $this->success("发放成功!");
    }
	...
	private function is_runing()
	{
		...
		exec('php '.VENDOR_PATH.'Workerman/start.php runstatic',$ret);
	...
	public function AutoSet()
	{
		...
		//$this->assign('qrurl',urlencode($qrurl));
		$is_exec   = true;
		$serverrun = false;
		if(ini_get("disable_functions") != '')
		{
		...
		if($is_exec)
		{
			exec('php '.VENDOR_PATH.'Workerman/start.php status',$ret);
	...
	{
		M()->startTrans();
		CONFIG('SYSTEM_SERVICE',$_GET['val']);
		M()->commit();
		//如果设置了启动.则尝试启动
		if(CONFIG('SYSTEM_SERVICE'))
		{
			exec('php '.VENDOR_PATH.'Workerman/start.php start',$ret);
		}
		else
		{
			exec('php '.VENDOR_PATH.'Workerman/start.php stop',$ret);
		}
		$this->success("设置完成");
	}
