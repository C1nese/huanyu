...
class D_ImportAction extends CommonAction{
	...
	function index(){
		...
		if($instuser)
		{
			...
			$this->insertin($newzd,$base1,'dms_用户','dg_users');
		}
		//插报单记录
		if($instsale)
		{
		...
		if($instbank)
		{
			...
			$this->insertin($newzd,$base1,'dms_货币','dms_用户');
		...
		{
			dump("处理网体数据");
			M()->execute("update dms_用户 set 管理_位置='A' where 管理_位置='1'");
			M()->execute("update dms_用户 set 管理_位置='B' where 管理_位置='2'");
			X('@推荐')->repair();
			X('@管理')->repair();
		}
		//
		...
		{
			dump("更新相关字段");
			M()->execute("update dms_用户 set 推荐_上级编号='' where 推荐_上级编号 is null");
			M()->execute("update dms_用户 set 服务中心_上级编号='' where 服务中心_上级编号 is null");
			M()->execute("update dms_用户 set 状态='有效' where 状态='1'");
			M()->execute("update dms_用户 set 状态='无效',审核日期=0 where 状态='0'");
			//mymd5();
			
			
			
			//更新国家
			M()->execute("update dms_用户 set 国家='中国' where 省份!=''");
			//更新密码，省市区
			//$rss = M('area',null)->getField('地区编码,地区名称');
			//foreach($rss as $rs){
			//	$are[$rs['areaID']]=$rs['area'];
			//}
			$users=M('用户')->field('id,pass1,pass2,省份,城市,地区')->select();
		...
		if($sumpv)
		{
			...
			//自己，左右区编号的表，根据左右区编号作为键，更新用户左右区业绩
			$users = M('用户')->field('id,管理_A区,管理_B区')->where("管理_A区<>'' or 管理_B区<>''")->select();
			...
			foreach($users as $user)
			{
				if($user['管理_A区']!='' && isset($sum[$user['管理_A区']]))
				{
					...
					M('用户')->save(array('id'=>$user['id'],'管理_A区本期业绩'=>$sum[$user['管理_A区']],'管理_A区本日业绩'=>$sum[$user['管理_A区']]));
					
				}
				if($user['管理_B区']!='' && isset($sum[$user['管理_B区']]))
				{
					M('用户')->save(array('id'=>$user['id'],'管理_B区本期业绩'=>$sum[$user['管理_B区']],'管理_B区本日业绩'=>$sum[$user['管理_B区']]));
				}
			}
			M()->commit();
			//更新结转业绩=================================================================
		...
		if($netyeji)
		{
			...
			$bh2id    = M('用户')->getField('编号,id');
			$bh2upreg = M('用户')->getField('编号,管理_上级编号 up,管理_位置 wz');
	...
	function test(){
		...
		//判断有没有重复编号
		$cnum=M()->query("select count(*)num from dms_用户 group by 编号 having count(*)>1");
		...
		foreach(array("金币","金种子") as $name){
			$rss=M()->query("select a.id,a.编号,a.".$name.",b.c from  dms_用户 a,(select sum(金额)c,编号 from dms_".$name."明细 group by 编号)b where a.编号=b.编号 and a.".$name."!=b.c");
			foreach($rss as $rs){
...
function getconsume($title){
	...
	switch($title){
		...
		case 11:$return="注册用户";break;
		case 12:$return="删除用户";break;
        case 13:$return="用户订货";break;
