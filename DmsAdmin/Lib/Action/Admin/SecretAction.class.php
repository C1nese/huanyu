...
class SecretAction extends CommonAction {
    ...
    function savesecret(){
        ...
        if(empty($_POST['密保问题'])){
        	...
        	$rs=$secret->where(array("密保问题"=>trim($_POST['密保问题'])))->find();
        	if($rs){
        		$this->error("密保问题已存在");
        	}
        }
        $data['密保问题'] = $_POST['密保问题'];
	...
	function editsecret(){
		$secretinfo = M('密保')->find($_REQUEST['id']);
	...
	function saveEditsecret(){
		...
		if(empty($_POST['密保问题'])){
        	...
        	$rs=$secret->where(array("密保问题"=>trim($_POST['密保问题']),"id"=>array("neq",$_POST['id'])))->find();
        	if($rs){
        		$this->error("密保问题已存在");
        	}
        }
