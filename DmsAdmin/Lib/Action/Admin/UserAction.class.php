...
class UserAction extends CommonAction 
{
	...
	{
        //set_time_limit(30);                                                      //不超时
        //ini_set('memory_limit','1M');
		//加载tablelist 实例化用户表
        $list=new TableListAction('用户');
        //$list->table('dms_用户 user');
        $list->table('dms_用户 user inner join dms_货币 b on user.id=b.userid left join dms_lvlog lv1 on user.id=lv1.userid  and lv1.lvname="用户级别" and lv1.nlv=user.用户级别 left join dms_lvlog lv2 on user.id=lv2.userid and lv2.lvname="商务中心级别" and lv2.nlv=user.商务中心级别');//货币分离
        ...
        */
        ...
        /*显示字段的内容*/
        $list->addshow("ID",array("row"=>'[user.id]',"hide"=>true));
		$list->addshow("原编号", array("row" => "[testno]", "searchMode" => "text", 'searchRow' => 'user.testno',"searchGet" => "testno","searchPosition" => "top",));
        ...
        $list->addshow("姓名",array("row"=>array(array(&$this,"_printName"),"[姓名]"),"css"=>'width:60px',"searchRow"=>'user.姓名',"searchMode"=>"text","searchPosition"=>"top"));
        ...
        foreach(X('levels') as $key=>$levels)
        {
            ...
            //自己添加原始级别 作显示作用
            $list->addshow('原始'.$levels->byname,array("row"=>array(array(&$this,"ysUserLevel"),"[olv".($key+1)."]",$levels->name),"searchMode"=>"num","searchSelect"=>$_temp));
		...
		$list->field('user.*,user.id uid,b.*,lv1.olv as olv1,lv2.olv as olv2'.$netnamerow);//货币分离
		$list->tl_id="uid";
		//显示到页面中
    ...
    //自己添加
    public function ysUserLevel($level,$levelname){
        $ret='';
		if($levelname!=''){
