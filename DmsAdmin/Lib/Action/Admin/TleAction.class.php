...
class TleAction extends CommonAction {
	...
	public function index(tle $tle)
	{
      ...
      //  {
       ...
       // {
        ...
        //}
        //$where['_logic']="or";
		$list ->join('inner join dms_用户 as b on a.编号=b.编号')->field('a.*,b.姓名');
        ...
        foreach(X('prize_*',$tle) as $prize)
        {
        	...
        	{
        		$list->addshow($prize->byname,array("row"=>"[{$prize->name}]","searchMode"=>"num",'searchRow'=>'a.'.$prize->name)); 
        	}
        }
	...
	public function getTotalExcel($tle){
        ...
        $result=$m->join('dms_用户 as b on a.编号=b.编号')->field("a.编号,b.姓名,b.证件号码,b.移动电话,b.开户银行,b.开户地址,b.省份,b.城市,b.银行卡号,a.奖金,a.收入")->where($where)->order("a.计算日期 desc")->select();
		$temp = array();
		$total = array(
			'编号'=>$v["编号"],
			'姓名'=>'',
    ...
    public function getHxExcel($tle){
		...
		//$whereArr = explode(' ',$where);
		//$where = preg_replace("/(\S+)\s*[=><]/U",'a.$0',$where);
        $result=$m->join('dms_用户 as b on a.编号=b.编号')->field("b.姓名,b.证件号码,b.移动电话,b.开户银行,b.开户地址,b.省份,b.城市,b.银行卡号,a.收入")->where($where)->order("a.计算日期 desc")->select();
		$temp = array();
		foreach($result as $v){
    ...
    */
    ...
    public function prizeForm(tle $tle){
       ...
       $tlddata=$m_tle->find();
       $id     = $tlddata['id'];
       $userid = M('用户')->where(array('编号'=>$tlddata['编号']))->getField('id');
       //dump($userid);
       ...
       foreach(X('prize_*',$tle) as $prize)
       {
        	if($prize->prizeMode>=0 && $prize->isSee)
        	{
	        	...
	        	//dump($fromlist);die();
        		/*$fromlist = $m_from->join("`dms_用户` as a on a.id=fromid")
        		->where("dataid=$id and name='".$prize->name."'")
        		->field("dms_{$tle->name}构成.*,a.编号")
        		->select();*/
        		if(is_array($fromlist)){
					...
					if($prize->unmemo)
					{
		        		/*$fromlist = $m_from->join("`dms_用户` as a on a.id=userid")
		        		->where("fromid<>userid and fromid=".$userid." and name='".$prize->name."' and dataid in (select id from dms_{$tle->name} where 计算日期=".$tlddata['计算日期'].")")
		        		->field("dms_{$tle->name}构成.*,a.编号")
		        		->select();*/
	...
	public function editTle(tle $tle){
		...
		//更新用户表中的累计收入
		M('用户')->where(array('编号'=>$tledata['编号']))->setInc('累计收入',$data['收入']-$tledata['收入']);
		//更新总账的本期奖金
        ...
        if($tlestate == 1 && isset($_POST['sum']))
        {
        	//执行货币同步
        	$tle->givePrice($m->where(array('id'=>$data['id']))->select());
        }
        $this->saveAdminLog($tledata,$resutlt,'修改用户奖金',"修改用户[".$tledata['编号']."]".date("Y-m-d",$tledata['计算日期']).'奖金');
        M()->commit();
		$this->success('修改成功！');
	}
	public function ledger(tle $tle)
	{
		...
		//	}
		//}
        $list->order("计算日期 desc");//->group('计算日期');
		if(!$tle->autoGive && !$tle->weekAutoGive){
			$list->setButton = array(
               ...
               "发放用户奖"    =>array("class"=>"add","href"=>"__APP__/Admin/Cal/gudong","target"=>"ajaxTodo","title"=>'确定发放吗?'),
			);
			//if(!$tle->haveScal()){
		...
		$list->addshow("新增".$this->userobj->byname,array("row"=>"[新增用户]","searchMode"=>"text","excelMode"=>"text"));
		...
		$list ->addshow("全部".$this->userobj->byname,array("row"=>"[全部用户]","searchMode"=>"text","excelMode"=>"text"));
	...
	{
		$data = M($tle->name.'总账')->where(array('id'=>$_GET['id']))->find();
		if(!$data)
		{
			$this->error('未找到回退记录');
		}
