...
class ShopAction extends CommonAction 
{
    ...
    public function index()
	{
		...
		';
		$extrawhere='';
		if($list->extraSearch!='' && isset($_POST['extra'])){
			if($_POST['province']){
				$extrawhere.="user.国家='中国' and user.省份='".$_POST['province']."' and ";
			}
			if($_POST['city']){
				$extrawhere.="user.城市='".$_POST['city']."' and ";
			}
			if($_POST['county']){
				$extrawhere.="user.地区='".$_POST['county']."' and ";
			}
			if($_POST['town']){
				$extrawhere.="user.街道='".$_POST['town']."' and ";
			}
		}
		...
		$list->addshow("移动电话",array("row"=>"[移动电话]","searchMode"=>"text",'searchRow'=>'user.移动电话',"hide"=>true));
   		$list->addshow("上级中心",array("row"=>"[服务中心_上级编号]","searchMode"=>"text",'searchRow'=>'user.服务中心_上级编号'));
   		$list->addshow("上级中心",array("row"=>"[服务中心_上级编号]","searchMode"=>"text",'searchRow'=>'user.服务中心_上级编号'));
