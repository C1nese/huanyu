...
class MenuAction extends Action
{
    ...
    public function getMenu()
    {
        ...
        if (adminshow('edit_wangluo')) {
            ...
            $menu[] = array('model' => 'Net', 'action' => 'modPwd', 'title' => '网体密码修改', 'level' => 1, 'parent' => '网络管理', 'setParent' => '网络修改');
            $menu[] = array('model' => 'Net', 'action' => 'modLock', 'title' => '网体登录锁定', 'level' => 1, 'parent' => '网络管理', 'setParent' => '网络修改');
        }
        // 只有豪华版才能看到网络显示
        if (C('VERSION_SWITCH') == '0') {
        ...
        //推荐商品
        $menu[] = array('model' => 'Recommend', 'action' => "index", 'title' => "商品列表", 'level' => 1, 'parent' => $parent, 'setParent' => $product->byname . '列表管理', 'actions' => 'index,UploadPhoto');
        $menu[] = array('model' => 'Recommend', 'action' => "UploadPhoto", 'title' => "商品图片上传", 'level' => 2, 'parent' => $parent, 'setParent' => $product->byname . '列表管理', 'actions' => "UploadPhoto,UploadPhotoSave");
        $menu[] = array('model' => 'Recommend', 'action' => "add", 'title' => "商品添加", 'level' => 2, 'parent' => $parent, 'setParent' => $product->byname . '列表管理', 'actions' => "add,addSave");
        $menu[] = array('model' => 'Recommend', 'action' => "edit", 'title' => "商品修改", 'level' => 2, 'parent' => $parent, 'setParent' => $product->byname . '列表管理', 'actions' => "edit,editSave");
        $menu[] = array('model' => 'Recommend', 'action' => "delete", 'title' => "商品删除", 'level' => 2, 'parent' => $parent, 'setParent' => $product->byname . '列表管理');
        // ++++++++++产品模块++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        /**
         * 财务管理
         */
        ...
        foreach (X('tle') as $tle) {
            if ($tle->tleMode != 's') {
                $menu[] = array('model' => 'Cal', 'action' => 'settlement', 'title' => '结算操作', 'level' => 1, 'parent' => $parent, 'setParent' => '结算操作管理', 'actions' => 'settlement,settlementExecute,presettlementExecute,ExecuteAjax');
            }
            $xpath = $tle->xpath;
