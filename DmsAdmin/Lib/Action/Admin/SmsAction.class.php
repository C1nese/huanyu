...
class SmsAction extends CommonAction {
    ...
    public function setSms(){
        ...
        M()->startTrans();
        if($_POST['smsPsw'] != '***********'){
			if(!isset($_POST['companyCode'])||$_POST['companyCode']==""){
	        	$this->error("请输入公司代码");
	        }
			$smsPsw = CONFIG('smsPsw',$_POST['companyCode'].$_POST['smsPsw']);
		}
		CONFIG('companyCode',$_POST["companyCode"]);
		if(isset($_POST["adminLogin"])){
            $adminLogin = true;
        }else{
            $adminLogin = false;
        }
		$telNumber = CONFIG('telNumber',$_POST["telNumber"]);
		$adminLogin = CONFIG('adminLogin',$adminLogin);
		$smsUser = CONFIG('smsUser',$_POST['smsUser']);
		CONFIG('regsmsContent',$_POST['regsmsContent']);
		CONFIG('accoksmsContent',$_POST['accoksmsContent']);
		CONFIG('changePwdsmsContent',$_POST['changePwdsmsContent']);
		CONFIG('verificatesmsContent',$_POST['verificatesmsContent']);
	    CONFIG("zhzhsmsContent",$_POST['zhzhsmsContent']);
	    CONFIG("zhzhgetsmsContent",$_POST['zhzhgetsmsContent']);
		CONFIG("txmsmsContent",$_POST['txmsmsContent']);
		
		CONFIG('regsmsMobile',$_POST['regsmsMobile']);
		CONFIG('accoksmsMobile',$_POST['accoksmsMobile']);
		CONFIG('changePwdsmsMobile',$_POST['changePwdsmsMobile']);
	    CONFIG("zhzhsmsMobile",$_POST['zhzhsmsMobile']);
	    CONFIG("zhzhgetsmsMobile",$_POST['zhzhgetsmsMobile']);
		CONFIG("txmsmsMobile",$_POST['txmsmsMobile']);
	...
	*/
	...
	{
		$mobiles = $_POST['telnum'];
		$content = $_POST['msg'];
		import('COM.SMS.DdkSms');
		
		M()->startTrans();
		$surplus = DdkSms::send($mobiles,$content,'用户注册');
		M()->commit();
		if($surplus['status'])
	...
	*/
	public function smsSave(){
		...
		$sendnums=explode(',',$_POST['sendnums']);
		$count=count($sendnums);
		$model=M('短信');
		$data['内容']=$_POST['content'];
		$data['发送时间']=time();
		$data['待发数量']=$count;
		...
		foreach ($sendnums as $k=>$val){
			$user=M('用户')->where('移动电话="'.$val.'"')->find();
			$arr11=array('[编号]','[姓名]');
			if($user['id']){
				$username=$user['姓名'];
				$arr22=array($user['编号'],$username);
				$content=str_replace($arr11,$arr22,$_POST['content']);
			}else{
	...
	function runThread($id)
    {
    	...
    	{
    		$fp = fsockopen('ssl://'.$_SERVER['HTTP_HOST'], $_SERVER["SERVER_PORT"], $errno, $errstr, 30);
    	}else{
		...
		{
			//调用发送短信的方法
			$out  = "GET /index.php?s=/Admin/Sms/sendsmslist/did/".$id." / HTTP/1.1\r\n";
	...
	public function sendsmslist(){
		...
		$duanxin_name = CONFIG('smsUser');//用户账户
		$duanxin_paw  = md5(CONFIG('smsPsw'));//用户密码
		file_put_contents('err.txt',$duanxin_name);
	...
	*/
	public function smsdatail(){
        ...
        $list->setButton = $setButton;      // 定义按钮显示
		$list->excel = false;
		$list->where('d_id="'.$_REQUEST['id'].'"')->order("发送时间 desc");		//定义查询条件
	...
	public function smsview(){
		$model=M("短信详细");
		$smsdetal=$model->where('id="'.$_REQUEST['id'].'"')->find();
	...
	*/
	public function check(){
		...
		$userinfo= M('用户') ->where('移动电话="'.$_REQUEST['num'].'"')->field('编号 id,姓名 name')->find();
		if($userinfo){
	...
	*/
	public function delgroup(){
		...
		$model1	= M('号码');
		M()->startTrans();
		$member = $model1->where('编组="'.$_REQUEST['id'].'"')->count('id');
		if($member!=0){
			$result1= $model1->where('编组="'.$_REQUEST['id'].'"')->delete();
		}
		$model	= M('号码编组');
		$result = $model->where('id="'.$_REQUEST['id'].'"')->delete();
		if($result1 && $result){
			M()->commit();
			$this->success('数据已清除');
		}else{
	...
	*/
	public function getnum(){
		$model	= M('号码');
		$numlist= $model->where('编组="'.$_REQUEST['id'].'"')->field('号码 num,姓名 name,编号 id')->select();
	...
	*/
	public function downout(){
		$model	= M('号码');
		$numlist= $model->where('编组="'.$_REQUEST['id'].'"')->select();
		$title = date("YmdHis");
	...
	*/
	public function putinto(){
		$model	= M('号码');
		$map['号码']  = $_REQUEST['num'];
		$map['编组']  = $_REQUEST['group'];
		M()->startTrans();
		$id = $model ->where ($map)-> getfield('id');//查看此条号码的信息是否已加入
		
		$data['编组'] = $_REQUEST['group'];
		$data['号码'] = $_REQUEST['num'];
		$data['姓名'] = $_REQUEST['name'];
		$data['编号'] = $_REQUEST['id'];
		if($id){
			$result = $model -> where('id="'.$id.'"') -> save($data);//如果号码已存在，则更新信息
		}else{
	...
	*/
	public function add_all(){
		$map = array();
		if($_REQUEST['leve']){
			$map['用户级别']=$_REQUEST['leve'];
		}
		$users= M('用户') -> where($map) -> field('编号 id,姓名 name,移动电话 num') -> select();
	...
	public function add_num(){
		...
		$users= M('用户')->where('编号="'.$_REQUEST['userid'].'"')->field('编号 id,姓名 name,移动电话 num')->select();
		echo json_encode($users);
	...
	public function add_team(){
		...
		$where['编号']=$userid;
		$user=M('用户')->where($where)->find();
		if($user){
			$users=$obj->getdown($user,0,0,'',true);
			foreach($users as $user)
			{
				$map['编号']=$user['编号'];
				$userarr[]=M('用户')->where($map)-> field('编号 id,姓名 name,移动电话 num')->find();
			}
			echo json_encode($userarr);
		}else{
			echo json_encode("用户不存在".$model->where($where)->select(false));
		}
		*/
	}
}
