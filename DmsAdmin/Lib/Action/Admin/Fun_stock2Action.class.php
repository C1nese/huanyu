...
class Fun_stock2Action extends CommonAction
{
	...
	public function configSave(fun_stock $fun_stock)
	{
		...
		$fun_stock->setatt('stockCloseMsg',$_POST['stockCloseMsg']);
		//开盘价格
		$fun_stock->setatt('StockOpening',$_POST['StockOpening']);
		//拆分价格
		$fun_stock->setatt('splitPrice',$_POST['splitPrice']);
		//小数位
		$fun_stock->setatt('decimalLen',$_POST['decimalLen']);
		//?
		$fun_stock->setatt('priceLen',$_POST['priceLen']);

//		$fun_stock->setatt('upSkip',$_POST['upSkip']);
		$fun_stock->setatt('stockTrade',$_POST['stockTrade']);
		$fun_stock->setatt('shouxufei',$_POST['shouxufei']);
			
		M($fun_stock->name."设置")->where(array("id"=>1))->save(array(
			'stockAllNum'=>$_POST['allnums'],
			'涨价额'=>$_POST['upNum'],
			'涨价幅度'=>$_POST['upSkip'],
			));
		M()->commit();
	...
	public function saleList($fun_stock)
	{
        ...
        $list->addshow("用户编号",array("row"=>"[编号]","searchMode"=>"text","excelMode"=>"text","order"=>"编号","searchPosition"=>"top"));
   ...
   //股票明细
	...
	{
		$id=$_REQUEST['id'];
		if(M($fun_stock->name)->where("id={$id}")->delete()){
			$this->success('删除成功');
		}else{
		    $this->error('删除失败');
		}
	...
	{
		$id=$_REQUEST['id'];
		if(M($fun_stock->name."走势")->where("id={$id}")->delete()){
			$this->success('删除成功');
		}else{
		    $this->error('删除失败');
		}
	...
	public function stockAddSave($fun_stock)
	{
		...
		$fun_stock->setrecord($user['编号'],0,$num,$_POST['account'],$_POST['memo'],2,true,false);
		M()->commit();
		$this->success('成功');
	}	
}
