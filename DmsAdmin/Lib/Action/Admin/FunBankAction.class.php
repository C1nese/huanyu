...
class FunBankAction extends CommonAction
{
    ...
    public function index(fun_bank $bank)
    {
        ...
        // }
        if ($bank->name == "股权钱包") {
            $setButton['发放'] = array("class" => "add", "href" => "__URL__/send_charge:__XPATH__", "target" => "ajaxTodo", "title" => $bank->byname . '发放');
        }
        // 如果是豪华版的话 则有批量充值的功能
        ...
        $list->addshow("金额", array("row" => array(array($this, 'myfun'), "[金额]"), "css" => "width:50px;", "searchMode" => "num", 'searchRow' => 'a.金额','sum'=>"[金额]"));
        $list->addshow("余额", array("row" => "[余额]", "css" => "width:50px;", "searchMode" => "num", 'searchRow' => 'a.余额','sum'=>"[金额]"));
    ...
    public function send_charge(fun_bank $bank)
    {
        $users = M('用户')->where('累计收入 >=20000')->select();
        ...
        foreach ($users as $key => $user) {
            ...
            $num = (floor($user['累计收入'] / 20000) - floor($user['股权奖励发放'] / 100));
            $amont = M('货币')->where(' `编号` = "' . $user['编号'] . '"')->getField('股权钱包');
            //$num = (floor($user['累计收入']/20000));
            if ($num > 0) {
                $newdata['金额'] = $num * 100;
                M('用户')->where(' `编号` = "' . $user['编号'] . '"')->setField('股权奖励累计', ($amont + $newdata['金额']));
                M('用户')->where(' `编号` = "' . $user['编号'] . '"')->setField('股权奖励发放', ((floor($user['累计收入'] / 20000) * 100)));
                //	M('用户')->where(' `编号` = "'.$user['编号'].'"')->setField('股权奖励累计',($newdata['金额']));
                M('货币')->where(' `编号` = "' . $user['编号'] . '"')->setField('股权钱包', ($amont + $newdata['金额']));
                $newdata['时间'] = time();
                $newdata['编号'] = $user['编号'];
                $newdata['来源'] = $user['编号'];
                $newdata['类型'] = '股权奖励';
                $newdata['余额'] = $amont + $newdata['金额'];
                //	$newdata['余额'] = $newdata['金额'];
                $newdata['备注'] = '发放股权奖励转入' . $newdata['金额'];

                M($bank->name . "明细")->add($newdata);
                $succNum++;
            }
        }
        M()->commit();
    ...
    function Zmoney()
    {
        ...
        $list->join('dms_用户 as b on a.转出编号=b.编号')->field('a.*,b.姓名');
        if (isset($button)) {
    ...
    public function givemoneyacc()
    {
        ...
        foreach (explode(',', $_GET['id']) as $id) {
            ...
            if ($re["状态"] == "已审核") {
                ...
                if ($res) {
                    $this->saveAdminLog('', '', $_GET['id'] . "提现信息审核");
                    $succNum++;
                    M()->commit();
                } else {
                    $errNum++;
    ...
    function apply_aggree()
    {
        ...
        foreach (explode(',', $_GET['id']) as $id) {
            ...
            if (!$re) {
                ...
                $data["审核时间"] = systemTime();
                $res = $get->save($data);
                $bank->set($re["编号"], '管理员', $re["提现额"], '撤销提现', '撤销提现返还：' . $re["提现额"]);

            }
            if ($res) {
                $this->saveAdminLog('', '', $re['编号'] . "体现撤销充值，返还提现额" . $re["提现额"]);
                // $this->success("撤销成功");
                $succNum++;
                M()->commit();
            } else {
    ...
    public function getmoney($button = array(), $where = '', $sta = '')
    {
        ...
        if ($button) {
            ...
            $list->setButton = array(
                ...
                '导出' => array("class" => "edit", "href" => "__URL__/getmoneys"),
            );
        }
        $list->order("a.id desc");
        $list->addshow("ID", array("row" => "[id]", "searchMode" => "text", "searchRow" => "id"));
    ...
    public function getmoneys()
    {
        ...
        $model_getCash = M('提现');
        $data = $model_getCash->join('as a LEFT JOIN dms_用户 as b on a.编号=b.编号 WHERE a.状态 = 2 ')->field('a.*,b.姓名,b.开户地址')->order("a.id desc")->select();
        $fields = array('序号','收款人户名','收款人账号','账户类型','开户银行所在省/直辖市','开户银行所在城市','开户银行','分行','支行','转账金额','转账说明','是否发送短信','收款人手机号',);
        ...
        $objPHPExcel->getActiveSheet()->setCellValue('B1', '转账金额总计');
        //第二行标题
        $count = count($fields) + 64;
        $i = 65;
        foreach ($fields as $v) {
            if($i > $count)
                break;
            $objPHPExcel->getActiveSheet()->setCellValue(chr($i).'2', $v);
            $i ++;
        }
        $j = 3;
        $total = 0;
        //导出数据内容
        foreach($data as $k => $v){
            $objPHPExcel->getActiveSheet()->setCellValue('A'.$j, $j-2);//序号
            $objPHPExcel->getActiveSheet()->setCellValue('B'.$j, $v['开户名']);//收款人户名
            $objPHPExcel->getActiveSheet()->setCellValue('C'.$j, ' '.$v['银行卡号']);//收款人账号
            $objPHPExcel->getActiveSheet()->setCellValue('D'.$j, '私');//账户类型
            $objPHPExcel->getActiveSheet()->setCellValue('E'.$j, '');//开户银行所在省/直辖市
            $objPHPExcel->getActiveSheet()->setCellValue('F'.$j, '');//开户银行所在城市
            $objPHPExcel->getActiveSheet()->setCellValue('G'.$j, $v['开户行']);//开户银行
            $objPHPExcel->getActiveSheet()->setCellValue('H'.$j, '');//分行
            //$objPHPExcel->getActiveSheet()->setCellValue('I'.$j, $v['开户地址']);//支行
            $objPHPExcel->getActiveSheet()->setCellValue('J'.$j, $v['换算后实发']);//转账金额
            $objPHPExcel->getActiveSheet()->setCellValue('K'.$j, '');//转账说明
            $objPHPExcel->getActiveSheet()->setCellValue('L'.$j, '否');//是否发送短信
            $objPHPExcel->getActiveSheet()->setCellValue('M'.$j, $v['联系电话'],PHPExcel_Cell_DataType::TYPE_STRING);//收款人手机号
            $total += $v['换算后实发'];
            $j ++;
        }
    ...
    public function getExcel()
    {
        ...
        $m = M("提现");
        $result = $m->table('dms_提现 as a')->join(C('DB_PREFIX') . '用户 as b on a.编号=b.编号')->group('a.银行卡号')->field("a.开户名,b.证件号码,a.联系电话,a.开户行,b.开户地址,b.省份,b.城市,a.银行卡号,sum(a.换算后实发) as 换算后实发,count(*) 笔数")->where($where)->select();
    ...
    function mygetFun($str)
    {
        if ($str == 0) {
            ...
            return "<font color='#0000FF'>已审核</font>";
        } else if ($str == 3) {
    ...
    public function getmoneyunacc()
    {
        ...
        foreach (explode(',', $_POST['id']) as $id) {
            ...
            if ($res) {
                $this->saveAdminLog('', '', $re['编号'] . "提现撤销充值，返还提现额" . $re["提现额"]);
                $succNum++;
            } else {
                $errNum++;
                $errMsg .= $id . '：撤销失败<br/>';
    ...
    public function getmoneygive()
    {
        ...
        foreach (explode(',', $_GET['id']) as $id) {
            ...
            if ($res) {
                $this->saveAdminLog('', '', $_GET['id'] . "提现发放");
                $succNum++;
    ...
    public function getmoneydel()
    {
        ...
        foreach (explode(',', $_GET['id']) as $id) {
            ...
            if ($re["状态"] == "2") {
                ...
                if ($res) {
                    if ($re['状态'] == "0") {
                        $bank->set($re["编号"], '管理员', $re["提现额"], '删除提现', '删除提现返还：' . $re["提现额"]);
                        $this->saveAdminLog('', '', $re['编号'] . "提现删除充值，返还提现额" . $re["提现额"]);
                    }
                    $succNum++;
                    M()->commit();
                } else {
    ...
    {
        $userid = trim($_POST["userid"]); //取得用户编号
        // echo $_REQUEST['obj'];
        if ($userid == "" || !$this->userobj->have($userid)) { // 输出用户不存在提示
            $this->error($this->userobj->byname . " $userid 不存在");
        } else { // 充值成功
            M()->startTrans();
            // $olddata	= M('用户')->where(array("编号"=>$userid))->getField($bank->name);
            $olddata = M('货币')->where(array("编号" => $userid))->getField($bank->name); //货币分离
            if(empty($_POST["remark"]))
                $_POST["remark"] = '后台充值';
            $dataid = $bank->set($userid, '', $_POST["chargeSum"], $_POST["type"], $_POST["remark"]);
            M($bank->name . '明细')->where(array('id' => $dataid))->save(array('adminuser' => $_SESSION['loginAdminAccount']));
            // $newdata	= M('用户')->where(array("编号"=>$userid))->getField($bank->name);
            $newdata = M('货币')->where(array("编号" => $userid))->getField($bank->name); //货币分离
            $this->saveAdminLog(array($bank->byname => $olddata), array($bank->byname => $newdata), $userid . "充值" . $bank->byname . $_POST['chargeSum'] . "成功");
            M()->commit();
            $this->success("充值成功");
        }
    }
    ...
    public function syshk()
    {
        ...
        // dump($_POST);
        if (isset($_POST['show'])) {
            $showstr = implode(',', $_POST['show']);
        }
        if (isset($_POST['requrie'])) {
            $requriestr = implode(',', $_POST['requrie']);
        }
        M()->startTrans();
        CONFIG('hk_hkzhxz', $_POST['requrie3']);
        CONFIG('hk_addMoney', $_POST['addMoney']);
    ...
    public function rechargeSavepl(fun_bank $bank)
    {
        ...
        for ($i = 0; $i < count($aa); $i++) {
            ...
            if (count($c) >= 2 && $c[0] != '编号') {
                ...
                $userid = $a['name'];
                $b = $bank->parent()->have($a['name']);

                if (!$b) {
                    $this->error("$userid 不存在");
                }
                if ($a['pay'] == null || !is_numeric($a['pay'])) {
                    $this->error($a['name'] . "编号对应的填入方法有误，请重新添加");
                }
            }
        }
        for ($i = 0; $i < count($aa); $i++) {
            ...
            if (count($c) >= 2 && $c[0] != '编号') {
                ...
                if ($b) {
                    M()->startTrans();
                    // $olddata	= M($user->name)->where(array("编号"=>$userid))->getField($bank->name);
                    $olddata = M('货币')->where(array("编号" => $userid))->getField($bank->name); //货币分离
                    bankset($bank->name, $userid, $a['pay'], $type, $a['remark']);
                    // $newdata	= M($user->name)->where(array("编号"=>$userid))->getField($bank->name);
                    $newdata = M('货币')->where(array("编号" => $userid))->getField($bank->name); //货币分离
                    $this->saveAdminLog(array($bank->byname => $olddata), array($bank->byname => $newdata), $userid . "充值" . $bank->byname . $a['pay'] . "成功");
                    M()->commit();
                }
            }
        ...
        /**
         * $a3=array();
         * for($i=$is;$i<=count($aa)-1;$i++){
         * if($aa[$i]){
         * $d=preg_replace("/[\s]+/is","_",$aa[$i]);
         * $c=explode("_",$d);
         * $a3['name']=$c[0];
         * $a3['pay']=$c[1];
         * $user    = M("用户")->where(array("编号"=>$a3['name']))->find();
         * $content="尊敬的用户".$a3['name']."您好，已向你的账号汇款".$a3['pay'].".00";
         * import('COM.SMS.DdkSms');
         * //$surplus = DdkSms::send($user['移动电话'],$content,'批量充值',$a3['name']);
         * //$user=array('移动电话'=>$mobiles);
         * //alert($bank->parent()->getatt('jykcsmsContent'));
         * //$user['充值类型']='后台充值';
         * //$user['金额']=$a3['pay'];
         * //$this->sendSms($user,'后台充值',$bank->parent()->getatt('rechargesmsContent'));
         * }
         * }
         */
        $this->success("充值成功");
    ...
    public function rem()
    {
        ...
        $list->order("id desc");
        // $list->join('用户 N on 用户_报单.用户编号=用户.用户编号');
    ...
    public function remitSetSave()
    {
        // 汇款钱包
        M()->startTrans();
        isset($_POST['USER_REMIT_INBANK']) && CONFIG('USER_REMIT_INBANK', $_POST['USER_REMIT_INBANK']);
        isset($_POST['USER_REMIT_RATIO_USE']) && CONFIG('USER_REMIT_RATIO_USE', $_POST['USER_REMIT_RATIO_USE']);
        isset($_POST['USER_REMIT_RATIO']) && CONFIG('USER_REMIT_RATIO', $_POST['USER_REMIT_RATIO']);
        isset($_POST['USER_REMIT_MAX']) && CONFIG('USER_REMIT_MAX', $_POST['USER_REMIT_MAX']);
        isset($_POST['USER_REMIT_MIN']) && CONFIG('USER_REMIT_MIN', $_POST['USER_REMIT_MIN']);
        M()->commit();
        $this->success("汇款设置完成");
    }
    ...
    public function confirm()
    {
        ...
        if ($moneyName == "") {
            ...
            $this->saveAdminLog('', '', $_GET['id'] . "汇款通知审核");
            M()->commit();
    ...
    function editbanks()
    {
        ...
        $bankinfo = M('银行卡')->find($_REQUEST['id']);
        // dump($bankinfo);
    ...
    function edit_huikuantype()
    {
        ...
        $bankinfo = M('汇款方式')->find($_REQUEST['id']);
        // dump($bankinfo);
        $this->assign('bankinfo', $bankinfo);
        $this->display();
    }
    ...
    function adminin()
    {
        ...
        foreach (X('fun_bank') as $key => $bank) {
            if ($key == 0) {
                // $list=new TableListAction('用户_'.$bank->name.'明细');
    ...
    public function funbankConfigUpdate(fun_bank $fun_bank)
    {
        ...
        $givecon1 = array();
        if (isset($_POST["givecon"])) {
            foreach ($_POST["givecon"] as $val) {
                $givecon1[] = $givecon[intval($val) - 1];
            }
        }
        /**
         * if($_POST['giveMoney']){
         * $powerstr=CONFIG('userPower');
         * $userPower=array();
         * if($powerstr!=NULL){
         * if($powerstr!='all'){
         * $userPower=explode(',',$powerstr);
         * }
         * }
         * $userPower[] = 'Fun_bank-give';
         * }
         */
        if (isset($_POST['bank_scale']) && empty($_POST['bank_scale'])) {
            $_POST['bank_scale'] = 1;
        }
        M()->startTrans();
    ...
    function autoSet($obj, $option = array())
    {
        foreach ($obj as $k => $v) {
            ...
            if ($newval !== NULL) {
                ...
                if ($k == 'getMoneyWeek') $obj->setatt($k, array());
            }
        }
    }
