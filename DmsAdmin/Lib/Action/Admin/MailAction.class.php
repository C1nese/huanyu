...
class MailAction extends CommonAction {
	...
	public function index(){
        ...
        $list->addshow("邮件标题",array("row"=>array(array(&$this,"getLink"),'[id]','[标题]')));
	...
	public function getDofun($status,$id,$senderType){
		...
		$str .= '<a target="ajaxTodo" title="确定要删除吗?" href="'.__APP__.'/Admin/Mail/del/id/'.$id.'">删除</a>';
		return $str;
	}
	//邮件标题变链接
    public function getLink($id,$title){
        $str = '<a target="dialog" mask="true" href="'.__APP__.'/Admin/Mail/view/id/'.$id.'" width="700" height="450">'.$title.'</a>';
        return $str;
    }
	//邮件状态
	...
	public function answerSave(){
		...
		$data['回复内容']=get_magic_quotes_gpc() ? stripslashes($_POST['answerContent']) : $_POST['answerContent'];
	...
	public function send_email(){
		...
		$where['编号']	= $_POST['receiver'];

		if($_POST['type']=="")
		{
			$list	= M('用户')->where($where)->getField('id');
