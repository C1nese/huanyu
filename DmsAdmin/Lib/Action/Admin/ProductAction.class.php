...
class ProductAction extends CommonAction {
    ...
    */
	public function index($product){
		...
		$setButton=array(
			"添加"=>array("class"=>"add"   ,"href"=>__APP__."/Admin/Product/add:__XPATH__","target"=>"dialog"  ,"mask"=>"true",'width'=>'600','height'=>'550'),
			"修改"=>array("class"=>"edit"  ,"href"=>__APP__."/Admin/Product/edit:__XPATH__/id/{tl_id}"  ,"target"=>"dialog"  ,"mask"=>"true",'width'=>'600','height'=>'550'),
	...
	private function getData($product,$action='add'){
		$data = array();		
		if(isset($_POST['pro_gong'])){
	    	$data['所属功能'] = implode(',',$_POST['pro_gong']);
		}
		isset($_POST['itemid']) && $data['产品编码'] = $_POST['itemid'];
		isset($_POST['category']) && $data['分类'] = $_POST['category'];
		isset($_POST['name']) && $data['名称'] = trim($_POST['name']);
		isset($_POST['image']) && $data['图片'] = $_POST['image'];
		isset($_POST['number']) && $data['数量'] = abs(intval($_POST['number']));
		isset($_POST['number2']) && $data['可订购数量'] = abs(intval($_POST['number2']));//修改
		isset($_POST['costprice']) && $data['成本价'] = abs(floatval($_POST['costprice']));
		isset($_POST['price']) && $data['价格'] = abs(floatval($_POST['price']));
		//if($product->productPV){
			isset($_POST['pv']) && $data['PV'] = abs(floatval($_POST['pv']));
		//}
		//获取自定义价格
		$cons = $product->getfieldCon();
		foreach($cons as $feild){
			isset($_POST[$feild['name']]) && $data[$feild['name']] = abs(floatval($_POST[$feild['name']]));
		}
		isset($_POST['wight']) && $data['重量'] = floatval($_POST['wight']);
		isset($_POST['guige']) && $data['规格'] = floatval($_POST['guige']);
		...
		$pro_count = M('产品_功能')->where(array('状态'=>'使用'))->count();
		if($pro_count>0 && !isset($_POST['pro_gong'])){
		  	$this->error('所属功能必选');
		}
		if($data['名称'] ===''){
			$this->error('名称必填!');
    ...
    */
	...
	public function addproNum($product){
		...
		$list->where("b.数量>0 and b.产品节点='".$product->name."'");
