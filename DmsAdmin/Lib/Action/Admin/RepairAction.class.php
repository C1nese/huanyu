...
class RepairAction extends CommonAction {
	...
	public function newtop()
	{
		...
		foreach(X('net_rec') as $net)
		{
			...
			//对原先原始点设置上级
			M()->execute("update `dms_用户` set ".$net->name."_上级编号='".$user['编号']."' where  ".$net->name."_层数=1 and id<>'".$user['id']."'");
			//对其他人网体数据做批量更新
			M()->execute("update `dms_用户` set ".$net->name."_层数=".$net->name."_层数+1,".$net->name."_网体数据=concat('".$user['id'].",',".$net->name."_网体数据) where ".$net->name."_层数>1 and id<>'".$user['id']."'");
			M()->execute("update `dms_用户` set ".$net->name."_层数=".$net->name."_层数+1,".$net->name."_网体数据='".$user['id']."' where ".$net->name."_层数=1 and id<>'".$user['id']."'");
		}
		foreach(X('net_place') as $net)
		{
			...
			M()->execute("update `dms_用户` set ".$net->name."_上级编号='".$user['编号']."',".$net->name."_位置='".$qu."' where  ".$net->name."_层数=1 and id<>'".$user['id']."'");
			M()->execute("update `dms_用户` set ".$net->name."_层数=".$net->name."_层数+1,".$net->name."_网体数据=concat('".$user['id']."-".$qu.",',".$net->name."_网体数据) where ".$net->name."_层数>1 and id<>'".$user['id']."'");
			M()->execute("update `dms_用户` set ".$net->name."_层数=".$net->name."_层数+1,".$net->name."_网体数据='".$user['id']."-".$qu."' where ".$net->name."_层数=1 and id<>'".$user['id']."'");
		}
		M()->commit();
		$this->success('操作完成！');
	}
    ...
    {
    	$sales=M('报单')->where(array('id'=>$_GET['saleid']))->select();
    	if(!$sales)
    	{
    		$this->error('未查到任何订单');
    	}
		...
		if(get_class($net)=='net_rec')
		{
			...
			foreach($m_users as $m_user)
			{
				...
				if($m_user[$net->name.'_团队人数'] != $netnum[$m_user['id']])
				{
					...
					M('用户')->where(array("编号"=>$m_user['编号']))->save(array($net->name.'_团队人数'=>$netnum[$m_user['id']]));
				}
				if($m_user[$net->name.'_团队总人数'] != $netallnum[$m_user['id']])
				{
					dump($m_user['编号'].'团队总人数异常目前为'.$m_user[$net->name.'_团队总人数'].'检测为'.$netallnum[$m_user['id']]);
					M('用户')->where(array("编号"=>$m_user['编号']))->save(array($net->name.'_团队总人数'=>$netallnum[$m_user['id']]));
				}
				//修改推荐_推荐人数
				$userCount = M('用户')->where(array($net->name."_上级编号"=>$m_user['编号']))->count();
				if($m_user[$net->name.'_推荐人数'] != $userCount)
				{
					dump($m_user['编号'].'推荐人数异常目前为'.$m_user[$net->name.'_推荐人数'].'检测为'.$userCount);
					M('用户')->where(array("编号"=>$m_user['编号']))->save(array($net->name.'_推荐人数'=>$userCount));
				}
			}
		}
		M()->commit();
