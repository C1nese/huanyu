...
class PublicAction extends Action {
	...
	public function login()
    {
		...
		if($SYSTEM_STATE==1){
			if($ifopen){
				...
				$this->assign('openTimeStr',isset($SYSTEM_CLOSE_TITLE)?$SYSTEM_CLOSE_TITLE:'非开放时间，当前时间不可访问');
			}else{
				$this->assign('isOpenTime','1');
				$this->assign('openTimeStr','当前时间可以访问');
			}
		}
		if($SYSTEM_STATE==2){//阻止状态
			if($ifopen){
				$this->assign('isOpenTime','1');
				$this->assign('openTimeStr','当前时间可以访问');
			}else{
				$this->assign('isOpenTime','0');
				$this->assign('openTimeStr',isset($SYSTEM_CLOSE_TITLE)?$SYSTEM_CLOSE_TITLE:'非开放时间，当前时间不可访问');
				//die('<div style="margin:200px 0 30px 0;text-align:center;"><img src="/Public/Images/SYSTEM_CLOSE_TITLE.jpg"></div><div style="margin-bottom:200px;text-align:center;font-size:35px;font-weight:bold">'.$SYSTEM_CLOSE_TITLE.'</div>');
			}
		}
		//无法访问
		//if($SYSTEM_STATE==3){
	...
	public function check()
	{
		...
		if($SYSTEM_STATE==1){
			...
			if(!$ifopen){
				die('<div style="margin:100px 0 30px 0;text-align:center;"><img src="/Public/Images/wzzzwhz_img.jpg">'.$SYSTEM_CLOSE_TITLE.'</div>');
			}
		}
		...
		if(adminshow('mustout')){//数据量大了后，每次刷新都更新时间有点占进程，需要优化
			...
			M()->startTrans();
			M('用户')->where(array('id'=>(int)$authInfo['id']))->save(array('最后登入IP'=>$_SESSION['ip'],'sessionid'=>$sessionid));
			M()->commit();
		}
		$this->redirect("Index/index");
	}
