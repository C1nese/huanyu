<?
	...
	class apiuser
	{
		...
		**商城与结算的用户信息对应字段 key值是商城字段  val值结算字段
		*/
		...
		*/
		function salecheck($data=array()){
			$resuser=M("用户")->where("编号='".$data["username"]."'")->find();
			if($resuser){
				$result['status']=true;
			}else{
				$result['status']=false;
		...
		/*
		**注册新用户，商城传输数据过来注册，获得注册的信息，然后根据用户信息字段对应生成提交的表单信息
		**注册时直接提交到sale_reg的regsave()函数，所以在xml中设置sale_reg节点<sale_reg lvName="用户级别" name='商城注册' byname="用户注册" user='商城'  confirm="true"></sale_reg>
		**节点中适合运行所有程序，与普通的用户和后台admin注册是一致的使用方法，可加addval、update等节点
		*/
		function salereg($regdata){
			 ...
			 *得到用户编号后，查询结算中用户是否存在
			*/
			$haveuser=M("用户")->where("编号='".$regdata['username']."'")->find();
			if($haveuser){
				$result['error']="用户已在结算注册";
				return $result;
			}else{
				/*
		...
		/*
		**升级用户，商城传输数据过来升级，用户进行升级必须在结算中存在这个用户，不存在返回商城 用户不存在，获得升级的信息，然后根据用户的级别以及要升的级别生成提交的表单信息
		**升级时直接提交到sale_up的upsave()函数，所以在xml中设置sale_up节点<sale_up name='商城升级' byname='商城升级' lvname='用户级别' user='商城'  confirm="true" productname="商城产品"></sale_up>
		**节点中适合运行所有程序，与普通的用户和后台admin注册是一致的使用方法，可加addval、update等节点
		*/
		function upsale($salelists){
			...
			*/
			...
			$user=M("用户")->where(array("编号"=>$salelists['username']))->find();
			if(!$user)
			{
				$result['error']="结算系统用户不存在";
				return $result;
			}
		...
		public function getuserinfo($postdata){
			...
			if($username){
				$userinfo=M("用户")->where(array("编号"=>$username))->find();
				if($userinfo){
		...
		public static function getbank($postdata,$getdata){
			...
			if($username){
				...
				if($userinfo){
					...
					$result=array("status"=>false,"money"=>0,"money1"=>0,"error"=>"用户不存在");
				}
			}
			return $result;
		}
		public function getbanks(&$postdata){
			...
			if($username){
				...
				if($userinfo){
					...
					$result=array("status"=>false,"money"=>0,"error"=>"用户不存在");
				}
			}
			unset($userinfo);
			return $result;
		...
		public function editpass($passinfo){
			...
			M("用户")->where(array("编号"=>$passinfo['usename']))->save($data);
			return $result;
		}
		public function clock($data){
			$result=array("status"=>false,"error"=>"");
			if($data['username']){
				$userinfo=M("用户")->where(array("编号"=>$data['username']))->find();
				if($userinfo){
					M("用户")->where(array("编号"=>$data['username']))->save(array("登陆锁定"=>$data['clock_status']));
					$result['status']=true;
				}else{
					$result['error']="未找到用户信息";
				}
			}else{
				$result['error']="未得到用户编号";
			}
			return $result;
		}
