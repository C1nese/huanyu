...
class duserModel extends model
{
	...
	//对各类级别，业绩做回退生成临时性业绩表
    // 数据表名（不包含表前缀）
    protected $tableName = '用户';
    // 实际数据表名（包含表前缀）
    protected $trueTableName ='dms_用户';
    private static $time;
    //设置日期
		...
		*/
		...
		foreach(X('net_rec') as $net)
		{
			...
			$rowset[]=array(
				...
				'join'    =>"(select ".$net->name."_上级编号,count(*) num from dms_用户 where 审核日期 >=".(self::$time + 86400)." and ".$net->name."_被推荐数>0  group by ".$net->name."_上级编号) netrec{$key} on netrec{$key}.".$net->name."_上级编号=u.编号",
				);
		}
		foreach(X('net_rec') as $net)
		{
