...
/*
	...
	第二种是某一层每个区有至少一个有效用户
		...
		如两条线至少都有一个2级用户能拿500,两条线各自都有一个3级用户能拿800,可以使用{minlv}关键字
...
class prize_layer extends prize
{
	...
	public function calculate($net,&$from,$userid,$sale=null,$minlayer,$maxlayer,&$cons,$num_ratio)
	{
		...
		foreach($upusers as $upuser)
		{
			...
			//判断当前层是否计算过,还是否需要在计算
			//$newdata=M('用户')->where(array("id"=>$upuser["id"]))->find();
			$where = array();
			$where['编号'] = $upuser["编号"];
			$where['层数'] = $layer;
			...
			{
				$downwhere="  and  find_in_set( '{$upuser['id']}-{$qu}',{$net->name}_网体数据 )";
				if($sql!='')$sql.=' UNION ALL ';
				$sql.="select * from( SELECT ".$fields.",'".$qu."' as region  FROM `dms_用户` WHERE ".$where.$downwhere." order by ".$orderName." ".$limit.") a".$key;
			}
				...
				foreach($cons as $con)
				{
					...
					if($con['minlv'] <= $up_rs_lv && $con['maxlv'] >= $up_rs_lv && $con['minlayer'] <= $layer && ($con['maxlayer'] >= $layer || $con['maxlayer']==-1) && transform($where,array(),$wheredata))
					{
						...
						if($prizenum>0){
							...
							if($upuser['用户级别']==10 && $prizenum>CONFIG('cfv')){
								$prizenum=CONFIG('cfv');
								$calculateType=$calculateType.'封顶';
							}elseif($upuser['用户级别']==20 && $prizenum>CONFIG('cfh')){
								$prizenum=CONFIG('cfh');
								$calculateType=$calculateType.'封顶';
							}elseif($upuser['用户级别']==30 && $prizenum>CONFIG('cfb')){
								$prizenum=CONFIG('cfb');
								$calculateType=$calculateType.'封顶';
							}elseif($upuser['用户级别']==40 && $prizenum>CONFIG('cfz')){
								$prizenum=CONFIG('cfz');
								$calculateType=$calculateType.'封顶';
							}
