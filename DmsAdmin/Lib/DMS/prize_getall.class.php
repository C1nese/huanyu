		...
		function cal()
		{
			...
			if($this->rowFrom == 1)
			{
				...
				foreach($cons as $con)
				{
					...
					//根据条件查询到所有用户
					$usercount=$con['weighing']=="" ? count($users) : D("user")->where($where)->sum(delsign($con['weighing']));
					if($users)
					{
						//遍历所有用户.产生奖金
						foreach($users as $user)
						{
							...
							if($weighing!=0)
							{
								...
								if(strstr($con["val"],'%')){
									$calculateType =$t_num.'/'.$usercount.'*'.$weighing.' * '.$con["val"].'%*'.$user[$this->name.'比例'].'%'; 
								}else if(strstr($con["val"],'*')){
									$calculateType = $t_num.'/'.$usercount.'*'.$weighing . $con["val"].'*'.$user[$this->name.'比例'].'%'; 
								}else{
									$calculateType = $con["val"].'*'.$user[$this->name.'比例'].'%';
								}
