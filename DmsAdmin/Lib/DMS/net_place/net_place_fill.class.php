...
class net_place_fill
{
	...
	static function findEmptyPostionByChilds($net,$user)
	{
		...
		//*******************************************
		$sql				= "select {$net->name}_层数 as 所在层数 from dms_用户 where {$net->name}_层数>=1 and (";
		$sql2				= '';
		foreach( $locationList as $key=>$location )
		{
			if( $key >=1 )
		...
		$sql		= "select {$field_str} from dms_用户 where {$net->name}_层数={$layer} and (";
		...
		$finally_result[]	= $result_order['编号'];
		//确定区位
		foreach( $locationList as $key=>$location )
		{
