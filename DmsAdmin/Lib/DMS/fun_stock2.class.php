	...
	class fun_stock2 extends stru
	{
		...
		private function sell($userid)
		{
			...
			if($jilu['num'] != '' && $jilu['isSell'] == 0)
			{
				...
				if($this->selltype==1)
				{
					...
					M('用户')->where(array('编号'=>$userid))->save(array($this->name=>$sellstock['num'],'出局'=>1));
		...
		*/
			...
			if($priceup)
			{
				...
				foreach($haves as $have)
				{
					...
					else
					{
						...
						//如果是用户之间不存在交易，那么挂单卖出以后。需要公司进行回购处理
		...
		public function event_sysclear()
		{
			...
			//持有的是，个人持有总股票数量的具体不同时间段购买的记录，持有记录中的股票总数，应该等于用户实际股权钱包
