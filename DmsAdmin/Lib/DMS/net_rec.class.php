<?php
	class net_rec extends net
	{
		...
		public static $_cache=array();//奖金计算时的用户缓存记录的数组
		...
		public function setRegData(&$udata,$data,$isAjax){
			if($this->regDisp)
			{
				//如果开启了regdisp并且在后台，或者在前台并且，setNowUser==false的时候则正常赋值
				//如果是前台并开启了setNowUser,则把上级编号赋予，当前登入用户
				//用户前台注册不设置当前用户为上级编号或后台注册
				if(GROUP_NAME == 'Admin' || $this->setNowUser == false)
				{
				...
				}else if(GROUP_NAME == 'User' && $this->setNowUser)//用户前台注册设置当前用户为上级编号
				{
					$map['id']=$_SESSION[C('USER_AUTH_KEY')];
					$upuser = M('用户','dms_')->where($map)->field('编号')->find();
					$udata[$this->name.'_上级编号'] = $upuser['编号'];
				}
				//根据上级编号处理相关默认数据
		...
		public function set_Depth(&$user)
		{
			...
			$sql="update dms_用户 set {$this->name}_深度={$thislayer}-{$this->name}_层数 where {$this->name}_深度<{$thislayer}-{$this->name}_层数";
			if($user[$this->name."_网体数据"]!='')$sql.="  and id in (".$user[$this->name."_网体数据"].")";
			M()->execute($sql);
		}		
		//根据用户从新计算上级推荐人数以及其被推荐数
		public function set_recnum(&$user)
		{
			...
			M('用户','dms_')->where(array('编号'=>$user[$this->name.'_上级编号']))->setInc($this->name."_推荐人数",1);
			$uptuijian=M('用户','dms_')->where(array('编号'=>$user[$this->name.'_上级编号']))->getField($this->name."_推荐人数");
			M('用户','dms_')->where(array('编号'=>$user['编号']))->setField($this->name."_被推荐数",$uptuijian);
			$user[$this->name.'_被推荐数']=$uptuijian;
		}
		//设置团队人数$verify=0为未审核人数,1为已审核人数,2为两类人数
		public function set_groupnum($user,$num,$verify)
		{
			//如果用户网体数据为空（表示原始点或者目前处于快速插入模式，不执行以下操作）
		...
		public function set_index(&$user)
		{
			...
			{
				$upuser = M("用户")->where(array("编号"=>$user[$this->name.'_上级编号']))->field("id,".$this->name.'_网体数据,'.$this->name.'_层数')->find();
		...
		{
			$tjcount=M("用户",'dms_')->lock(true)->where(array($this->name."_上级编号"=>$username))->count();
        ...
        public function recLock($netname,$recname){
			$recnet=M('用户','dms_')->lock(true)->where(array("编号"=>$netname))->getField($this->name."_网体数据");
		...
		public function renovate()
		{
			...
			$m_user  = M('用户');
			//找到无上级用户
			M()->execute("update dms_用户 set ".$this->name."_上级编号='' where ".$this->name."_上级编号 is null");
		...
		public function editTjnum($upuser,$user=''){
			...
			$Downusers=M('用户')->where($where)->order("审核日期 asc")->field('id')->select();
			if($Downusers){
				$px=1;
				foreach($Downusers as $duser){
					M('用户')->where(array('id'=>$duser['id']))->setField($this->name."_被推荐数",$px);
					$px++;
				}
			}
		}
		...
		public function move($user,$newup,$region)
		{
			...
			//判断推荐人数
			if($this->maxUser>0){
				$tjcount=M('用户')->where(array($this->name."_上级编号"=>$newup['编号']))->count();
				if($tjcount>=$this->maxUser){
			...
			if($user['状态']=='有效'){
				if(!($this->nullRecer=='false' && $user['空点']==1))M('用户')->where(array('编号'=>$newup['编号']))->setInc($this->name."_推荐人数",1);
				if(!($this->nullRecer=='false' && $user['空点']==1))M('用户')->where(array('编号'=>$user[$this->name.'_上级编号']))->setDec($this->name."_推荐人数",1);
			}
			//=======================================================================
			//对移网用户本身的上级信息更新
			M('用户')->where(array("编号"=>$user['编号']))->save(array( $this->name . '_上级编号' => $newup['编号']));
			//生成新网体数据
			...
			//对老网体数据进行更新
			M()->execute("update `dms_用户` set ".$this->name."_网体数据=replace(".$this->name."_网体数据,'".$user[$this->name.'_网体数据'].','.$user['id'].",','".$newnet.",".$user['id'].",')");
			M()->execute("update `dms_用户` set ".$this->name."_网体数据='".$newnet.",".$user['id']."' where ".$this->name."_上级编号='".$user['编号']."'");
			M()->execute("update `dms_用户` set ".$this->name."_网体数据='".$newnet."' where id=" . $user['id']);
			//触发网体移动事件
		...
		public function repair()
		{
	   	    ...
	   	    //取得需要用的信息表
	   	    $userdata = M('用户')->getField("编号,{$this->name}_上级编号 上级编号,id,状态,0 num,0 allnum");
	   	    //取得要处理用户的信息表
	   	    ...
	   	    foreach($upusers as $id=>$name)
	   	    {
	   	   	...
	   	   	M('用户')->bUpdate();
	   	   	//更新推荐人数
	   	   	M()->execute('update dms_用户 a left join (select '.$this->name.'_上级编号 编号,count(*) num from dms_用户 where 状态=\'有效\' group by '.$this->name.'_上级编号) b on a.编号=b.编号 set a.'.$this->name.'_推荐人数 = ifnull(b.num,0)');
	   	   	//更新被推荐数
	   	   	M()->execute('update dms_用户 set '.$this->name.'_被推荐数=0');
	   	   	M()->execute("set @pdept:='';");
	   	   	M()->execute("set @rank:=0;");
	   	   	M()->execute('update dms_用户 a  join (select * from (SELECT id,if(@pdept='.$this->name.'_上级编号,@rank:=@rank+1,@rank:=1) as rank,@pdept:='.$this->name.'_上级编号  FROM `dms_用户`  where 状态=\'有效\' order by '.$this->name.'_上级编号 asc,审核日期 asc,id asc) c) b on a.id=b.id set a.'.$this->name.'_被推荐数=b.rank;');
		}
