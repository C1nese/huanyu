	...
	class sale_up extends sale
	{
		...
		public function getValidate($data_post=array())
		{
			...
			if($this->lvName==X("sale_reg@")->lvName){
				//$ret[] = array('lv',array($this,"is_usernull"),'用户为空点，请先转正',2,'function',3,array(trim($data_post['userid'])));
			}
			$m=M();
		...
		public function is_usernull($lv,$userid){
			if(!isset($userid) || $userid == '')
				return false;
			$null=M("用户")->where(array("编号"=>$userid))->getField("空点");
			if($null==1){//回填或空点激活
		...
		public function v_iflv($lv,$userid)
		{
			if(!isset($userid) || $userid == '')
				return true;
			$user   =M('用户')->lock(true)->where(array('编号'=>$userid))->find();
			if(!$user)
			return true;
			//第一个注册的sale_reg级别
		...
		public function upSave($option=array())
		{
            ...
            $smess=M('报单')->where("编号='".$sdata['编号']."'")->order("id desc")->limit(1)->find();
            if($sdata['byname']=='用户升级'){
                $sdata['原始用户级别']=$sdata['old_lv'];
                $sdata['用户级别']=$sdata['升级数据'];
                
                $sdata['原始商务中心级别']=$smess['原始商务中心级别'];
                $sdata['商务中心级别']=$smess['商务中心级别'];
            }
            
            if($sdata['byname']=='申请商务中心'){
                $sdata['商务中心级别']=$smess['商务中心级别'];
                $sdata['用户级别']=$smess['用户级别'];
                $sdata['原始商务中心级别']=$smess['原始商务中心级别'];
                $sdata['原始用户级别']=$smess['原始用户级别'];
                if($option['tjr']!='')M('用户')->where("编号='".$udata['编号']."'")->setField("服务中心推荐人",$option['tjr']);
            }
            if($sdata['byname']=='设置商务中心'){
			...
			if($confirm)
			{
				...
				M('lvlog')->add($lvlog);
	            M('用户')->save($upudata);
				if(!isset($sdata['回填']) || $sdata['回填'] !='1'){
					$this->runconfirm($udata,$sdata,$product);
				}else{
					//回填进网
					$addcons=$this->getcon("addval",array("from"=>"","to"=>"","now"=>1,'set'=>0),true);
					foreach($addcons as $addcon){
						if($addcon['set'] == 1){
							$netplace = X('*@'.$addcon['to']);
							$netplace->event_valadd($udata,'',$addcon);
						}
					}
				}
			}
		...
		/*
			udata   完整用户信息
			upudata 用于更新用户信息（进行局部数据更新，避免发生更新冲突）
			sdata   用于插入订单的信息
			productdata 用于产品的信息
		*/
		public function getUpdata($option=array()){
			...
			if($this->lockMe && $this->user !='admin'){
				$udata = M("用户")->where(array("编号"=>$_SESSION[C('USER_AUTH_NUM')]))->find();
			}else{
				$udata = M('用户')->where(array('编号'=>$userid))->find();
			}
			$confirm = isset($option['confirm']) ? $option['confirm'] : $this->confirm;
			$sdata=array();
		...
		public function getSaleMoneys($option)
		{
			$ret=array();
			$userid = trim($option['userid']);
			$udata = M('用户')->where("编号='$userid'")->find();
