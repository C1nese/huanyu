	...
	class user extends stru
	{
		...
		public function getnewid()
		{
			...
			if($this->idRand && $this->idLength){
				$result = M('用户')->lock(true)->where(array('编号'=>$ret))->find();
				if($result){
        ...
        public function event_sysclear()
		{
			...
			$model->execute('truncate table `dms_用户`');
		...
		/*
		* 包含网体下级的删除 找出网体下级的用户  循环删除，中间如果有用户未被删除，那么最终执行是一部分被删除
		...
		*/
		public function down_delete($userid){
			...
			$deluname=array();
			//判断用户是否在系统中
			$user = M("用户")->where(array("id"=>$userid))->find();
			if(!$user){
				return "系统不存在";
			}
			...
			foreach(X("net_*") as $net){
				...
				//找出net_place和net_rec中最下面的用户判断语句
				if(get_class($net)=="net_place"){
					...
					$brabchwhere.=$brabchwhere!=""?" and ":"";
					$brabchwhere.="编号 not in (select {$net->name}_上级编号 from dms_用户)";
				}
			}
			//最终语句生成
			$sqlwhere=$sqlwhere==""?"1!=1":$sqlwhere;$brabchwhere=$brabchwhere==""?"1!=1":$brabchwhere;
			$sql="select id,编号 from dms_用户 where (".$sqlwhere.") and ".$brabchwhere;
			//生成while循环查找删除
			$checkcontinue=true;
			while($checkcontinue){
				$delusers=array();
				...
				foreach($downusers as $downuser){
					...
					//删除成功记录所删除的用户编号用于最后的返回
					if($result==true){
						$delusers[]=$downuser;
						$deluname[]=$downuser['编号'];
					}
				}
				//判断是否还要继续查找删除  如果没有要删除的下级用户跳出循环
				if(!$downusers){
					$checkcontinue=false;
				}
				if(count($delusers)>0){
		...
		public function delete($userid){
			$userInfo = M("用户")->where(array("id"=>$userid))->lock(true)->find();
			foreach(X('net_rec,net_place') as $net){
				if($net->getdown($userInfo,0,1)){
					return L('该'.$this->byname.'已有下级网体关系，不能删除');
				}
			}
			$userdata= M('用户')->where(array("id"=>$userid))->find();
			if(!$userdata){
				return L('要删除的'.$this->byname.'不存在');
			}
			//服务中心2014-1-25
			if($this->shopWhere){
				$fwuser=M('用户')->where(array("服务中心_上级编号"=>$userdata['编号']))->find();
		...
		public function deleteAllInvalidUser(){
			...
			$users = M('用户')->where(array('状态'=>'无效'))->select();
			...
			foreach($users as $userInfo){
				...
				if(!$havedown){
					...
					M('用户')->where("id = {$userInfo['id']}")->delete();
					$num++;
				}
			}
			return $num;
		...
		public function isRegular($userid){
			$userinfo = M("用户")->where(array("编号"=>$userid))->lock(true)->find();
		...
		public function checkOnly($val,$type)
		{
			...
			if($type == 'mobile')
			{
				...
				$havenum=M('用户')->lock(true)->where(array('移动电话'=>$val))->count();
			...
			if($type == 'idcard')
			{
				$havenum=M('用户')->lock(true)->where(array('证件号码'=>$val))->count();
			...
			if($type == 'bankcard')
			{
				$havenum=M('用户')->lock(true)->where(array('银行卡号'=>$val))->count();
		...
		public function placeallusers($netname='管理',$regionnum=1,$where='',$con='',$endtime){
			...
			$users = M("用户")->where("审核日期<=".$endtime)->order("{$netname}_层数 desc")->getField($rows);
			//统计网体的下级符合条件的人数
			$newUsers=array();
			foreach($users as $user){
				if($user[$netname.'_上级编号']!=''){
					...
					foreach($upids as $upkey=>$upidstr){
						...
						unset($netdata);
						//判断条件 上级和下级同时条件判断 并判断当前的用户是否在上级符合要求的区域内
		...
		/**
			...
			没有上一天的记录 那么把用户表的编号id空点赋值过来
		*/
		public function updatelog($caltime){
			//查询上期的记录  移动本期  如果没有上期的数据那么默认为默认值
			$getresult=M()->table("dms_用户 as user")->join("inner join (select * from dms_prizelog where 时间=".($caltime-86400).") prizelog on prizelog.userid=user.id")->where("审核日期<".($caltime+86400))->field("user.id uid")->select();
			//获取levels相关字段 根据用户信息设置默认值
			$levelstr="";
			...
			if(!empty($getresult)){
				...
				M()->execute($sql);
				//插入上期没有的用户数据
				$ids=M()->table("dms_用户 as user")->join("inner join (select * from dms_prizelog where 时间=".($caltime-86400).") prizelog on prizelog.userid=user.id")->where("审核日期<".($caltime+86400))->getField("user.id,user.id as uid");
				if(isset($ids))
					$ids=implode(',',$ids);
				//保存上期没有用户记录的用户
				$sql="insert into dms_prizelog (userid,编号,时间,空点{$levelstr}) 
					select id,编号,{$caltime},空点{$levelstr} from dms_用户 where 审核日期<({$caltime}+86400) and id not in (".$ids.")
				";
				M()->execute($sql);
			}else{
				//保存上期没有用户记录的用户
				$sql="insert into dms_prizelog (userid,编号,时间,空点{$levelstr}) select id,编号,{$caltime},空点{$levelstr} from dms_用户 where 审核日期<({$caltime}+86400)";
				M()->execute($sql);
			}
		}
		//
		...
		{ 
			M()->execute("update dms_用户 set 编号='{$newbh}' where 编号='{$oldbh}'");
			M()->execute("update dms_用户 set 服务中心_上级编号='{$newbh}' where 服务中心_上级编号='{$oldbh}'");
			M()->execute("update dms_用户 set 注册人编号='{$newbh}' where 注册人编号='{$oldbh}'");
			M()->execute("update dms_用户 set 关联账号='{$newbh}' where 关联账号='{$oldbh}'");
			//订单组
			M()->execute("update dms_报单 set 编号='{$newbh}'         where 编号='{$oldbh}'");
			M()->execute("update dms_报单 set 报单中心编号='{$newbh}' where 报单中心编号='{$oldbh}'");
