<?php
	class sale_buy extends sale{
		...
		public function buy($data = array()){
            ...
            $sdata['accbank']="";
			/*付款审核完成*/
            $umess=M('用户')->where("编号='".$sdata['编号']."'")->find();
            $smess=M('报单')->where("编号='".$sdata['编号']."'")->order("id desc")->limit(1)->find();
            $sdata['商务中心级别']=$umess['商务中心级别'];
            $sdata['用户级别']=$umess['用户级别'];
            $sdata['原始商务中心级别']=$smess['原始商务中心级别'];
            $sdata['原始用户级别']=$smess['原始用户级别'];
            $sdata['报单中心编号']=$udata['服务中心_上级编号'];
            if($sdata['报单类别']=='业绩消费')$sdata['报单金额']=$sdata['购物金额'];
			$rs=M('报单')->add($sdata);
			if(!$rs)
			{
				M()->rollback();
			...
			if($this->extra){
				...
				if(count($zlary)>0){
					M('用户')->where(array("id"=>$udata['id']))->save($zlary);
				}
			}
			$sdata['id']=$rs;
		...
		public function getBuyData($data){
			if($this->lockMe && $this->user !='admin'){
				...
				$udata = M('用户')->where(array('编号'=>trim($data['userid'])))->find();
			}
			$sdata=array();
            ...
            if($data['remarks']!='')$sdata["申请备注"]  =$data['remarks'];//此处自己添加
			if($this->extra == true){
		...
		public function getValidate($data_post=array(),$ajax=false){
			...
			if($this->setMoney)
			{
				...
				$ret[] = array('setMoney',array($this,'beis'),L('单数必须为50的倍数'),1,'function');
				if($this->maxMoney!=0){
			...
			if($m->create() === false){
				$error = $m->getError();  //错误信息
					...
					if(isset($data_post['postname']) && strpos($data_post["postname"],'shop') !== false && !isset($errs['shop']))
					{
						...
						$error['shop']=M('用户')->where($where)->getfield('姓名');
					}
			}
