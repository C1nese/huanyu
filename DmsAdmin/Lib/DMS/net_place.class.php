<?php
	class net_place extends net
	{
       ...
       */
       public $pv_denominator=1;
       /*增加业绩值的最小层数
       未来准备去掉最大最小层数
       改为<addval from='...' to='管理' min='2' max='3'/>
       */
       ...
       */
		...
		*/
		...
		public $tleMode ='';
		public $tleDay ='';
		public $_cache=array();//奖金计算时的用户缓存记录的数组
		public $_cachenum=0;//奖金计算时调用getups的次数
		public $_maxnum=10;//默认最多调用十次  启动缓存机制保存所有用户  不在查询数据库
		...
		public function event_user_reg($user)
		{
			//创建索引
			if($this->regDisp){
		...
		public function event_sale_delete($sale)
		{
			...
			if($movetime > 0 && $sale['到款日期'] >= $movetime )
			{
				...
				if(isset($ids))
				{
					...
					{
						//更新结转业绩
						M()->execute('update dms_用户 a left join (select  userid,sum(val) val from dms_'.$this->name.'_业绩 where userid in ('.$ids.') and pid<>0 and time<' . ($movetime) . ' and region=' . ($key+1) . ' group by userid) b 
						...
						//累计业绩
						M()->execute("update dms_用户 a left join (select  userid,sum(val) val from dms_".$this->name."_业绩 where userid in (".$ids.") and pid>0 and val>0 and region=" . ($key+1) . " group by userid) b 
		...
		//注册期间的用户字段的补全
		public function setRegData(&$udata,$data,$isAjax){
			if($this->regDisp)
			{
				$udata[$this->name.'_网体数据'] ='';
				$udata[$this->name.'_上级编号'] ='';
				//默认编号和数据库一致
				if(isset($data["net_".$this->getPos()])){
					$netup=M("用户")->where(array("编号"=>trim($data["net_".$this->getPos()])))->field('编号')->find();
					if($netup) $udata[$this->name.'_上级编号'] =$netup['编号'];
				}
				//判断不是注册第一个人
				if(X("user")->have()){
					//强制选择位置
					if($this->setRegion)
					{
						$Region  = $data["net_".$this->getPos()."_Region"];
						$branch  = $this->getBranch();
						$oldUpId = $udata[$this->name.'_上级编号'];
						//如果开启滑落
						$upuserid=M("用户")->where(array("编号"=>$udata[$this->name.'_上级编号']))->getField('id');
						if($this->backFall && $udata[$this->name.'_上级编号'] != '' && $upuserid)
						{
							while(true)
							{
								//找到上级
								$upuser = M('用户','dms_')->where(array('编号'=>$udata[$this->name.'_上级编号']))->find();
								if($upuser)
								{
									//上级特定区不为空的情况
									if($upuser[$this->name.'_'.$Region."区"]!='')
									{
										//向特定区进行滑落
										$udata[$this->name.'_上级编号']=$upuser[$this->name.'_'.$Region."区"];
									}
									else
									{
										//特定区为空,取得所有区,进行对比,看当前滑落区是否为左区
										if($branch[0] == $Region || $oldUpId == $udata[$this->name.'_上级编号'])
										{
											//如果是左区,则跳出循环
											break;
										}
						...
						$udata[$this->name.'_位置'] = $Region;
					}
					else
					{
						//自动选择位置
						$udata[$this->name.'_位置'] = $this->getNullRegion($udata[$this->name.'_上级编号']);
					}
				}
				if(!isset($udata[$this->name.'_上级编号']) || $udata[$this->name.'_上级编号']==NULL)
					$udata[$this->name.'_上级编号']='';
				if(!isset($udata[$this->name.'_位置']) || $udata[$this->name.'_位置']==NULL)
					$udata[$this->name.'_位置']='';
				//设置默认层数
				$udata[$this->name.'_层数']=1;
				if($udata[$this->name.'_上级编号']!='')
				{
					$upuser = M('用户')->where(array('编号'=>$udata[$this->name.'_上级编号']))->find();
					$udata[$this->name.'_网体数据'] = trim($upuser[$this->name.'_网体数据'].",".$upuser["id"].'-'.$udata[$this->name.'_位置'],',');
					$udata[$this->name.'_层数'] = $upuser[$this->name.'_层数'] + 1;
					if(!$isAjax)
					{
						$data=array();
						$data[$this->name."_".$udata[$this->name.'_位置'].'区'] = $udata['编号'];
						M('用户')->where("编号='".$udata[$this->name.'_上级编号']."'")->save($data);
					}
				}
			}
			else
			{
				$udata[$this->name.'_上级编号']='';
				$udata[$this->name.'_位置']='';
				$udata[$this->name.'_层数']=0;
				$udata[$this->name.'_网体数据'] ='';
			}
			foreach($this->getBranch() as $Branch)
			{
				$udata[$this->name.'_'.$Branch."区"]='';
			}
		}
		/**
		 //返回指定用户是否有空缺的线
		...
		public function getNullRegion($name)
		{
			...
			$user=M('用户','dms_')->where(array('编号'=>$name))->find();
		...
		*/
		public function findUp($ifuser,$thisuser=false,$exp='')
		{
			...
			foreach(explode(",",$exp) as $set)
			{
				...
				{
					$ifuser = M('用户')->where(array('编号'=>$ret[0]))->find();
		...
		public function autoSetUp(&$user)
		{
			//需要测试引用变量是否会导致排网的问题
			//原始点判定
			if(!M('用户','dms_')->lock(true)->where(array($this->name.'_层数'=>1))->find())
			{
				$this->set_index($user);
				return ;
			}
			...
			M('用户')->where(array('id'=>$user['id']))->save($userdata);
			return;
		}
		...
		public function addpv($user,$val,$option)
		{
			...
			/*业绩表字段说明
			userid业绩所属用户ID.表示这个业绩记录属于某要一个用户
			...
			删除用户的规则,根据(userid=被删除人id and pid=0)得到这个人的原始记录.在根据这些记录的ID进行其他记录的pid查询
			得到关联产生的业绩记录.并进行删除
			...
			条件的涵义：'产生业绩用户所在的位置（相对于第一个上级用户）,对上累计的最小层数,对上累计的最大层数'
			...
			*/
			...
			if(isset($option["min"]) && $option["min"]=='true')
			{
				...
				M('用户')->where(array('id'=>$user['id']))->save(array(
				...
				/*函数内的参数定义 自身业绩表中的id,业绩数值,对上累计的网体数据,业绩来源用户id,订单id,时间,不对上累计的条件,用户的网体位置*/
		...
		public function getdown($user, $minlayer = 0, $maxlayer = 0, $where = '',$fromprize=false,$haveme = false){
			$m_user=D("user");
			//判断用户是否是第一个用户 如果是第一个用户的话 应该是'id-%' 如果不是的话应该是'%,id-%'
		...
		public function set_groupnum($user,$num,$verify)
		{
			...
			if($verify==1||$verify==2){
				$sqlstr="update dms_用户 set ".$this->name."_团队人数  =".$this->name."_团队人数  + ".$num." where id in(".$ids.")";
				M()->execute($sqlstr);
			}
			if($verify==0||$verify==2){
				$sqlstr="update dms_用户 set ".$this->name."_团队总人数=".$this->name."_团队总人数 + ".$num." where id in(".$ids.")";
				M()->execute($sqlstr);
			}
		}
		...
		public function set_Depth(&$user){
			...
			if($user[$this->name."_网体数据"] != ''){
				...
				foreach($datas as $data){
					$res = explode('-',$data);
					$sql="update user set {$this->name}_{$res[1]}区层深={$thislayer}-{$this->name}_层数 where {$this->name}_{$res[1]}区层深<{$thislayer}-{$this->name}_层数 and id = {$res[0]}";
					M()->execute($sql);
				}
			}
		}
		...
		public function set_index(&$user,$update=true)
		{
			...
			{
				$upuser = M('用户')->where(array('编号'=>$user[$this->name.'_上级编号']))->find();;
		...
		{
			$user=M('用户','dms_')->lock(true)->where(array("编号"=>$username))->find();
		...
		{
            $user=M('用户','dms_')->lock(true)->where(array('编号'=>$userid))->find();
			$ableregion=array();
			foreach($this->getRegion() as $region)
			{
				if($user[$this->name.'_'.$region['name'].'区'] =='')
		...
		//最左区判定
		public function isInLeftbak($placename,$recname,$regionName)
		{
			$rec=M('用户','dms_')->lock(true)->where("编号='".$recname."'")->find();
			$recid=$rec['id'];
			$Branch=$this->getBranch();
			...
			if($recname==$placename){
			  $recplace=M('用户','dms_')->lock(true)->where($this->name."_上级编号='".$recname."'")->find();
			  if($recplace){
				  return false;
			...
			$placenet=M('用户','dms_')->lock(true)->where("编号='".$placename."'")->getField($this->name.'_网体数据');
			if($placenet=="")
			{
				return true;
			}else{
		...
		{
			//得到推荐人编号
			$recuser   = M('用户','dms_')->lock(true)->where("编号='".$recname."'")->find();
			$placeuser = M('用户','dms_')->lock(true)->where("编号='".$placename."'")->find();

			if($recuser['id'] == $placeuser['id'])
			{
				return true;
			}
			$netdata=M('用户','dms_')->lock(true)->where(array('id'=>$placeuser['id']))->getField($this->name.'_网体数据');
			//推荐人和安置人不符，并且安置人为原始点
			if($netdata=='')
			{
				return false;
		...
		public function placeLock($netname,$recname){
			$placenet=M('用户','dms_')->lock(true)->where(array("编号"=>$netname))->getField($this->name."_网体数据");
			$recid=M('用户','dms_')->lock(true)->where(array("编号"=>$recname))->getField("id");
		...
		public function event_userdelete($user)
		{
			...
			{
				M('用户','dms_')->where(array("编号"=>$user[$this->name."_上级编号"]))->save(array($this->name."_".$user[$this->name."_位置"]."区"=>''));
				if($user['状态']=='有效')
				{
					//更新总数和已审核
					$this->set_groupnum($user,-1,2);
		...
		public function event_scal()
		{
	        ...
	        $this->cache = M('用户')->lock(true)->getField('id keyid,id,编号,'.implode(array_keys($rows),','));
		}
		public function event_cal($tle,$caltime)
		{
			...
			if($this->getTleMode() == 's')
			{
		        ...
		        foreach($this -> getBranch() as $key => $Branch)
		        {
		        	$lstr=$this->name.'_'.$Branch.'区';
		        	//本期业绩连表
		        	$user_m->join('(select userid,sum(val) '.$lstr.'本日业绩 from dms_'.$this->name.'_业绩 where time>='.$caltime.' and time<'.$caltime.'+86400 and region='.($key+1).' and pid>0 group by userid) new'.$key.' on dms_用户.id = new'.$key.'.userid');
		        	//结转业绩连表
					$user_m->join('(select userid,sum(val) '.$lstr.'结转业绩 from dms_'.$this->name.'_业绩 where time<'.$caltime.' and region='.($key+1).' and pid<>0 group by userid) jie'.$key.' on dms_用户.id = jie'.$key.'.userid');
					//累计业绩连表
					$user_m->join('(select userid,sum(val) '.$lstr.'累计业绩 from dms_'.$this->name.'_业绩 where time<'.$caltime.'+86400 and region='.($key+1).' and pid>0 group by userid) sum'.$key.' on dms_用户.id = sum'.$key.'.userid');
		...
		public function event_caldayover($caltime)
		{
			...
			if($this->getTleMode() == 'd')
			{
		        foreach($this -> getBranch() as $key => $Branch){
		        	//结转等于结转+当天业绩加当天扣除
		            M() -> execute('update dms_用户 a inner join (select  userid,sum(val) val from dms_' . $this -> name . '_业绩 where pid<>0 and time>=' . ($caltime ) . ' and time<' . ($caltime + 86400) .' and region=' . ($key + 1) . ' group by userid) b
						on a.id=b.userid set a.' . $this -> name . '_' . $Branch . '区结转业绩=a.' . $this -> name . '_' . $Branch . '区结转业绩+ifnull(b.val,0) where ifnull(b.val,0)<>0');
					//dump(M()->_sql());
		            M() -> execute("update dms_用户 a left join (select  userid,sum(val) val from dms_" . $this -> name . "_业绩 where pid<>0 and time>=" . ($caltime + 86400) . " and region=" . ($key + 1) . " group by userid) b
	        ...
	        if($this->getTleMode() == 'w' && date('N', $caltime) == (int)$this -> tleDay)
	        {
	        	foreach($this -> getBranch() as $key => $Branch){
		        	//结转等于结转+当周新增-当周扣除
		            M() -> execute('update dms_用户 a inner join (select  userid,sum(val) val from dms_' . $this -> name . '_业绩 where pid<>0 and time>=' . ($caltime - 86400*6 ) . ' and time<' . ($caltime + 86400) .' and region=' . ($key + 1) . ' group by userid) b
						on a.id=b.userid set a.' . $this -> name . '_' . $Branch . '区结转业绩=a.' . $this -> name . '_' . $Branch . '区结转业绩+ifnull(b.val,0) where ifnull(b.val,0)<>0');
					//更新本期业绩
		            M() -> execute("update dms_用户 a left join (select  userid,sum(val) val from dms_" . $this -> name . "_业绩 where pid<>0 and time>=" . ($caltime + 86400) . " and region=" . ($key + 1) . " group by userid) b
						on a.id=b.userid set a." . $this -> name . '_' . $Branch . "区本期业绩=ifnull(b.val,0), a." . $this -> name . '_' . $Branch . "区本日业绩=ifnull(b.val,0) where a." . $this -> name . '_' . $Branch . "区本期业绩>0 and a." . $this -> name . '_' . $Branch . "区本期业绩<>ifnull(b.val,0) ");
		...
		public function repairPv($datetime)
		{
			...
			$yjs = M('用户')->lock(true)->getField('id'.$rowstr);
			foreach($this->getBranch() as $Branchkey=>$Branch)
			{
		    	...
		    	if($ids)
		    	{
		    		M()->execute('update dms_用户 set '.$this->name.'_'.$Branch.'区结转业绩 = case id '.$upsql.' END where id in ('.trim($ids,',').')');
		    	}
		    	//处理新增业绩===========================
				$upsql='';$ids='';
				$vals = M($this->name.'_业绩')->lock(true)->where("pid<>0 and time>=" . ($datetime) . " and region=" . ($Branchkey+1))->group('userid')->getField('userid,sum(val) val');
		    	...
		    	if($ids)
		    	{
		    		...
		    		if($this->getTleMode()!='s')
		    		M()->execute('update dms_用户 set '.$this->name.'_'.$Branch.'区本期业绩 = case id '.$upsql.' END where id in ('.trim($ids,',').')');
		    		M()->execute('update dms_用户 set '.$this->name.'_'.$Branch.'区本日业绩 = case id '.$upsql.' END where id in ('.trim($ids,',').')');
		    	}
				//更新累计业绩
				$upsql='';$ids='';
		    	...
		    	if($ids)
		    	{
		    		...
		    		M()->execute('update dms_用户 set '.$this->name.'_'.$Branch.'区累计业绩 = case id '.$upsql.' END where id in ('.trim($ids,',').')');
		    	}
			}
		}
		...
		{
			//打开$userbh用户，循环所有区，如果指定的这个$qu前边有空位返回false
			$branch  = $this->getBranch();
        	//查询出所有的用户的管理上级编号为$userbh的记录
			$results=M('用户','dms_')->lock(true)->where(array($this->name.'_上级编号'=>$userbh))->select();
			if($results){
		...
		{
			//选出第一个用户
			$first=M('用户')->where(array("状态"=>'有效'))->order("id asc")->field('id,编号')->find();
			if($first['编号']==$user['编号']){
				return "第一人不能进行移动";
			}
			//如果新要求与目前网络体系一致则直接返回
			...
			//减团队人数
			M('用户')->where(array('id'=>array('in',$oldids)))->setDec($this->name.'_团队总人数',$allnum);
			M('用户')->where(array('id'=>array('in',$oldids)))->setDec($this->name.'_团队人数'  ,$comfrimnum);
			//增加团队人数
			M('用户')->where(array('id'=>array('in',$newids)))->setInc($this->name.'_团队总人数',$allnum);
			M('用户')->where(array('id'=>array('in',$newids)))->setInc($this->name.'_团队人数'  ,$comfrimnum);
			//业绩处理==============================================================
			$movetime=0;
			//日结的处理方式
			if($this->getTleMode() == 'd') 
			...
			//=======================================================================
			//清空原上级的对应区的编号
			M('用户')->where(array("编号"=>$user[$this->name."_上级编号"]))->save(array($this->name.'_'.$user[$this->name.'_位置'].'区' =>''));
			//更新新上级对应区编号
			M('用户')->where(array("编号"=>$newup['编号']))->save(array($this->name.'_'.$region.'区' =>$user['编号']));
			//对移网用户本身的上级信息更新
			M('用户')->where(array("编号"=>$user['编号']))->save(array( $this->name . '_上级编号' => $newup['编号'],$this->name .'_位置' => $region));
			//生成新网体数据
	        ...
	        $where .= " or (编号='{$user['编号']}')";
			M('用户')->where($where)->setInc($this->name.'_层数',$layerdiff);
			//对老网体数据进行更新
			$down1=0;//是否移动第一个人的第一层用户
			foreach($this->getRegion() as $Region){
				$Regionname=$first['id'].'-'.$Region['name'];//其中$first['id']直接用的move下面第一行的获取（即得到公司网体的第一个用户）
				if($user[$this->name.'_网体数据']==$Regionname){
					//首先处理这个特殊用户
					M()->execute("update `dms_用户` set {$this->name}_网体数据='{$newnet}' where 编号='".$user['编号']."'");
					$weizhi=strlen($Regionname)+1;//得到逗号首次出现的位置
					//得到移动的用户网体下数据
					$downusers  = M('用户')->where("find_in_set('".$user[$this->name.'_网体数据']."',".$this->name."_网体数据)")->getField("编号,".$this->name."_网体数据");
					//进行循环
					foreach($downusers as $downbianhao=>$downuser){
						//为合成用户网体做准备,得到后半部分网体数据
						$downuserh=mb_substr($downuser,$weizhi);
						//对移动过的用户合成新网体
						$downusernewnet=$newnet.','.$downuserh;
						//对移动过的用户网体数据进行更新
						M()->execute("update `dms_用户` set {$this->name}_网体数据='{$downusernewnet}' where 编号='".$downbianhao."'");
					}
			...
			if($down1==0){
				M()->execute("update `dms_用户` set ".$this->name."_网体数据=replace(".$this->name."_网体数据,'".$user[$this->name.'_网体数据']."','".$newnet."')");
			}
			//对新增业绩在新网体中进行还原
			if($movetime>0 && $ids)
			{
				//找到所有需要增加业绩的记录,并附带网体数据
				$adds = M()->table('dms_'.$this->name.'_业绩 a')->join('dms_用户 b on b.id=a.userid')->field('b.'.$this->name.'_网体数据 netdata,b.'.$this->name.'_位置,b.id uid,a.val,a.id,a.saleid,a.time')->where('pid=0 and `time` >='.$movetime)->select();
				foreach($adds as $add)
				{
					//获取不进业绩的条件节点
					$salename=M("报单")->where(array("id"=>$add['saleid']))->getfield("报单类别");
					$cons=X("@".$salename)->getcon("addval",array("from"=>"","to"=>"","now"=>1),true);
					foreach($cons as $con){
				...
				{
					//更新结转业绩
					M()->execute('update dms_用户 a left join (select  userid,sum(val) val from dms_'.$this->name.'_业绩 where pid<>0 and time<' . ($movetime) . ' and region=' . ($key+1) . ' group by userid) b 
					...
					//更新累计业绩
					M()->execute("update dms_用户 a left join (select  userid,sum(val) val from dms_".$this->name."_业绩 where pid>0 and val>0 and region=" . ($key+1) . " group by userid) b 
					on a.id=b.userid set a.".$this->name.'_'.$Branch."区累计业绩=ifnull(b.val,0)");
				}
			}
			foreach(X("fun_placenum") as $funrec){
				$funrec->event_netmove($this,$user,$movetime);
			}
			//保存网体修改日志
		...
		*/
		public function repairTable()
		{
			...
			{
				if(M('用户')->where($this->name.'_'.$region.'区本期业绩>0')->find())
				{
					throw_exception('目前有用户'.$this->name.'_'.$region.'区本期业绩大于0,请将本期业绩更新到结转后在执行，
					参考SQL:update dms_用户 set '.$this->name.'_'.$region.'区结转业绩='.$this->name.'_'.$region.'区结转业绩+'.$this->name.'_'.$region.'区本期业绩,'.$this->name.'_'.$region.'区本期业绩=0');
				}
			}
		...
		public function repair()
		{
	   	    ...
	   	    M()->execute("update dms_用户 a,dms_用户 b set a.".$this->name."_上级编号=b.编号 where a.".$this->name."_上级编号=b.编号");
	   	    //清空现有网体数据信息
	   	    M()->execute("update dms_用户 set {$this->name}_网体数据=''");
	   	    M()->execute("update dms_用户 set {$this->name}_层数=1 where {$this->name}_上级编号=''");
	   	    //取得需要用的信息表
	   	    $userdata=M('用户')->getField("编号,{$this->name}_上级编号 上级编号,id,{$this->name}_位置 位置,状态,0 num,0 allnum");
	   	    //取得要处理用户的信息表
	   	    $upusers  = M('用户')->getField('id,编号');
	   	    //更新网体数据临时SQL
	   	    $netsql   = '';
	   	    //更新层数临时SQL
	   	    $layersql = '';
		...
		{ 
			M()->execute("update dms_用户 set {$this->name}_上级编号='{$newbh}' where {$this->name}_上级编号='{$oldbh}'");
			foreach($this->getBranch() as $key=>$Branch)
			{
				M()->execute("update dms_用户 set {$this->name}_{$Branch}区='{$newbh}' where {$this->name}_{$Branch}区='{$oldbh}'");
			}
		}
		/*
		* 根据订单表重新进业绩 删除用户表以及业绩表中的信息
		...
		*/
		public function repairupv(){
			...
			foreach($sales as $sale){
				...
				//找出订单的用户
				$user=M("用户")->where(array("编号"=>$sale['编号']))->find();
				if($user){
					//获取sale节点的信息
					...
					foreach($cons as $con){
						if($con['to']==$this->name && !isset($con['set'])){
							//首先执行特殊定制程序
							$sql="update dms_用户 d inner join(select a.推荐_上级编号 编号,count(1) val  from `dms_用户` a inner join dms_用户 ck on a.推荐_上级编号=ck.编号 where a.审核日期>0 and a.审核日期<".$sale['到款日期']." group by a.推荐_上级编号) b on d.编号=b.编号 set 业绩=1 where d.业绩=0 and ifnull(b.val,0)>=2";
							M()->execute($sql);
							//执行业绩进网方法
