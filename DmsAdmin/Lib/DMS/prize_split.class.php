<?php
	...
	{
		//SELECT 创业组_盘号,创业组_序号,创业组_直推,创业组_间推,创业组_总推 FROM `dms_用户`;
		//产生奖金模式
		public $prizeMode = -1;
		//来源表条件
		public $where = '';
		...
		function make()
		{
			$panid = M('用户')->where($this->name . '_盘号>0')->Max($this->name . '_盘号');
			if($panid == null)
				$panid=0;
			return $panid +1;
		}
		...
		public function giverec($user,$fromuser,$val)
		{
			...
			M('用户','dms_')->where(array('id'=>$user['id']))->save($uparr);
		...
		public function makecom($name,$user,$formuser,$val,$alluser)
		{
			...
			if($this->logid==0)
			{
				...
				$recdata = M($this->name.'rec')->where("编号 in (select 编号 from dms_用户 where ".$this->name."_盘号=".$user[$this->name.'_盘号'].")")->select();
