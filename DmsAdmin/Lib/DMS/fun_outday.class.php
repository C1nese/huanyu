<?php
	/*
	  剩余时间模块.用于计算客户的到期时间
	  <_con day='10' where="[用户级别]=1"/>
	  如以上设置则表示用户在审核后10天内计数
	  如以上设置表示用户累计收入大于1000时在原基础上在增加15天
	...
	*/
	class fun_outday extends stru
	{
		...
		public function event_valadd($user,$val,$option)
		{
			...
			if($syday<0)
				$syday = 0;
			M('用户')->where(array('id'=>$user['id']))->save(array($this->name=>$user[$this->name],$this->name.'剩余'=>$syday));
		}
		function event_cal($tle,$caltime)
		{
			...
			*/
			M()->execute("update dms_用户 set ".$this->name."剩余 = DATEDIFF(from_unixtime($this->name),from_unixtime(".$caltime.")) where 状态='有效'");
			//对剩余天数小于0的归零
			M()->execute("update dms_用户 set ".$this->name."剩余 = 0 where ".$this->name."剩余 < 0");
			/*
			...
			*/
			M()->execute("update dms_用户 set ".$this->name."剩余 = 0,".$this->name." = 0 where 状态<>'有效'");
		}
	}
?>