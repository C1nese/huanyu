<?php
	...
	class fun_gold extends stru
	{
		...
		*/
		...
		{
			M("用户")->where(array("编号"=>$user['编号']))->save(array($this->name."信誉"=>$this->creditNum));
		}
		...
		tel     电话
    	显示所有的交易记录和各种查询
		仲裁管理：用户交易时遇到问题，可以选择是否需要公司协助处理，处理之后可以选择是否扣特定数量信誉评级或直接封权限。
		杂项
		用户有额外成交数量和成交总额的累计。用于在信誉评级之外作为参考
		管理员可以设置  信誉用户 以及授权额信誉用户在购买其他用户出售的货币时，货币可以直接到账并使用，然后在由信誉用户付款，一般服务中心可以设置为信誉用户。信誉用户的未确认购买交易总额不能超过其授权额。
		*/
		...
		*/
		public function accokTrad($user,$buyinfo){
			...
			M("用户")->where(array("编号"=>$buyinfo['编号']))->setInc($this->name."累计购买",$buyinfo['数量']);
			$sellinfo=M($this->name."挂单")->lock(true)->where(array("id"=>$buyinfo['pid']))->find();
			//更新挂单数据
			$selldata=array(
				"成交中数量"=>$sellinfo['成交中数量']-$buyinfo['数量'],
				"已成交数量"=>$sellinfo['已成交数量']+$num
			);
			if($selldata["成交中数量"]==0 && $sellinfo['未成交数量']==0){
				$selldata["状态"]="完成";
			}
			M($this->name."挂单")->where(array("id"=>$sellinfo['id']))->save($selldata);
			M("用户")->where(array("编号"=>$sellinfo['编号']))->setInc($this->name."累计出售",$buyinfo['数量']);
			return true;
		}
		/*
		** 撤销挂单
		...
		*/
		public function cancelSell($sellid){
			...
			bankset($this->bankName,$sellinfo['编号'],$backtax,$this->name."卖家撤销","卖家撤销".$this->name."返回未出售的手续费".$backtax,$user['编号']);
		...
		*/
		public function arbitrate($username,$buyid,$num,$zccontent){
			//仲裁的用户
			if($username!=""){
				$user=M("用户")->lock(true)->where(array("编号"=>$username))->find();
				if(!$user){
					return "未找到仲裁用户";
				}
		...
		*/
		function recharge($username,$chargenum){
			...
			M("用户")->where(array("编号"=>$username))->setInc($this->name."信誉",$chargenum);
			return true;
		}
	}
?>