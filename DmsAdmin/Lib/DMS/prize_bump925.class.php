		...
		function cal()
		{
			...
			foreach($users as $user)
			{
				...
				/*
					...
					4.net_place->addUpPv()对上级业绩增加.和用户表本期本日累积增加
					5.*秒结bump对本期业绩碰对
					6.*秒结bump扣减本期业绩字段
					7.入奖金表
					如果为日结算处理
					...
					3.net_place->addUpPv()对上级业绩增加.和用户表本期本日累积增加
					4.*net_place->event_cal();统计所有用户本期和结转业绩保存到用户表
				...
				*/
				...
				foreach($branch as $key)
				{
					...
					else
					{
						if($user[$net->name.'_'.$key.'区结转业绩']<0 || $user[$net->name.'_'.$key.'区本期业绩']<0){
							//业绩有负数产生
							throw_exception("结算".$this->name."时用户【".$user['编号']."】的".$key."区业绩错误");
						}
				...
				foreach($branch as $key => $name)
				{
					if($oldbumpval[$name]>$bumpval[$name])
					{
						...
						if($isscal)
						{
							M('用户')->where(array('id'=>$user['id']))->setDec($net->name.'_'.$name.'区本期业绩',($oldbumpval[$name] - $bumpval[$name]));
						}
					}
				}
			}
			...
			foreach($cons as $con)
			{
				...
				while(true)
				{
					...
					foreach($bumpset as $key=>$val)
					{
					 ...
					 $result += $prizenum;
					//产生奖金构成信息,可详见tle中的make_composition说明
					if(strstr($con["val"],'%')){
