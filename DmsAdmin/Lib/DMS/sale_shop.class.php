<?php
	class sale_shop extends sale
	{
		public $money=0;		//当setmoney为false时的订单金额
		public $minMoney=0;		//最小报单金额
		public $maxMoney=0;		//最大报单金额
		public $extra = true;  //是否填写物流信息
		public $lockMe = true;  //此属性在sale_shop中无用，可以忽略
		...
		public function buy($data = array())
		{
			...
			//清除购物车
			M($this->name.'购物车')->where(array('编号'=>$udata['编号']))->delete();
			//更新用户资料
			if($this->extra){
				$zlary=array();
		...
		public function getBuyData($data){
			...
			$sdata['到款日期']=$confirm ? systemTime():NULL;
			$sdata['报单状态']=$confirm ? "已确认":"未确认";
			$sdata['报单类别']=$this->name;
			$sdata["byname"]      =$this->byname;
			$sdata['订单备注']=isset($data['beizhu'])?$data['beizhu']:'';
			if($this->extra == true){
		...
		public function getValidate($data_post=array(),$ajax=false)
		{
			...
			if($this->setMoney)
			{
				...
				if($this->maxMoney!=0){
					$ret[] = array('setMoney',$this->minMoney.','.$this->maxMoney,'您输入的报单金额必须大于等于'.$this->minMoney.'并小于'.$this->maxMoney.'！',1,'between');
				}else{
			...
			//lock配置中的验证条件
			$this->lockconVerify($ret,$this->getBuyData($data_post));
			$ajax=true;
