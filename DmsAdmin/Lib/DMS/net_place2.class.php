	...
	//		M($this->name,'dms_')->where(array('计算'=>0))->save(array('计算'=>1));
		...
		*/
		...
		public function event_valadd(&$user,$val,$option)
		{
			if($val>50)
			{
				throw_exception("net_place2网体接收到大量进点需求,".$user['编号'].'用户要进'.$val.'个点位，如果此情况是不正确的。请修改addval中的from属性写死为要进点的数量');
			}
			for($i=1;$i<=$val;$i++)
			{
				$this->add($user);
		...
		public function add($user)
		{
			...
			if($this->inrec != '')
			{
				...
				if($newxh == 1)
				{
					...
					$up = $net->getups($user,$minlayer=0,$maxlayer=0,"exists(select * from dms_{$this->name} where dms_{$this->name}.userid=dms_用户.id)");
					//找到推荐人
					if($up)
					{
						$upnode=$up[0];
