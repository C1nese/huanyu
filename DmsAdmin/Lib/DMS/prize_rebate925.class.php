	...
	class prize_rebate extends prize
	{
		...
		function cal()
		{
			...
			if($this->rowFrom == 0)
			{
				...
				if($users){
					//在rowfrom=0且不进行K值的情况下.使用SQL批量的形式执行奖金计算.而不是单个用户循环执行addprize
					//where条件中不要用M
					/*************************************************************/
					if($this->usesql && $this->K==0){
						foreach($cons as $con)
						{
							...
							if(empty($topcons)){
								$sqlstr = "update dms_用户 set ".$this->name."=".$val.",".$this->name."本日=".$this->name."本日+".$val.",".$this->name."本周=".$this->name."本周+".$val.",".$this->name."本月=".$this->name."本月+".$val.",".$this->name."累计=".$this->name."累计+".$val." where 1";
								...
								if($topcon['where']!=''){
									//符合where条件
									$sqlstr = "update dms_用户 set ".$this->name."=if(".$val.$withprize.">".$topcon['val'].",if((".$topcon['val']."-".$withprize.")<0,0,(".$topcon['val']."-".$withprize.")),".$val."),".$this->name."本日=".$this->name."本日+if(".$val.$withprize.">".$topcon['val'].",if((".$topcon['val']."-".$withprize.")<0,0,(".$topcon['val']."-".$withprize.")),".$val."),".$this->name."本周=".$this->name."本周+if(".$val.$withprize.">".$topcon['val'].",if((".$topcon['val']."-".$withprize.")<0,0,(".$topcon['val']."-".$withprize.")),".$val."),".$this->name."本月=".$this->name."本月+if(".$val.$withprize.">".$topcon['val'].",if((".$topcon['val']."-".$withprize.")<0,0,(".$topcon['val']."-".$withprize.")),".$val."),".$this->name."累计=".$this->name."累计+if(".$val.$withprize.">".$topcon['val'].",if((".$topcon['val']."-".$withprize.")<0,0,(".$topcon['val']."-".$withprize.")),".$val.") where 1";
		...
		{			
			...
			{
				$user   =M('用户')->where(array("id"=>$userid))->lockr(true)->find();
			}
			$pri=0;
			foreach($cons as $con)
			{
				...
				//取双方最小级别,则做降级操作
				if($con['minlv'] <= $user[$this->lvName] && $con['maxlv'] >= $user[$this->lvName] && transform($con['where'],array(),$wheredata))
				{
					//得到最终数字
			...
			if($user['用户级别']==80){
				M('用户')->where(array('编号'=>$user['编号']))->setInc('用户天数',1);
			}
			//如果是市场分红，保存期数
			if($this->name=='市场分红' && $pri>0){
				M('用户')->where(array('编号'=>$user['编号']))->setInc('分红期数',1);
			}
			//获取对应级别封顶参数
			if($user['用户级别']==10){
				$cap=CONFIG('sfv');
			...
			if($this->name=='市场分红'){
				$dayt=CONFIG('fb');//保存市场分红天数，封闭期内不计算
				$dec=($this->_caltime+86400-$user['审核日期'])/86400;
				if($dec>$dayt && $user['市场分红累计']<$cap){
					M('用户')->where(array('编号'=>$user['编号']))->setInc('分红天数',1);
				}
				unset($dec);
			}
			unset($user);
