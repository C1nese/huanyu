<?php
	...
	*/
	public static function movetle($tlename,$prizename,$frombh,$tobh,$caltime=null)
	{
		...
		//来源ID
		$fromid=M('用户')->where(array('编号'=>$frombh))->getfield('id');
		if(!$fromid)
		{
			die('来源用户未找到');
		}
		//目标id,用于筛选不符合的记录
		$toid  =M('用户')->where(array('编号'=>$tobh))->getfield('id');
		if(!$toid)
		{
			die('目标用户未找到');
		}
		...
		foreach($memos as $memo)
		{
			...
			$val = $memo['trueval'];
			//找到要扣除的用户
			$kuser = M('用户')->where(array('id'=>$memo['userid']))->find();
			//对原奖金记录对应的奖金扣减
	...
	public static function maketle($tlename,$bh,$caltime)
	{
		...
		if(!$data)
		{
			$user = M('用户')->where(array('编号'=>$bh))->find();
			$data=array();
			$data['计算日期']=$caltime;
			$data['编号']    =$user['编号'];
			//未完待续
