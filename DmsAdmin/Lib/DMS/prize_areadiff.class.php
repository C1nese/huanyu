...
//区域代理级差，支持订单和用户表
	class prize_areadiff extends prize
	{
		...
		public $haveMe = false;
		//朝上返奖的人读取哪个区域
		public $userarea="";// ''用户表的省市区，'代理'用户表的代理省市区,'收货'订单表的收货省市区，'用户代理'表示，如果此用户也是代理，就读代理，否则读本身的
		//拿奖的人读取哪个区域
		public $uparea="代理";// ''|代理,用户表对应的省市区
		public $from=""; //默认以自己的区域朝上返，如果填写报单中心编号，则表示以报单中心的区域朝上返
		...
		function cal()
		{
			...
			if($this->rowFrom==0 && $this->userarea=='收货'){
				throw_exception($this->name.'奖金模块的userarea设置错误,用户表没有【收货】区域选项');
			}
			//5种区域代理数字化
			...
			$fieldsarr=D("user")->get_Property("fields");
			...
			foreach($cons as $con)
			{
				$lvusers=M('用户')->where($this->lvName.'='.$con['lv'])->getField("concat({$areastr[$lvareas[$con['lv']]]}) area,{$fields}");
				if(isset($lvusers))
					$this->lvcache[$con['lv']]=$lvusers;
			}
			//未找到代理用户 结算提示
			if(!$this->lvcache){
				calmsg('未找到'.$this->lvName.'代理用户','/Public/Images/ExtJSicons/bell_error.png');
				return ;
			}
			//从订单获取奖金来源
			if($this->rowFrom == 1)
		...
		public function calculate(&$from,$userid,$sale=null,&$cons,$areaary,$realcon,$lvareas)
		{
			if($this->rowFrom == 0){
				...
				$user   =M('用户')->find((int)$userid);
			}
			$curuser=$user;
			//如果直接给上级报单中心
			if($this->from!='') 
				$user=M('用户')->where(array("编号"=>$from[$this->from]))->find();
			if(!$user)
				return;
			$upusers=array();
			//以用户的本身区域或者代理区域
