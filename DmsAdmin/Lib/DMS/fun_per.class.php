	...
	class fun_per extends stru{
		...
		function update($caltime){
			if($caltime==0)
		      ...
		      //更新本日业绩
		      M()->execute("update dms_用户 a left join (select  userid,sum(val) val from dms_".$this->name."_业绩 where  time>=" . ($caltime) . " and time<".($caltime+86400)."  group by userid) b 
					on a.id=b.userid set a.".$this->name."本日 =ifnull(b.val,0)");
		   //统计本周的
		   if($this->week){
		   	  $firstweek=$caltime-3600*24*(date("N",$caltime)-1);
		      //统计本周的业绩
		      M()->execute("update dms_用户 a left join (select  userid,sum(val) val from dms_".$this->name."_业绩 where  time>=".($firstweek)." and time<".($caltime+86400)."  group by userid) b 
					on a.id=b.userid set a.".$this->name."本周 =ifnull(b.val,0)");
		   }
		   //如果开启了月结的开关
		   if($this->month){
		     M()->execute("update dms_用户 a left join (select  userid,sum(val) val from dms_".$this->name."_业绩 where  FROM_UNIXTIME(time,'%Y%m')='" . date('Ym',$caltime) . "'  group by userid) b 
		   ...
		   if($this->year){
		      ...
		      //统计本周的业绩
		      M()->execute("update dms_用户 a left join (select  userid,sum(val) val from dms_".$this->name."_业绩 where   time>=".($firstyear)." and time<".($caltime+86400)."  group by userid) b 
					on a.id=b.userid set a.".$this->name."本年 =ifnull(b.val,0)");
		   }
		   //如果开启了累计的开关
		    M()->execute("update dms_用户 a left join (select  userid,sum(val) vals from dms_".$this->name."_业绩 group by userid) b 
