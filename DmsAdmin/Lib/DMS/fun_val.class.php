<?php
	class fun_val extends stru
	{
				...
				if(preg_match("/\{([^,]+),(.*)\}/U",$con['val'],$exp))
				{
					...
					switch($exp[1])
					{
						...
						if(is_numeric($con1))
						{
							...
							$sql .= " left join ({$tmpstr}) b on b.编号=dms_用户.编号";
						...
						list($net,$con1) = explode(',',$exp[2]);
							if($con1=='')
								$con1='1=1';
							//$tmpstr="select {$net}_上级编号 编号,count(1) val  from `dms_用户` where {$con1} group by {$net}_上级编号";
							$tmpstr="select a.{$net}_上级编号 编号,count(1) val  from `dms_用户` a inner join dms_用户 ck on a.{$net}_上级编号=ck.编号 where {$con1} group by a.{$net}_上级编号";
						...
						case 'allsum':
							...
							$data=M('用户')->where($where)->getField('id,'.$net.'_网体数据');
							$update=array();
							...
							{
								if($addstate)
									M('用户')->where(array('id'=>array('in',$data['id'])))->setInc($this->name,$data['val']);
								else
									M('用户')->where(array('id'=>array('in',$data['id'])))->save(array($this->name=>$data['val']));
							}
							continue 2;
						break;
					}
		...
		public function event_valadd($user,$val,$option)
		{
			...
			M("用户")->where(array("id"=>$user['id']))->save($data);
			//$this->update();
		}
	}
?>