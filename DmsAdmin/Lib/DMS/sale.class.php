<?php
	class sale extends stru
	{
		...
		// 产品金额整数倍必须为多少的整数倍
		public $productMoneyInt = 0;
		// 产品验证模式 0不验证 1必须与报单金额相等 2>=报单金额
		public $productMoneyMode = 2; 
		// 产品使用的价格
		public $productMoney = '价格';
		// 是否增加产品PV
		public $productPV = false;
		//有产品的情况下验证pv还是金额，默认空验证金额，输入pv验证pv
		public $productCheck='';
		// 显示菜单条件
		public $dispWhere = '';
		...
		public function fromVerify(&$ret,$data_post=array()){
		    ...
		    if($this->fromNoName !='') {
				...
				if(X('user')->have('')){
					//根据店铺编号取得用户
				    $shopinfo=M("用户")->where(array("编号"=>trim($data_post['shop'])))->find();
				    if(!$shopinfo)
				    {
                        //如果没有店铺存在,则提示
		      			$ret[]   = array('shop','',$this->fromNoName.'不存在',2);
					}
					else if(!transform($this->fromNoWhere,$shopinfo))
			        {
                        //如果店铺不符合条件,则提示
		...
		public function lockconVerify(&$ret,$data){
			/*
			  根据订单表中传进来的data数组,得到的用户表记录,以及订单表记录
			  对两个记录进行了合并,用于transform判断
			*/
			if(!isset($data['udata']['编号']))
				return true;
			$datas = array_merge($data['udata'],$data['sdata']);
			$lockcons = $this->getcon('lock',array('where'=>'','msg'=>'','state'=>'lockcon'));
			foreach($lockcons as $lockcon){
				$result = transform($lockcon['where'],$datas);
				if($result){
					//判断成功,state配置项作为要显示器判定内容的表单名称.做提示输出.
					$lockcon['msg'] = transform($lockcon['msg'],$datas,array(),false);
					$data[$lockcon['state']]='';
					if($lockcon['state']=='lockcon' && !isset($_POST['lockcon']))$_POST['lockcon']='lockcon';
					$ret[] = array($lockcon['state'],array($this,'returnFalse'),$lockcon['msg'],1,'function');
		...
		public function getFromInfo(&$sdata,&$data,$user=array()){
			...
			if($this->fromNoName !='' && isset($data['shop'])){
				...
				{
					//默认编号和数据库一致
					$shopuser=M('用户')->where(array('编号'=>trim($data['shop'])))->field('编号')->find();
					if($shopuser){
						$sdata["报单中心编号"]     = $shopuser['编号'];
					}
				}
				...
				{
					//不需要填并且有默认字段的情况下
					$curuser=M("用户")->where(array('编号'=>$_SESSION[C('USER_AUTH_NUM')]))->find();
					if($curuser && $this->fromNo!='' && $this->fromNoName=='')$sdata["报单中心编号"]  = $curuser[$this->fromNo];
				}
				else
				{
					$sdata["报单中心编号"]     = $user['服务中心_上级编号'];
				}
			}
			return $sdata;
		}
		...
		public function accbank(&$sale,$data=array(),$user,$adminacc=false){
			...
			if($accbank){
				$result=$accbank->accok($sale,$data=array(),$user,$this,$adminacc=false);
			}else{
		...
		public function runconfirm($user,$sale,$product){
			...
			//获取内存设置的值
			if((int)ini_get('memory_limit')<(int)'1500M'){
			...
			PrizeData::commit(systemTime(),true);
		}
		//内部函数.处理数值增加
		public function salerunadd($user,$sale){
			...
			$addcon=$this->getcon("addval",array("from"=>"","to"=>"","now"=>1,"userwhere"=>'',"salewhere"=>''),true);	
			foreach($addcon as $k=>$v)
			{
				$v['saleid']=$sale['id'];
				if(transform($v['userwhere'],$user) && transform($v['salewhere'],$sale))
				{
					if($v['from'] == ""){
						$v['from'] = "1";
					}
					$num=transform($v['from'],$sale);
					//非审核订单
					if($sale['报单状态']!='已确认' && $v['now']!=1){
						$num=0;
					}
					$to=$v['to'];
					if(!is_numeric($num))
					...
					else
					{
						...
						if($toarr[0] == 'shop'){
							...
							if($fromuser != 'admin'){
								$fuser=M("用户")->where(array("编号"=>$fromnum))->find();
		...
		//对其标签下的update和addval进行操作
		/*
			now=1 表示在订单审核后执行，now=0 表示审核直接执行 主要包括空单和空点回填
		*/
		public function runupdate($udata,$sdata){
			$updatecon=$this->getcon("update",array('set'=>'','now'=>1,'where'=>'','to'=>'编号'));
			if(!empty($updatecon)){
				foreach($updatecon as $k=>$v)
				{
					...
					{
						$set = $v['set'];
						$sql = "update dms_用户 set " . $set . " where 编号='".$udata[$v['to']]."'";
						$rs=M()->execute($sql);
						if($rs===false){
		...
		public static function event_commit(){
			...
			foreach($sales as $sale){
				$num=0;
				//获取回填奖金的金额
				$user=M("用户")->where(array("编号"=>$sale['编号']))->field("id,编号,".X("prize_backfill@")->name."结转")->find();
				//判断目前剩下需要回填的金额
				...
				if($sale['已回金额']==$sale['回填金额']){
					...
					$data['报单状态']	= '已结算';
					$data["回填"]	 	= 1;
					$data['到款日期']	= systemTime();
					$data['回填日期']	= systemTime();
					$data['实付款']		= $sale['回填金额'];//订单的实付金额
					$data['已回金额']	= $sale['已回金额'];//已回金额累计
					//修改用户状态
					M("用户")->where(array("编号"=>$sale['编号']))->save(array("空点"=>0));
					//审核订单
					//$sale['报单状态']="未确认";
					$this->accstr="编号";
					$sale=M("报单")->where(array('id'=>$sale['id']))->save($data);
					//审核订单
					$sale=X('sale_*@'.$sale['报单类别']);
					$result=$sale->accok($sale,true);
				}
				//处理回填奖金的结转问题
		...
		public function applyok($saledata,$accbank){
			...
			if($saledata['转正方式']=="回填转正"){
				...
				$this->accstr="编号";
				M("用户")->where(array("编号"=>$saledata['编号']))->save(array("空点"=>0));
				//保存订单的相关回填信息
		...
		public function accok($sdata,$isAdmin=false){
			...
			$sdata['报单状态']='已确认';
            if($sdata['报单类别']=='申请商务中心'){
                $sdata['商务中心级别']=$sdata['升级数据'];
                $sdata['原始商务中心级别']=$sdata['old_lv'];
            }
			$rs=M('报单')->save($sdata);
			if ($rs)
			{ 
				...
				if (get_class($this)=='sale_reg')
				{
					...
					$urs=M('用户')->where(array("编号"=>$udata['编号']))->save($updata);
					if ($urs)
					{
					...
					if(!empty($updata)){
						$uprs=M('用户')->where("编号='".$udata['编号']."'")->save($updata);
						if (!$uprs){
							return "执行".$this->name."更新".X('user')->name.'表数据时错误';
						}
					}
		...
		public function backAccMoney($saledata){
			...
			if($saledata['accbank']=="")
				return ;
			//根据付款的配置退款
			$accbankary=json_decode($saledata['accbank'],true);
			$paymoney=$saledata['实付款'];
			foreach($accbankary as $accbank){
				$money=$saledata['实付款']*$accbank['maxval']/100;
				if($money>$paymoney)
					$money=$paymoney;
				bankset($accbank['name'],$saledata['付款人编号'],$money,"订单删除回退","订单删除回退付款金额".$money,$saledata['编号']);
				$paymoney-=$money;
			}
			return;
		}
