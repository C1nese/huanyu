<?php
	class fun_treenum extends stru
	{
		...
		private function addUpPv($pid,$val,$netdata,$fromid,$saleid,$time){
			...
			if($this->haveMe)
			{
				M()->execute("update dms_用户 set ".$this->name."本日=".$this->name."本日+".$val.",".$weeks.$mouths.$years.$this->name."累计=".$this->name."累计+".$val." where id=".$fromid);
			}
			...
			foreach($t_arrs as $key=>$t_arr)
			{
				...
				if(($this->MinLayer == -1 || $key+1 >= $this->MinLayer) && ($this->MaxLayer == -1 || $key+1 <= $this->MaxLayer)) {
				    ...
				    //对用户的本日本月累计进行增加
				    M()->execute("update dms_用户 set ".$this->name."本日=".$this->name."本日+".$val.",".$weeks.$mouths.$years.$this->name."累计=".$this->name."累计+".$val." where id=".$t_arr);
				}
			}
			//如果SQL数组为空.就退出
			if(!$sql) return;
		...
		public function update($caltime=0){
			...
			//更新本日业绩
			M()->execute("update dms_用户 set ".$this->name."本日=0 where ".$this->name."本日>0");
			M()->execute("update dms_用户 a inner join (select  userid,sum(val) val from dms_".$this->name."_业绩 where ".$haveme." time>=".($caltime)." and time<".($caltime+86400)." group by userid) b 
			...
			if($this->nowWeek){
				...
				M()->execute("update dms_用户 set ".$this->name."本周=0 where ".$this->name."本周>0");
				M()->execute("update dms_用户 a inner join (select  userid,sum(val) val from dms_".$this->name."_业绩 where ".$haveme." time>=".($firstweek)." and time<".($caltime+86400)." group by userid) b 
			...
			if($this->nowMonth){
				...
				M()->execute("update dms_用户 set ".$this->name."本月=0 where ".$this->name."本月>0");
				M()->execute("update dms_用户 a inner join (select  userid,sum(val) val from dms_".$this->name."_业绩 where ".$haveme." time>=".($firstmonth)." and time<".($caltime+86400)." group by userid) b 
			...
			if($this->nowYear){
				...
				M()->execute("update dms_用户 set ".$this->name."本年=0 where ".$this->name."本年>0");
				M()->execute("update dms_用户 a inner join (select  userid,sum(val) val from dms_".$this->name."_业绩 where ".$haveme." time>=".($firstyear)." and time<".($caltime+86400)." group by userid) b 
				on a.id=b.userid set a.".$this->name."本年=ifnull(b.val,0) where a.".$this->name."本年!=ifnull(b.val,0)");
			}
			//更新累计
			M()->execute("update dms_用户 set ".$this->name."累计=0 where ".$this->name."累计>0");
			M()->execute("update dms_用户 a inner join (select  userid,sum(val) val from dms_".$this->name."_业绩 where ".$haveme." time<".($caltime+86400)." group by userid) b 
		...
		{
			$uidsql = $users = M('用户')->where($net->name."_网体数据 like '".($user[$net->name.'_网体数据'].','.$user['id']).",%' or ".$net->name."_上级编号='".$user['编号']."'")->Field('id')->select(false);
			...
			if($ids)
			{
				...
				$adds = M()->table('dms_'.$this->name.'_业绩 a')->join('dms_用户 b on b.id=a.userid')->field('b.'.$net->name.'_网体数据 netdata,b.id uid,a.val,a.id,a.saleid,a.time')->where('a.id in ('.implode(",",$ids).')')->select();
				foreach($adds as $add)
				{
