    ...
    */
    class tle extends stru
    {
        ...
        */
        ...
        {
            //(举例) 删除 用户_销售奖金表,用户_销售奖金构成表,用户_销售奖金总账表
        ...
        function event_rollback($time)
        {
        	...
        	$sql="update dms_用户 u inner join (select * from dms_".$this->name." where 计算日期>=".$time.") p on u.编号=p.编号 set ".$upstr;
        	if($upstr!=""){
        		M()->execute($sql);
        	}
        	//删除销售奖金
        ...
        public function saleeffect($date){
            $startdate=$date;
        ...
        /*
            进行手动结算操作的处理入口
            此方法会被批量用户注册调用.也会被/Admin/TleAction.php调用
        */
        function cal($caltime,$recal=false)
        {
			...
			if(count($prizes)>0  && false)
			{
				...
				foreach($prizes as $prize)
				{
					if($prize->ifrun())
					{
						if($prize->_salestarttime != null && ($prize->_salestarttime<$backarrTime || $backarrTime==0))
						{
							$backarrTime=$prize->_salestarttime;
						}
					}
				}
			}
			*/
            ...
            foreach(X('prize_*',$this) as $prize)
            {
                ...
                if(method_exists($prize,"cal"))
                {
                    ...
                    if(($prize->tleMode!='s' || $recal) && !$prize->split){
                       $prize->cal();
                    }
                }
            }
        ...
        public function save()
        {
            ...
            if($this->k >0 && $this->tleMode == 'd'){
                ...
                $krate_temp = 0;
                if($thisAchievement>0)$krate_temp=(M('用户')->sum($prizesumstr)/$thisAchievement)*100;
                //是否超出了K值
            ...
            foreach($users as $user)
            {
                ...
                foreach(X('net_place') as $net_place){
                	foreach($net_place -> getcon("region", array("name" => "")) as $nameconf){
                		$plstr = $net_place->name."_".$nameconf['name']."区";
                		$tledata[$plstr . "本日业绩"] = $user[$plstr . "本日业绩"];
                		$tledata[$plstr . "累计业绩"] = $user[$plstr . "累计业绩"];
                		$tledata[$plstr . "结转业绩"] = $user[$plstr . "结转业绩"];
                	}
                }
			...
			//补全本次已经产生奖金记录的编号ID对照信息
			foreach($hasuserarr as $key=>$val)
			{
				$justin[$key]=$val['id'];
			}
            //创建奖金表构成
            $m_tlecom = M($this->name."构成");
            //合成具体要添加的记录
            foreach($this->fromdata as $val){
                $val['dataid']=$justin[$val['编号']];
                unset($val['编号']);
                $m_tlecom->bAdd($val);
            }
            $m_tlecom->bUpdate();
            ...
            if($this->secAutoGive && $this->_caltype == 0){
                  //取得未处理奖金记录
                 $tlelist=$m_tle->where("state=1 and 计算日期=".$this->_caltime)->select();
                 //进行发放操作
            ...
            $this->fromdata=array();
            //删除用户临时表
			M()->execute("drop TEMPORARY table if exists dms_user");
            //执行清理过程
        ...
        ** userwhere表示发放奖金的条件 有用户表中的字段做判断，字段需加上"[]"，比如"[用户级别]>2"，运行将字段的值查出 条件判断发放
        ...
        */
        public function inBank($tlelist,$prizename,$addcon,$showname)
        {
            ...
            //查询用户信息以及userwhere条件中的字段$wherestr
            $wherestr="";
            if($addcon['userwhere']!=""){
            ...
            //合成奖金表id和数组KEY的对应表 奖金表中所有用户的编号数组
            ...
            foreach($tlelist as $key=>$val)
            {
            	...
            	if(count($bhs)>1000 || $i==$tlelistcnt){
         			$haveSupsub = M('货币')->lock(true)->where(array('编号'=>array('in',$bhs)))->getField('编号,id,'.$bank->name.' num'.$wherestr);//货币分离
         			$haveSups = $haveSups+$haveSupsub;
         			$bhs=array();
         		}
            }

            //$haveSups = M('用户')->lock(true)->where(array('编号'=>array('in',$bhs)))->getField('编号,id,'.$bank->name.' num'.$wherestr);
            //$haveSups = M('货币')->lock(true)->where(array('编号'=>array('in',$bhs)))->getField('编号,id,'.$bank->name.' num'.$wherestr);//货币分离
            //循环奖金名称数组
            foreach($rowary as $rkey=>$row){
	            ...
	            if($havelog)
	            {
	            	...
	            	foreach($havelog as $key=>$log)
	            	{
	            		...
	            		{
	            			throw_exception($bank->name.'货币记录,与发放奖金记录存在数据错误,货币记录中id为'.$log['id'].'的记录,不是用户'.$tlelist[$tleids[$key]]['编号'].'或者货币记录的日期,早于奖金发放日.此问题可能由于数据导入而产生.如果不存在秒结算,可尝试对货币表中的dataid清零处理');
	            		}
	            	...
	            	if($logset)
	            	{
	            		...
	            		foreach($logset as $key=>$log)
	            		{
	            			...
	            			//如果当前用户的货币记录.不等于其拥有的最大记录.则需要额外做一个更新
                            ...
                            if($bank->name == '换购钱包'){
                                $aPostData = array();
                                $aPostData['username'] = $log["编号"];
                                $aPostData['num'] = $log['val'];
                                $aPostData['note'] = '用户'.$log["编号"].'在环宇天下'.$bankmemo;
                                $aPostData['operate'] = $shownameary[$rkey];
                                $aPostData['type'] = 1; //换购钱包对应慕悦集储值钱包
                                cCurlInit(C('CURRENCY_URL'),$aPostData);
                            }
                            /*btx 同步货币 end*/
	            		}
	            		$m_user->bUpdate();
		         	...
		         	foreach($tleids as $key=>$val)
			      	{
			      		...
			      		if(transform($addcon['userwhere'],$haveSups[$data['编号']])){
                            ...
                            if($bank->name == '换购钱包'){
                                $aPostData = array();
                                $aPostData['username'] = $data["编号"];
                                $aPostData['num'] = $price;
                                $aPostData['note'] = '用户'.$data["编号"].'在环宇天下'.$bankmemo;
                                $aPostData['operate'] = $shownameary[$rkey];
                                $aPostData['type'] = 1;//换购钱包对应慕悦集储值钱包
                                cCurlInit(C('CURRENCY_URL'),$aPostData);
                            }
        ...
        /**
         +----------------------------------------------------------
         * 清零本日.以及本月累计数据
         * 用户表中存在每项奖金的本日,和本月累计数据.需要在结算到特定日期以后.进行清零
         +----------------------------------------------------------
			...
			*/
            ...
            for($i=0;$i<=floor(($caltime-$startTime)/86400);$i++)
            {
                ...
                $thisPrize       += M($this->name)->where(array('计算日期'=> array(array('egt',$modeData['sdate']),array('elt',$modeData['edate']))))->sum('收入');
                 //统计当日用户
                $newuser  = M('用户')->where(array('审核日期'=> array(array('egt',$modeData['sdate']),array('lt',$modeData['edate'])),'状态'=>array('eq','有效')))->count('id');
                //全部用户数量增加当日用户
                $sumUser += $newuser;
                //总业绩增加当日业绩
               ...
               foreach(X('prize_*',$this) as $price)
               {
                ...
                if($m_Ledger->where(array('计算日期'=>array('eq',$iftime)))->lock(true)->find())
                {
		...
		public function event_diffTime($time)
		{
			...
			if($uprow != '')
			{
				M()->execute('update dms_用户 set '.trim($uprow,',').' where '.trim($where,' or '));
			}
            
            
            //循环所有用户更新市场分红天数
            /*$users=M('用户')->select();
            $jr=strtotime(date('Y-m-d',systemtime()));
            foreach($users as $u){
				$dayt=CONFIG('fb');//封闭期内不计算
                $jg=CONFIG('jgd');//间隔期
                $data['分红时间']=$u['分红时间'];
				$dec=($jr-$u['审核日期'])/86400;//判断当前时间-审核时间与封闭期比较
				if($dec>$dayt){//如果不在封闭期内
                    $data['分红天数']=$u['分红天数']+1;//更新分红天数
                    if(($data['分红天数']%$jg)==0){//达到分红条件 更新达到分红条件的时间
                        $data['分红时间']=$jr;
                    }
                    if($data['分红时间']!=0 && ($data['分红天数']%$jg)==0){//此处是防止很多天没结算
                        $data['分红次数']=$u['分红次数']+1;
                    }
					M('用户')->where(array('编号'=>$u['编号']))->save($data);
				}
             }
			unset($dec);*/
		}
		public function make($name,$user,$val,$memo)
		{
            ...
            foreach(X('prize_*',$this) as $prize)
            {
            	if($prize->name==$name)
            	{
            		...
            		$this->save();
            		$this->makeLedger();
            		X('user')->callevent('calover',array('tle'=>$this,'caltime'=>$caltime,'type'=>'scal'));
            	}
            }
		}
