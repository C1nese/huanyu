...
/*
	...
	gettop       处理用户奖金封顶_top配置项
	isrowfrom    判定奖金的来源类型
	ifrun        判断奖金是否可以运行
	             对lockDate属性的特定日期不能执行奖金的功能做实现
	             判定奖金是否符合tlemode的计算周期
	             根据tlemode定义订单表，用户表的额外查询条件。供getuser,getsale使用
	             输出计算提示信息
	             奖金计算前升级处理
	getsale      取得符合计算周期范围内的订单
	getuser      取得符合计算周期范围内的用户
	addprize     产生奖金，并加入缓存，再其内部会调用gettop处理封顶
	prizeUpdate   更新奖金更新用户表，并处理奖金K值
	getSelRow    得到各类配置可能调用到的用户字段
*/
	class prize extends stru
	{
		...
		*/
		public function ifrun()
		{
			...
			switch ($tlemode) 
			{
				...
				case 'r':
					...
					$_day=(int)$_set[0];
                    //推迟天数
                    $_tcday=isset($_set[1])?(int)$_set[1]:0;
                    //条件
					$this->_userWhere =" and MOD((DATEDIFF(from_unixtime($_caltime),from_unixtime(审核日期))-".$_tcday."),$_day)=0";
					$this->_userWhere.=" and DATEDIFF(from_unixtime($_caltime),from_unixtime(审核日期))>=".$_tcday;
					//起始期数
					if(count($_set)>=3)
					{
						$this->_userWhere.=" and floor((DATEDIFF(from_unixtime($_caltime),from_unixtime(审核日期))-".$_tcday.")/".abs($_day).")>=".$_set[2];
					}
					//结束期数
					if(count($_set)>=4)
					{
						$this->_userWhere.=" and floor((DATEDIFF(from_unixtime($_caltime),from_unixtime(审核日期))-".$_tcday.")/".abs($_day).")<=".$_set[3];
					}
					break;
			}
			//处理获取字段中的{uprow}转义
		...
		public function getextent($where='',$rows='*',$table="")
		{
			...
			$ret =  $m_user->where($calwhere)->select();
			if($ret === false)
			{
				throw_exception($this->name.'获取数据失败,sql信息('.htmlentities($m_user->getDbError(),ENT_COMPAT ,'UTF-8').")");
			}
			return $ret;
		}
		//取得USER对象
		public function getuser($where='',$rows='*')
		{
			...
			$ret = $m_user->where($calwhere)->field($rows)->select();
			if($ret === false)
			{
		...
		public function addprize($user,$num,$fromuser=null,$memo=null,$layer=0)
		{
			...
			if($memo !== null && $this->memo)
			{
	            ...
	            $comdata["userid"]   =$user["id"];      //得到奖金的用户的ID
	            $comdata["编号"]     =$user["编号"];    //得到奖金的用户的编号
	            $comdata["fromid"]   =$fromuser["id"];  //从哪个用户身上获得奖金的用户ID
			...
			if($this->K != 0)
			{
                ...
                if($this->K != 0 && $this->K < $krate_temp && $krate_temp > 0 )
                {
					...
					if($this->Kwhere!=''){
						...
						$tmpuser = array_intersect_key($dataarr,M('用户')->where($this->Kwhere)->getfield('id idkey,id'));
						//找到可以K的用户
						if($tmpuser)
						{
