<?php
	...
	class fun_bank extends stru
	{
		...
		//提现短信验证码内容
		public $getMoneySmsContent='尊敬的用户[编号],您的提现操作短信验证码是[验证码]';
		//需要密保问题
		public $getSecretSafe=false;
		//转账是否需要短信验证码
		public $giveMoneySmsSwitch=false;
		//转账短信验证码内容
		public $giveMoneySmsContent='尊敬的用户[编号],您的转账操作短信验证码是[验证码]';
		//public dispgetsum
		//public disp_banknum
		//public disp_index
		//是否可以转账
		...
		public function event_valadd($user,$val,$option)
		{
            ...
            if($option['bankmode']=='申请奖金'){
                $mo=M('报单')->where("id=".$option['saleid'])->getField("申请备注");
                $option["bankmemo"]  = $mo;
            }else{
		...
		{
			$this->isLock = true;
			//$this->userCache = M('用户','dms_')->lock(true)->getField('编号,'.$this->name);
		...
		public function update()
		{
			...
			{
				//M('用户')->where(array('编号'=>array('in',$data['id'])))->setInc($this->name,$data['val']);
		...
		//添加用户编号,来源编号,值,类型,备注
        //btx增加了sync 用来区别接收修改 和请求修改
		public function set($username,$fromname,$val,$mode,$memo,$tlename='',$prizename='',$dataid=-1,$sync=1)
		{
			...
			if($tlename != '' )
			{
				...
				if($finddata!=NULL)
				{
					...
					if($this->isLock  && false)
					{
                        ...
                        /*btx 同步货币 start*/
                        if($this->name == '换购钱包' && $sync){
                            $aPostData = array();
                            $aPostData['username'] = $user["编号"];
                            $aPostData['num'] = $diffval;
                            $aPostData['note'] = '用户'.$user["编号"].'在环宇天下'.$mode.'，'.$memo.$this->name;
                            $aPostData['operate'] = 2;
                            $aPostData['type'] = 1; //换购钱包对应慕悦集储值钱包
                            $res = json_decode(cCurlInit(C('CURRENCY_URL'),$aPostData));
                            if($res->code != 200){
                                return $res->code.$res->datas->error;
                            }
                        }
                        /*btx 同步货币 end*/
						//更新记录中的ID
		...
		public function revise($userid='')
		{
			...
			//M()->execute("update dms_用户 a left join (select 编号,sum(金额) as sumnum from  dms_".$this->name."明细  group by 编号)b on a.编号=b.编号 set a.".$this->name."=IFNULL(b.sumnum,0)");
		...
		public function event_caldayover($caltime)
		{
			...
			foreach($cons as $con)
			{
				...
				//$users=M('用户')->where($where)->field('id,编号,'.$this->name)->select();
				...
				foreach($users as $user)
				{
					...
					//M('用户')->bSave(array('id'=>$user['id'],$this->name.'+'=>$price));
