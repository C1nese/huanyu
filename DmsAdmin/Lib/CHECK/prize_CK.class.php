	...
	class prize_CK
	{
		...
		public function checklj($tlename='',$prizename='',$prizemode=0)
		{
			...
			if(is_array($tlename)){
				...
				$rs=M('用户')->alias('a')->join("inner join (SELECT 编号,sum(收入)cnt FROM (".$tblstr.")c  group by 编号)b on a.编号=b.编号 and a.累计收入<>b.cnt")->getField('id');
				dump(M()->_sql());
				if(!empty($rs)){
					return "<span style='color:red;'>用户表中累计收入与实际收入不符</span><br>";
				}else{
					return 1;
				}
			}else{
				if($prizemode==1){
					$rs=M('用户')->alias('a')->join("inner join (SELECT 编号,sum(".$prizename.")cnt FROM `dms_".$tlename."`  group by 编号)b on a.编号=b.编号 and a.".$prizename."累计<>b.cnt")->getField('id');
					if(!empty($rs)){
						return "<span style='color:red;'>用户表中".$prizename."与奖金记录不符</span><br>";
					}else{
						return 1;
					}
				}elseif($prizemode==2){
					$rs=M('用户')->alias('a')->join("inner join (SELECT 编号,sum(".$prizename.")*(-1) cnt FROM `dms_".$tlename."`  group by 编号)b on a.编号=b.编号 and a.".$prizename."累计<>b.cnt")->getField('id');
					if(!empty($rs)){
						return "<span style='color:red;'>用户表中".$prizename."与奖金记录不符</span><br>";
					}else{
						return 1;
		...
		{	
			...
			//按照 编号,时间排序查奖金表
			$olddata=mysql_unbuffered_query("select id,编号,用户级别,计算日期".$newrows." from dms_".$tlename." order by 编号,计算日期",$db->_linkID);
