...
$(document).ready(function() {
	...
	$( "#slider-range" ).slider({
		...
		slide: function( event, ui ) {
			$( ".price-filter" ).text( "$" + ui.values[ 0 ] + " - $" + ui.values[ 1 ] );
		}
	});
	$( ".price-filter" ).text( "$" + $( "#slider-range" ).slider( "values", 0 ) +" - $" + $( "#slider-range" ).slider( "values", 1 ) );
	
	$('.plus').click(function(){
		qty_field = $(this).parent('.quantity').find('.qty');
