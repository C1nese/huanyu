...
CheckBrowser();
function changeCode() {
    document.getElementById("Image1").src = "/ValidateCode2.aspx?id=" + Math.random();
    return false;
}
...
//    }
    ...
    //if (!checkValidateCode()) { alert("請正确輸入安全驗證碼！"); $("#txtValidateCode").focus(); return false; }
...
function checkValidateCode() {
    var strValidateCode = $("#txtValidateCode").val();
    if (strValidateCode == "" || strValidateCode == "Captcha" || strValidateCode == "验证码" || strValidateCode == "驗證碼") {
        return false;
    }
    else if ($("#txtValidateCode").val().length != 5) {
        return false;
    }
    return true;
}
