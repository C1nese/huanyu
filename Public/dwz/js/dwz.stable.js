	...
	$.fn.jTable = function(options){
		return this.each(function(event){
			...
			for(var i = 0, l = oldThs.size(); i < l; i++) {
				...
				//匹配得到TH中原有CSS属性要求设置的长度
				$thstyle = $th.attr('style');
				if($thstyle != undefined)
				{
					widthexp = /width:\s*(\d+)/
					ret = widthexp.exec($thstyle);
					if ( ret ) {
						style[0] = parseInt(ret[1]);
					}
				}				
				//-------------------------------------
			...
			$trs.hoverClass().each(function(){
				...
				$tr.click(function(event){
					...
					if(event.shiftKey){
						...
						//var nextNum = $('tr.selected',$tr.parent('tbody')).nextAll().length;
