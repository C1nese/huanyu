...
if(alertMsg){
alertMsg.error("<style>.alert{width:100%;margin-left: -50%;}</style><div>Http status: "+xhr.status+" "+xhr.statusText+"</div>"+"<div>ajaxOptions: "+ajaxOptions+"</div>"+"<div>thrownError: "+thrownError+"</div>"+"<div>"+xhr.responseText+"</div>");}else{
...
if(ckboxed==0){$(">div>.ckbox>input",parent).attr('checked',false);}
else{
$(">div>.ckbox>input",parent).attr('checked',true);}
...
if ($.fn.rightSelect) $("a[rSelect=true]", $p).rightSelect();
}(function($){
$.fn.extend({
...
return false;});
if($.isFunction(op.callback))op.callback(response);}});}},
open:function(url,dlgid,title,options,data){
...
desc:$this.attr("desc")||"desc"});});}});})(jQuery);
...
$tr.bind("selectstart",function(event){return !(event.shiftKey || event.ctrlKey);});
$(document).keydown(function(event){
	if(event.shiftKey || event.ctrlKey){
		$trs.css('-moz-user-select','none');
	}
}).keyup(function(event){
	$trs.css('-moz-user-select','');
});
...
var $callback=callback||navTabAjaxDoneReload;
