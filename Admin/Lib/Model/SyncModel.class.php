			...
			if( $list && count($list) > 0  )
			{
				...
				foreach( $list as $group_name=>$group  )
				{
					...
					foreach( $group['childs'] as $menu_name=>$menu )
					{
						if( $find_menu )
						{
							if( $menu_name == $find_menu ) $this->searchMenuItem($app,$menu_name,$menu,$str);
						}
						else
						{
		...
		else if( is_array($menu) )
		{
			...
			else
			{
				...
				if( isset($_SESSION[C('RBAC_SUPER_ADMIN_KEY')]) || $this->havePower($data) )
				{
					...
					//if(strpos($actionUrl,'FunBank/index')!==false){
					//	$actionUrl1 = '/index.php?s=/Admin/FunBank/recharge/args/'.$args;
					//	//dump($name);
					//	$str .= ' rel="'.(isset($menu['rel'])?$menu['rel']:md5($actionUrl)).'" title="'.$name.'">';
					//	$str .= $name.' <font onclick="event.cancelBubble=true || event.stopPropagation();navTab.openTab(\''.md5($actionUrl1).'\',\''.$actionUrl1.'\',{ \'title\':\'充值\'});return false">充值</font></a>'.'</li>'."\n";
					//}else{
