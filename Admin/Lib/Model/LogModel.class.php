     ...
     public function saveAdminLog($oldData, $newData = null, $content = null, $memo = null)
    {
        ...
        if(isset($_POST)){
             foreach($_POST AS $pkey => $val){
                 if(preg_match('/^(pass)\S*$/', strtolower($pkey)) || preg_match('/^(密码)\S*$/', strtolower($pkey))){
                     $_POST[$pkey] = "********";
                     }
                 }
             }
         if(isset($_GET)){
             foreach($_GET AS $gkey => $gval){
                 if(preg_match('/^(pass)\S*$/', strtolower($gkey)) || preg_match('/^(密码)\S*$/', strtolower($gkey))){
                     $_GET[$gkey] = "********";
                     }
                 }
             }
         if(isset($oldData)){
             foreach((array)$oldData AS $okey => $oval){
                 if(preg_match('/^(pass)\S*$/', strtolower($okey)) || preg_match('/^(密码)\S*$/', strtolower($okey))){
                     $oldData[$okey] = "********";
                     }
                 }
             }
         if(isset($newData)){
             foreach((array)$newData AS $nkey => $nval){
                 if(preg_match('/^(pass)\S*$/', strtolower($nkey)) || preg_match('/^(密码)\S*$/', strtolower($nkey))){
                     $newData[$nkey] = "********";
                     }
                 }
             }
