...
class TableListAction extends Model{
    ...
    public function getHtml($actionobj=null){
        $this->resetShow();
        if($this->excel && isset($_GET["excel"]) && $_GET["excel"] == '1'){
            $this->excel($actionobj);
        }else if($this->showSearch && isset($_GET["search"]) && $_GET["search"] == '1'){
            $this->searchHtml();
        }else if(isset($_GET["_editList"]) && $_GET["_editList"] == '1'){
            $this->editList();
        }else if(isset($_GET["_conformEditList"]) && $_GET["_conformEditList"] == '1'){
            $this->conformEditList();
        }else if(isset($_GET["_resetEditList"]) && $_GET["_resetEditList"] == '1'){
            $this->resetEditList();
        }else if(isset($_GET["_getsumList"]) && $_GET["_getsumList"] == '1'){
            $this->getsumList();
        }else{
        ...
        foreach($_REQUEST as $key=>$value){
            ...
            if($i == $k){
                ...
                if(isset($svalue['searchSql'])){
					...
					if($_REQUEST[substr($key,0,-5).'end'] == ''){
    ...
    protected function showTable($actionobj){
            //  排序
            if(isset($_POST["_order"]) && isset($_POST["_sort"]) && $_POST["_order"] !="" && $_POST["_sort"] != ""){
                $this->order("{$_POST["_order"]} {$_POST["_sort"]}");
            }
            $options=$this->options;                                        // Model中 查询表达式参数
            //获得加入搜索条件后 新的查询表达式参数和搜索条件数
            $searchResult = $this->doSearch($this->setSearch,$options); 
            $this->options = $searchResult[0];                              //  带搜索条件的查询表达式参数
			if(isset($_POST["searchExcel"]) && $_POST["searchExcel"] == '1'){
				$this->excel();
			}
			...
			//判断打开页面是否袭击显示内容
			$search=false;
			if($this->showAll==false && isset($_REQUEST)){
				foreach($_REQUEST as $rfkey=>$rfval){
            ...
            if(isset($_REQUEST["loadNextPage"]) && $_REQUEST["loadNextPage"]==1){
    ...
    public function tableList($con,$count,$firstRow,$list,$actionobj=null,$options1=null){
        ...
        $p=isset($_REQUEST['p'])?$_REQUEST['p']:'1';        //当前页数
	    ...
	    $parseStr  .= '<input type="hidden" name="_order" value="'.(isset($_REQUEST["_order"])?$_REQUEST["_order"]:'').'"/>'."\r\n";
	    $parseStr  .= '<input type="hidden" name="_sort" value="'.(isset($_REQUEST["_sort"])?$_REQUEST["_sort"]:'').'"/>'."\r\n";
        ...
        foreach($search as $key=>$value){
            if(isset($value["searchPosition"]) && $value["searchPosition"] == "top"){
                if($value["searchMode"]=="date" ){
                    $topSearch  .= '<li><label>'.$key.'：</label><input type="text" name="_search'.$i.'_start" class="date" style="width:90px" value="'.(isset($_REQUEST["_search{$i}_start"])?$_REQUEST["_search{$i}_start"]:'').'"> - <input style="width:90px" type="text" name="_search'.$i.'_end" class="date" value="'.(isset($_REQUEST["_search{$i}_end"])?$_REQUEST["_search{$i}_end"]:'').'"></li>';  
                    ...
                    if(isset($value["searchSelect"])){
                        ...
                        foreach($value["searchSelect"] as $selectkey => $selectvalue){
                            if(isset($_REQUEST["_search{$i}_start"]) && $_REQUEST["_search{$i}_start"]!=""&& $_REQUEST["_search{$i}_start"]!="" && $_REQUEST["_search{$i}_start"]==$selectvalue){
                        ...
                        foreach($value["searchSelect"] as $selectkey => $selectvalue){
                            if(isset($_REQUEST["_search{$i}_end"]) && $_REQUEST["_search{$i}_end"]!=""&& $_REQUEST["_search{$i}_end"]==$selectvalue){
                                $topSearch .='<option value="'.$selectvalue.'" selected>'.$selectkey.'</option>';
                            }else{
                                $topSearch .='<option value="'.$selectvalue.'">'.$selectkey.'</option>';
                            }
                        ...
                        $topSearch  .= '<li><label>'.$key.'：</label><input style="width:90px" type="text" name="_search'.$i.'_start" value="'.(isset($_REQUEST["_search{$i}_start"])?$_REQUEST["_search{$i}_start"]:'').'"> - <input style="width:90px" type="text" name="_search'.$i.'_end" value="'.(isset($_REQUEST["_search{$i}_end"])?$_REQUEST["_search{$i}_end"]:'').'"></li>'."\r\n";  
                    }
                    ...
                    if(false and isset($value['searchGet'])){
						...
						if(isset($_GET[$inputName])){
							$isutf8 = preg_match('/^.*$/u', $_GET[$inputName]) > 0;
							if(!$isutf8){
								$inputval =  iconv('GB2312','UTF-8',$_GET[$inputName]);
							}else{
						...
						if(isset($_REQUEST[$inputName])){
							$inputval = trim($_REQUEST[$inputName]);
						}
					}
                    if(isset($value['searchSelect'])){
                        $topSearch  .= '<li><label>'.$key.'：</label>';
                        ...
                        foreach($value["searchSelect"] as $selectkey => $selectvalue){
                            if(isset($_REQUEST[$inputName]) && $_REQUEST[$inputName]!=""&& $_REQUEST[$inputName]==$selectvalue){
                                $topSearch .='<option value="'.$selectvalue.'" selected>'.$selectkey.'</option>';
                            }else{
                                $topSearch .='<option value="'.$selectvalue.'">'.$selectkey.'</option>';
                            }
						...
						if(isset($value["searchFuzzy"])){
							if(urldecode($_REQUEST["_search{$i}_mohu"])){$mohu=checked;}else{$mohu="";}
        ...
        if($this->excel  && empty($setButton['登陆锁定'])){
        ...
        $request = array();
        foreach(array_merge($_POST,$_GET) as $rk=>$req){
            if($req !="")
            $request[$rk] = $req;
        }
    ...
    public function getTbodyData($datasource,$count,$options1,$colNum=1){
		...
		if(isset($options1['order']) && strstr($options1['order'],'desc')) 
    		...
    		$sort="asc";
    	if(isset($_POST['_sort'])) $sort=$_POST['_sort'];
    	$p=isset($_REQUEST['p'])?$_REQUEST['p']:'1';//页数
        $firstRow=($p-1)*$this->numPerPage;
		//遍历行数据
                ...
                if($value){
                    if(isset($value["sum"])){
                    	...
                    	if($count<=$this->sumPoint){//查询结果小于10000条自动汇总
                        	...
                        	if($m==0)$sum='<a style="cursor:pointer;color:#316eda;" onclick="getsumList(\''.$actionUrl.'/args/'.(isset($_REQUEST['args'])?$_REQUEST['args']:'').'/_getsumList/1\')"><span>刷新</span></a>';
                        }
    ...
    protected function doSearch($searchArray,$options){
		...
		foreach($this ->setShow as $kk => $vv){
			...
			if(isset($vv['searchGet']) && isset($_REQUEST[$vv['searchGet']]) && $_REQUEST[$vv['searchGet']] != ""){
				//$reGet =isset($_REQUEST[$vv['searchGet']])?base64_decode(str_replace(' ','+',$_REQUEST[$vv['searchGet']])): $_POST[$vv['searchGet']];
				if(isset($_REQUEST[$vv['searchGet']])){
					$isutf8 = preg_match('/^.*$/u', $_REQUEST[$vv['searchGet']]) > 0;
					if(!$isutf8){
						$reGet =  iconv('GB2312','UTF-8',$_REQUEST[$vv['searchGet']]);
					}else{
						$reGet = $_REQUEST[$vv['searchGet']];
					}
				}else{
					$reGet = $_POST[$vv['searchGet']];
				}
				$whereArr[$row] = array('eq',$reGet);
                $whereStr .=  $row . "='$reGet' AND ";
			}
			//dump($vv);
			if(isset($vv['searchGetStart']) && isset($_REQUEST[$vv['searchGetStart']]) && $_REQUEST[$vv['searchGetStart']] != ""){
				$whereArr[$row][] = array("egt",$_REQUEST[$vv['searchGetStart']]);
				$whereStr .=  $row . ">='{$_REQUEST[$vv['searchGetStart']]}' AND ";
			}
			if(isset($vv['searchGetEnd']) && isset($_REQUEST[$vv['searchGetEnd']]) && $_REQUEST[$vv['searchGetEnd']] != ""){
				$whereArr[$row][] = array("elt",$_REQUEST[$vv['searchGetEnd']]);
				$whereStr .=  $row . "<='{$_REQUEST[$vv['searchGetEnd']]}' AND ";
			}
		}
		$i = 0;$con=array();
    ...
    public function conformEditList(){
        ...
        $data["标题"]=$titleList;
        M()->startTrans();
        $result=M()->execute("update xtable_set set 显示='{$showList}',排序='{$sortList}',标题='{$titleList}' where 地址='{$url}'");
        if(!$result){
            echo "没有发生更改！";
        }
        M()->commit();
    ...
    function searchHtml(){
        ...
        foreach($this->setSearch as $key=>$value){
            if(isset($value["searchMode"]) && $value["searchMode"]=="date" ){
                if($d % 2 == 0){$datestr  .= '<tr>'."\r\n";}
                $datestr  .= '<td style="width:80px;text-align:right;padding-right:3px">'.$key.'</td><td style="width:220px;text-align:left"><input type="text" name="_search'.$i.'_start" class="date" style="width:90px" value="'.urldecode(isset($_GET["_search{$i}_start"])?$_GET["_search{$i}_start"]:'').'"> - <input style="width:90px" type="text" name="_search'.$i.'_end" class="date" value="'.urldecode(isset($_GET["_search{$i}_end"])?$_GET["_search{$i}_end"]:'').'"></td>'."\r\n";
                 if($d % 2 == 1){$datestr  .= '</tr>'."\r\n";}
                 $d++;
            }else if(isset($value["searchMode"]) && $value["searchMode"]=="num"){
                if($n % 2 == 0){$numstr  .= '<tr>'."\r\n";}
                if(isset($value["searchSelect"])){
                    $numstr  .= '<td style="width:80px;text-align:right;padding-right:3px">'.$key.'</td><td style="width:220px;text-align:left"><nobr><input style="width:90px" type="text" name="_search'.$i.'_start" value="'.urldecode(isset($_GET["_search{$i}_start"])?$_GET["_search{$i}_start"]:'').'" onfocus="selectstr(\'_selectsearch'.$i.'_start\',event)"> - <input style="width:90px" type="text" name="_search'.$i.'_end" value="'.urldecode(isset($_GET["_search{$i}_end"])?$_GET["_search{$i}_end"]:'').'" onfocus="selectstr(\'_selectsearch'.$i.'_end\',event)"></nobr></td>'."\r\n";  
                    ...
                    $numstr  .= '<td style="width:80px;text-align:right;padding-right:3px">'.$key.'</td><td style="width:220px;text-align:left"><nobr><input style="width:90px" type="text" name="_search'.$i.'_start" value="'.urldecode(isset($_GET["_search{$i}_start"])?$_GET["_search{$i}_start"]:'').'"> - <input style="width:90px" type="text" name="_search'.$i.'_end" value="'.urldecode(isset($_GET["_search{$i}_end"])?$_GET["_search{$i}_end"]:'').'"></nobr></td>'."\r\n";  
                }
                if($n % 2 == 1){$numstr  .= '</tr>'."\r\n";}
                $n++;
            }else if(isset($value["searchMode"]) && $value["searchMode"]=="text"){
                ...
                if(isset($value["searchSelect"])){
                    $otherstr  .= '<td style="width:80px;text-align:right;padding-right:3px">'.$key.'</td><td style="width:220px;text-align:left"><nobr><input style="width:90px" type="text" name="_search'.$i.'_other"  value="'.urldecode(isset($_GET["_search{$i}_other"])?$_GET["_search{$i}_other"]:'').'" onfocus="selectstr(\'_selectsearch'.$i.'_other\',event)"></td>';
                    ...
                    $otherstr  .= '<td style="width:80px;text-align:right;padding-right:3px">'.$key.'</td><td style="width:220px;text-align:left"><nobr><input style="width:90px" type="text" name="_search'.$i.'_other"  value="'.urldecode(isset($_GET["_search{$i}_other"])?$_GET["_search{$i}_other"]:'').'">';
                    if(!isset($value["searchFuzzy"])||$value["searchFuzzy"]){
                        if(urldecode(isset($_GET["_search{$i}_mohu"])?$_GET["_search{$i}_mohu"]:'')){$mohu="checked";}else{$mohu="";}
    ...
    protected function excel_old($actionobj){
        ...
        if(!isset($_POST['searchExcel']) || $_POST['searchExcel'] != '1'){
			$this->options['where'] = unserialize(base64_decode($_REQUEST['_where']));
		}
    ...
    protected function excel($actionobj){
        ...
        if(!isset($_POST['searchExcel']) || $_POST['searchExcel'] != '1'){
			$this->options['where'] = unserialize(base64_decode($_REQUEST['_where']));
			/*btx 加密过程产生斜线 解密时_where就会缺少一部分 start*/ 
			if(!$this->options['where'] && !empty($_REQUEST['_where'])){
				$key = array_search($_REQUEST['_where'],$_REQUEST['_URL_']);
				foreach($_REQUEST['_URL_'] as $k){
					if($k >= $key){
						$_REQUEST['_where'] .= '/'.$k;
					}
				}
			...
			$this->options['where'] = unserialize(base64_decode($_REQUEST['_where']));
			/*btx 加密过程产生斜线 解密时_where就会缺少一部分 end*/
		}
        ...
        foreach($show as $showtitle=>$showval) {
           if(!$showval || (isset($showval["excel"]) && !$showval["excel"])){continue;}
           $objPHPExcel->setActiveSheetIndex(0)->setCellValue(chr($asc).'1', strval($showtitle));
           $objPHPExcel->getActiveSheet()->getStyle(chr($asc).'1')->getFont()->getColor()->setARGB(PHPExcel_Style_Color::COLOR_WHITE); 
           $objPHPExcel->getActiveSheet()->getStyle(chr($asc).'1')->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);   
           $objPHPExcel->getActiveSheet()->getStyle(chr($asc).'1')->getFill()->getStartColor()->setARGB('0098BEEE'); 
           $asc++;
        }
            ...
            foreach($show as $showkey=>$showval) {
                ...
                if(isset($showval["excelMode"]))
                {
                	if($showval["excelMode"]=='text'){
                		$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit(chr($asc).$no, $showrow,PHPExcel_Cell_DataType::TYPE_STRING);
                	}else{
                		$objPHPExcel->setActiveSheetIndex(0)->setCellValue(chr($asc).$no, $showrow);
                	}
                }
                else
                {
