...
class BackupAction extends CommonAction  {
    ...
    //初始化数据
     ...
     public function _initialize() {
        ...
        $this->config = array(
            'path' => ROOT_PATH."/dbbackup/",                          //备份文件存在哪里
	...
	*
	*/
	public function project_data_reset()
	{
		//如果不是调试模式禁止调用
		if( !defined('APP_DEBUG') ||  !APP_DEBUG )
		{
			$this->error('只允许调试模式执行!');
		}
		if( isset($_POST['submit']) )
		{
			$project_ids		= $_POST['project'];
			$Node				= M('Node');

			$sql_array			= array(); //待执行的sql数组
			foreach( $project_ids as $id )
			{
				$app			= $Node->find($id);
				$methodUrl		= $app['group']==''?
				"{$app['name']}://Sync{$app['name']}/returnSqlStr"
				:
				"{$app['name']}://{$app['group']}/Sync{$app['name']}{$app['group']}/returnSqlStr";
				$sql_content	= R($methodUrl);

				//$sql_array		= array_merge($sql_array,explode(';',$sql_content));
				$sql_array		= array_merge($sql_array,$this->parse_sql_str($sql_content));
			...
			{
				$result = $Node->execute($sql_line);
				if( $result === false )
				{
					$this->error("执行失败:".$sql_line);
				}
			}

			$sql_total = count($sql_array);
			$this->success("项目数据 {$sql_total} 条,重置成功!");
		}
		else
		{
			//读取当前已安装的项目的列表
			$Node			= M('Node');
			$where['name']	= array('neq','Admin');
			$where['level']	= 1;
			$list			= $Node->where($where)->select();
   ...
   public  function clear(){
    ...
    private function backList(){
        ...
        foreach ($fileArr as $key => $value) {
            if ($key > 1) {
                ...
                //构建列表数组
                if(substr($value,-3)=='.gz'){
                    $shortname=substr($value,33,-7);
                }else if(substr($value,-4)=='.zip'){
     ...
     */
 	function prebackall(){
 		$name='新数据备份';
 		$name = isset($_POST["backname"]) ? $_POST["backname"] : $name;
    ...
    function backall($name='新数据备份',$return = false) {
    	...
    	G('run');
    	$stime = time();
        $name = isset($_GET["backname"]) ? $_GET["backname"] : $name;
        $backname=trim($name);
        $backname=str_replace(" ","",$backname);
        $backname = urlencode(base64_encode($backname));
        ...
        $bktype = 0;
		if($bktype==1){
			$fileName = $this->trimPath($this->config['path'] . md5(date('YmdHis').$encrypt_key) . 'B' . $backname. '.zip');
			import("COM.BakRec.BakRec");
			$BakRec = new BakRec();
		}else{
			$fileName = $this->trimPath($this->config['path'] . md5(date('YmdHis').$encrypt_key) . 'A' . $backname. '.zip');
        ...
        ini_set('memory_limit','-1');
 		...
 		if(adminshow('cliSwitch')){
	 		...
	 		if(IS_WIN){
		        ...
		        $real_path = $php_path . 'php.exe';
				chdir(ROOT_PATH);//更改当前工作路径
				$cmd = $real_path." ".ROOT_PATH."clibr.php Backup recover file,".$name." >recerr.log";
				pclose(popen("start /B ". $cmd, "r"));  
	 		}else{
	 			chdir(ROOT_PATH);
	 			$cmd="php ".ROOT_PATH."clibr.php Backup recover file,".$name." >recerr.log";
				exec($cmd . " &",$out,$re);
	 		}
	 		$this->success('正在还原中');
 		}else{
    ...
    function recover($name='') {
        ...
        if(substr($backname,-3)=='.gz'||substr($backname,32,1)=="A"||substr($backname,32,1)=="B"||substr($backname,-4)=='.sql'){
            ...
            $message = $BakRec->recoverFile($backname);
            if(IS_CLI){
				if ($message =="") {
	            ...
	            if ($message=="") {
					//$name=base64_decode(substr($backname,33,-4));
					$this->saveAdminLog('','',"数据库还原",'还原备份：'.$name);
					$t=true;
					B('SaveConfig',$t);
					////////////还原后修正数据//////////////
    ...
    function deletebak(){
		$succNum = 0;
		$errNum = 0; 
		foreach(explode(',',$_GET['file']) as $id){
			if($id == '') continue;
    ...
    function delectzi(){	
		...
		if($_REQUEST['shanj']!="" && $_REQUEST['shanq']!=""){
			$qitime=strtotime($_REQUEST['shanq']);
			$jietime=strtotime($_REQUEST['shanj']);
			$f=0;
    ...
    function cleandb() {
        ...
        //修改，添加yubicloud验证通过
        if(chkpass($_POST['repwd'],$result['password']) || chkyubicloud($_POST['repwd'],$_SESSION['loginAdminAccount'])){
    ...
    */
    ...
    function query_sql()
	{
	 ...
	 function querysql()
	{
		$sql	= trim(base64_decode($_REQUEST['sql']));
		$list	= M()->query($sql);
		if(!$list)
		{
