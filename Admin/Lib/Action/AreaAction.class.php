...
class AreaAction extends CommonAction 
{
	...
	*/
	public function update()
	{
		...
		CONFIG('default_country',isset($_POST['default_country'])?$_POST['default_country']:"");
	
		CONFIG('default_province',isset($_POST['default_province'])?$_POST['default_province']:"");
	
		CONFIG('default_city',isset($_POST['default_city'])?$_POST['default_city']:"");
	
		CONFIG('default_county',isset($_POST['default_county'])?$_POST['default_county']:"");
	
		CONFIG('default_town',isset($_POST['default_town'])?$_POST['default_town']:"");
	...
	function country_add_save(){
		M()->startTrans();
		$areaary=M("area")->where(array("地区级别"=>$_POST['lv']))->getField("地区编码,id,上级编码,地区名称");
		//判断订国家所在
		if($_POST['lv']==1){
			$country=$_POST['name'];
		}else{
			$country=$_POST['upname'];
		}
		...
		//保存到数据库
		if($_POST['lv']==1 && !isset($areaary[$countrykey])){
		...
		if($_POST['lv']==2 && (!isset($country_json[$countrykey]) || !array_key_exists($_POST['name'],$country_json[$countrykey]))){
			if(isset($_POST['oldname'])){
				$procincekeys=array_keys($country_json[$countrykey],$_POST['oldname']);
				$data["地区名称"]=$_POST['name'];
				M("area")->where(array("上级编码"=>$countrykey,"地区编码"=>$procincekeys['0']))->save($data);
				$country_json[$countrykey][$procincekeys['0']]=$_POST['name'];
			}else{
				$num=count($country_json[$countrykey]);
				$provincekey=$countrykey.str_pad($num,2,'0',STR_PAD_LEFT);
				$country_json[$countrykey][$provincekey]=$_POST['name'];
	...
	function area_add_save(){
		M()->startTrans();
		//判断上级的编码
		$countrykey=$this->transCountryCode($_POST['country']);
		//获取上级编码
		$uparea=M("area")->where("地区名称='".$_POST['upname']."' and 地区编码 like '".$countrykey."%' and 地区级别=".($_POST['lv']-1))->field("id,上级编码,地区名称,地区编码")->find();
		//找出文件
		...
		if(isset($_POST['oldname'])){
			//获取数据库数据
			$olddata=M("area")->where("上级编码='".$uparea['地区编码']."' and 地区编码 like '".$countrykey."%' and 地区名称='".$_POST['oldname']."' and 地区级别=".($_POST['lv']))->find();
			if($olddata){
				$olddata['地区名称']=$_POST['name'];
				M("area")->save($olddata);
			}else{
			...
			//找出本地区已有几个下级
			$maxarea=M("area")->where("上级编码='".$uparea['地区编码']."' and 地区编码 like '".$countrykey."%' and 地区级别=".($_POST['lv']))->order("地区编码 desc")->find();
			...
			$data["地区名称"]=$_POST['name'];
			$data["地区级别"]=$_POST['lv'];
			$data["是否末级"]=$_POST['lv']>=5?1:0;
			M("area")->add($data);
			$area_json[$uparea['地区编码']][$areakey]=$_POST['name'];
		}
	...
	function area_delete(){
		...
		if(isset($_POST['country'])){
			...
			if(isset($_POST['province'])){
				$provincekey=M("area")->where("地区名称='".$_POST['province']."' and 上级编码='".$upkey."' and 地区编码 like '".$upkey."%' and 地区级别=2")->getField("地区编码");
				$upkey=$provincekey;
				if(isset($_POST['city'])){
					$citykey=M("area")->where("地区名称='".$_POST['city']."' and 上级编码='".$upkey."' and 地区编码 like '".$upkey."%' and 地区级别=3")->getField("地区编码");
					$upkey=$citykey;
					if(isset($_POST['county'])){
						$countykey=M("area")->where("地区名称='".$_POST['county']."' and 上级编码='".$upkey."' and 地区编码 like '".$upkey."%' and 地区级别=4")->getField("地区编码");
						$upkey=$countykey;
					}
				}
			}
		}else{
			$countrykey=$this->transCountryCode($_POST['name']);
		}
		//找出当前地区所在的编码
		$deleteid=array();
		$areadata=M("area")->where("上级编码='".$upkey."' and 地区级别=".$_POST['lv']." and 地区名称='".$_POST['name']."'")->find();
		$downareas=M("area")->where("地区编码 like '".$areadata['地区编码']."%' and 地区级别>".$_POST['lv']."")->getField("id keyid,id,地区名称,地区编码,上级编码,地区级别");
		//清除上级所在的json数据
