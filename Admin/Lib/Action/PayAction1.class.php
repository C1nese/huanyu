...
class PayAction extends CommonAction{
    ...
    public $interface_data = array(
        ...
        'update_read' => true, // 当发现有新接口时，是否从新读取所有接口程序（用于支付接口更名等操作）
    ...
    public function config(){
         if(!empty($_POST['interface']) && !empty($this -> interface_data['data'][$_POST['interface']])){
    ...
    public function do_add(){
         if(!empty($_POST['payment']) && !empty($_POST['payment']) && !empty($_POST['name'])){
    ...
    public function do_pay_edit(){
         if(!empty($_POST['ids']) && !empty($_POST['name']) && !empty($_POST['attr']) && !empty($_POST['state'])){
             ...
             $res['credit'] = $attr[$payment . '_credit']?$attr[$payment . '_credit']:'No';
             ...
             if($upd_res){
                 if(!empty($_POST['secondSelect'])){
                     $banklist = F('banklist');
                     $banklist[$payment] = $this -> _post("secondSelect", "htmlspecialchars");
                     F('banklist', $banklist);
                     }
                 $this -> iud();
                 M('pay_onlineaccount', ' ') -> commit();
